...
/**
 * Accumulator for the (receiver, args) tuple captured at a wrapped invoke site,
 * paired with a
 * cached {@link MethodHandle} so the wrapper can re-invoke the original
 * implementation on
...
public final class MethodWrapper {
    ...
    /**
     * Bytecode entry point: pushes one argument onto the wrapper. Args are
     * accumulated in reverse
     * (the call site pushes them last-arg first), so we always insert at index 0 —
     * the final
