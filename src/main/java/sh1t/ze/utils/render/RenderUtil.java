...
public final class RenderUtil
    ...
    public static class ShadowTexture {
        public sh1t.ze.render.ResourceLocationWrapper resourceLocation = new sh1t.ze.render.ResourceLocationWrapper(
                "texture/remote/"
    ...
    public static void drawGradientV(PoseStack poseStack, float x, float y, float width, float height, int colorTop,
            int colorBottom) {
    ...
    public static void drawGradientH(PoseStack poseStack, float x, float y, float width, float height, int colorLeft,
            int colorRight) {
        RenderSystem.enableBlend();
        RenderSystem.defaultBlendFunc();
        RenderHelper.resetShaderColor();
    ...
    public static void drawDiamond(PoseStack poseStack, float centerX, float centerY, float size, float widthRatio,
            float bottomRatio, int color) {
        GL11.glEnable(3042);
        ...
        bufferBuilder.vertex(matrix4f, centerX - size / widthRatio, centerY + size, 0.0f).color(red, green, blue, alpha)
                .endVertex();
        bufferBuilder.vertex(matrix4f, centerX, centerY + size / bottomRatio, 0.0f).color(red, green, blue, alpha)
                .endVertex();
        bufferBuilder.vertex(matrix4f, centerX + size / widthRatio, centerY + size, 0.0f).color(red, green, blue, alpha)
                .endVertex();
        bufferBuilder.vertex(matrix4f, centerX, centerY, 0.0f).color(red, green, blue, alpha).endVertex();
    ...
    public static void drawRoundedRect(PoseStack poseStack, float x, float y, float width, float height, float radius,
            int color) {
        RenderUtil.drawRoundedRect(poseStack, x, y, width, height, radius, 1.0f, color);
    }

    public static void drawRoundedRect(PoseStack poseStack, float x, float y, float width, float height, float radius,
            float smoothness, int color) {
    ...
    public static void drawRoundedRectCorners(PoseStack poseStack, float x, float y, float width, float height,
            float radius, boolean topLeft, boolean topRight, boolean bottomLeft, boolean bottomRight, int color) {
        ...
        if (topRight) {
            ...
            RenderUtil.drawRoundedRect(poseStack, x + width - radius * 2.0f, y, radius * 2.0f, radius * 2.0f, radius,
                    color);
            RenderUtil.popScissor();
        } else {
        ...
        if (bottomLeft) {
            ...
            RenderUtil.drawRoundedRect(poseStack, x, y + height - radius * 2.0f, radius * 2.0f, radius * 2.0f, radius,
                    color);
            RenderUtil.popScissor();
        } else {
        ...
        if (bottomRight) {
            ...
            RenderUtil.drawRoundedRect(poseStack, x + width - radius * 2.0f, y + height - radius * 2.0f, radius * 2.0f,
                    radius * 2.0f, radius, color);
            RenderUtil.popScissor();
        } else {
    ...
    public static void drawBlurredRect(PoseStack poseStack, float x, float y, float width, float height, float radius,
        ...
        try {
            ...
            if (textureTarget.width != RenderUtil.mainRenderTarget.width
                    || textureTarget.height != RenderUtil.mainRenderTarget.height) {
                textureTarget.resize(RenderUtil.mainRenderTarget.width, RenderUtil.mainRenderTarget.height,
                        Minecraft.ON_OSX);
            }
            Matrix4f matrix4f = poseStack.last().pose();
            ...
            if (shouldBlur) {
                ...
                RenderHelper.blitRenderTarget(mainRenderTarget, blitPoseStack, textureTarget.width,
    ...
    public static void drawTexturedRect(PoseStack poseStack, int x, int y, int width, int height, int u, int v,
    ...
    public static void drawQuad(BufferBuilder bufferBuilder, Matrix4f matrix4f, float left, float top, float right,
    ...
    public static void drawSpiralEffect(PoseStack poseStack, Entity entity, float partialTicks) {
        ...
        double extraOffset = (double) entityHeight / 1.2 * (progress > 0.5 ? 1.0 - progress : progress)
                * (double) (secondHalf ? -1 : 1);
    ...
    public static void drawColoredBox(AABB aABB, PoseStack poseStack, Color topColor, Color bottomColor) {
        ...
        bufferBuilder.vertex(matrix4f, minX, minY, minZ)
                .color(bottomColor.getRed(), bottomColor.getGreen(), bottomColor.getBlue(), bottomColor.getAlpha())
                .endVertex();
        ...
        bufferBuilder.vertex(matrix4f, minX, maxY, minZ)
                .color(topColor.getRed(), topColor.getGreen(), topColor.getBlue(), topColor.getAlpha()).endVertex();
    ...
    public static void drawTexture(ResourceLocation resourceLocation, PoseStack poseStack, float x, float y,
        ...
        RenderUtil.drawTexture(mc.getTextureManager().getTexture(resourceLocation).getId(), poseStack, x, y, width,
                height, alpha, color);
    }
    ...
    public static void drawShadow(PoseStack poseStack, float x, float y, float width, float height, int blurRadius,
            int color) {
        int cacheKey;
        x -= (float) blurRadius;
        y -= (float) blurRadius;
        if (!shadowCache.containsKey(
                cacheKey = (int) ((width += (float) (blurRadius * 2)) * (height += (float) (blurRadius * 2))
                        + width * (float) blurRadius))) {
            BufferedImage bufferedImage = new BufferedImage((int) width, (int) height, 2);
            Graphics graphics = bufferedImage.getGraphics();
            graphics.setColor(new Color(-1));
            graphics.fillRect(blurRadius, blurRadius, (int) (width - (float) (blurRadius * 2)),
                    (int) (height - (float) (blurRadius * 2)));
            graphics.dispose();
    ...
    public static int lerpColorHSB(int colorA, int colorB, float progress) {
        ...
        int alpha = (int) ((float) (colorA >> 24 & 0xFF)
                + (float) ((colorB >> 24 & 0xFF) - (colorA >> 24 & 0xFF)) * progress);
