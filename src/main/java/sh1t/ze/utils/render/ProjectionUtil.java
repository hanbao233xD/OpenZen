...
public final class ProjectionUtil
    ...
    public static void updateMatrices() {
        ...
        tempVec4a.set((float) viewport.get(0), (float) viewport.get(1), (float) viewport.get(2),
                (float) viewport.get(3));
    }
    ...
    public static Vector2f project(double worldX, double worldY, double worldZ, float partialTicks) {
        ...
        Vector3f relativePos = new Vector3f((float) (cameraPos.x - worldX), (float) (cameraPos.y - worldY),
                (float) (cameraPos.z - worldZ));
        ...
        double fov = 1.2f;
        try {
            Method method = mc.gameRenderer.getClass().getDeclaredMethod(ReflectionUtil
                    .getMappedMethodName(mc.gameRenderer.getClass(), "getFov", "(Lnet/minecraft/client/Camera;FZ)D"),
                    Camera.class, Float.TYPE, Boolean.TYPE);
            method.setAccessible(true);
            fov = (Double) method.invoke(mc.gameRenderer,
                    new Object[] { mc.getEntityRenderDispatcher().camera, partialTicks, true });
    ...
    private static void applyBobbing(Player player, Vector3f relativePos, float partialTicks) {
        ...
        Quaternionf pitchRotation = new Quaternionf().rotationX(
                Math.abs(Mth.cos(walkProgress * (float) Math.PI - 0.2f) * bobAmount) * 5.0f * ((float) Math.PI / 180));
        pitchRotation.conjugate();
        relativePos.rotate(pitchRotation);
        Quaternionf rollRotation = new Quaternionf()
                .rotationZ(Mth.sin(walkProgress * (float) Math.PI) * bobAmount * 3.0f * ((float) Math.PI / 180));
        rollRotation.conjugate();
        relativePos.rotate(rollRotation);
        Vector3f bobOffset = new Vector3f(Mth.sin(walkProgress * (float) Math.PI) * bobAmount * 0.5f,
    ...
    private static Vector2f projectInternal(Vector3f relativePos, double fov) {
        ...
        if (relativePos.z() < 0.0f) {
            return new Vector2f(-relativePos.x() * scale + (float) mc.getWindow().getGuiScaledWidth() / 2.0f,
                    (float) mc.getWindow().getGuiScaledHeight() / 2.0f - relativePos.y() * scale);
        }
