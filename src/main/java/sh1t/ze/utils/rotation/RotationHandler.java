...
import sh1t.ze.event.impl.FallFlyingEvent;
import sh1t.ze.event.impl.RotationAnimationEvent;
import sh1t.ze.event.impl.JumpMarkerEvent;
...
public class RotationHandler
    ...
    public void onTickHigh(TickEvent tickEvent) {
        if (mc.player != null) {
            ...
            } else if (helper != null && helper.isEnabled() && helper.hasTargetRotation()
                    && Helper.targetRotation != null) {
                RotationHandler.setTargetRotation(Helper.targetRotation);
            } else if (antiWeb != null && antiWeb.isEnabled() && AntiWeb.currentPhase != AntiWeb.Phase.IDLE
            ...
            } else if (killAura != null && killAura.isEnabled() && KillAura.target != null
                    && killAura.rotation != null) {
                RotationHandler
                        .setTargetRotation(new Rotation(killAura.rotation.getYaw(), killAura.rotation.getPitch()));
