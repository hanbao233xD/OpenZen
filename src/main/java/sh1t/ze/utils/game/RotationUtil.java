...
public final class RotationUtil
    ...
    public static Rotation smoothRotation(Rotation current, Rotation target, double speed) {
        ...
        for (int i = 1; i <= (int) (2.0 + Math.random() * 2.0); ++i) {
            ...
            if ((snapped = (candidate = new Rotation(targetYaw, targetPitch))
                    .snapToSensitivity(mc.options.sensitivity().get().floatValue())) == null)
                continue;
    ...
    public static boolean isLookingAt(float maxAngle, LivingEntity livingEntity) {
        ...
        Vec3 targetPos = new Vec3(livingEntity.getX(), livingEntity.getY() + (double) livingEntity.getBbHeight() * 0.5,
                livingEntity.getZ());
    ...
    public static Rotation rotationToBlock(BlockPos blockPos, float partialTicks) {
        Vec3 predictedPos = new Vec3(mc.player.getX() + mc.player.getDeltaMovement().x * (double) partialTicks,
                mc.player.getY() + (double) mc.player.getEyeHeight()
                        + mc.player.getDeltaMovement().y() * (double) partialTicks,
        ...
        return RotationUtil.rotationFromDeltas(RotationUtil.addNoise(dx), RotationUtil.addNoise(dy),
                RotationUtil.addNoise(dz));
    }
    ...
    public static HitResult raycastForBB(AABB aABB, Rotation rotation, Vec3 eyePos, double range) {
        ...
        return ProjectileUtil.getEntityHitResult(mc.player, eyePos, endPos, aABB,
    ...
    public static float ballisticPitch(float horizontalDist, float verticalDist, float velocity, float gravity) {
        float discriminant = velocity * velocity * velocity * velocity
                - gravity * (gravity * (horizontalDist * horizontalDist) + 2.0f * verticalDist * (velocity * velocity));
        return (float) Math.toDegrees(Math.atan(
                ((double) (velocity * velocity) - Math.sqrt(discriminant)) / (double) (gravity * horizontalDist)));
    }
    ...
    public static float[] getBallisticAngles(Vec3 target) {
        ...
        block0: for (float pitch = 90.0f; pitch >= -90.0f; pitch -= 0.5f) {
            ...
            for (int i = 0; i < 300; ++i) {
                ...
                projectileVel = new Vec3(projectileVel.x * drag, projectileVel.y * drag - gravity,
                        projectileVel.z * drag);
                if (projectilePos.y < (double) (mc.level.getMinBuildHeight() - 10))
                    continue block0;
                double traveledDist = Math
                        .sqrt(Math.pow(projectilePos.x - eyePos.x, 2.0) + Math.pow(projectilePos.z - eyePos.z, 2.0));
                if (!(traveledDist >= horizontalDist))
                    continue;
                if (Math.abs(projectilePos.y - target.y) < 1.0) {
                    return new float[] { Mth.wrapDegrees(yaw), Mth.wrapDegrees(pitch) };
                }
                if (projectileVel.y < 0.0 && projectilePos.y < target.y)
                    continue block0;
            }
        }
        return null;
    ...
    public static RotationUtil.BestHitInfo getBestHit(Entity entity) {
        ...
        for (Vec3 samplePoint : samplePoints) {
            ...
            if (!RotationUtil.isHitValid(eyePos, hitResult, entity))
                continue;
            try {
                Vec3 hitLocation = hitResult.getLocation();
                return new RotationUtil.BestHitInfo(eyePos, hitLocation, hitLocation.distanceTo(eyePos),
                        RotationUtil.getSensitivitySnappedRotation(rotation.getYaw(), rotation.getPitch(),
                                RotationHandler.prevRotation.yaw, RotationHandler.prevRotation.pitch));
    ...
    public static Rotation getEntityRotation(Entity entity, float spreadFactor, float verticalSpread,
        ...
        aimY = heightFraction <= 0.1f ? (aimY += entity.getBbHeight() * 0.1f)
                : (heightFraction >= 0.9f ? (aimY += entity.getEyeHeight() * (0.8f + random.nextFloat() * 0.2f))
                        : (aimY += entity.getBbHeight() * Mth.clamp(heightFraction, 0.1f, 0.9f)));
        ...
        return RotationUtil.getSensitivitySnappedRotation(finalYaw, finalPitch, RotationHandler.prevRotation.yaw,
                RotationHandler.prevRotation.pitch);
    }
    ...
    private static boolean isHitValid(Vec3 eyePos, HitResult hitResult, Entity entity) {
        if (hitResult.getType() == HitResult.Type.ENTITY && ((EntityHitResult) hitResult).getEntity() == entity) {
            ...
            return RotationUtil.isInsideAABB(RotationUtil.getEntityBB(entity), eyePos)
                    || hitLocation.distanceTo(eyePos) <= 3.0;
        }
        return false;
    }
    ...
    private static HitResult performRaycast(Rotation rotation) {
        ...
        EntityHitResult entityHitResult = ProjectileUtil.getEntityHitResult(
                mc.player, eyePos, endPos, expandedBB = mc.player.getBoundingBox()
                        .expandTowards(direction.scale(pickRange)).inflate(1.0, 1.0, 1.0),
                entity -> !entity.isSpectator() && entity.isPickable(), maxRangeSqr);
        if (entityHitResult != null) {
            ...
            if (checkClampedRange && hitDistSqr > 9.0) {
                hitResult = BlockHitResult.miss(hitLocation,
                        Direction.getNearest(direction.x, direction.y, direction.z), BlockPos.containing(hitLocation));
    ...
    public static Rotation rotationFromCoords(double x, double y, double z) {
        ...
        return RotationUtil.rotationFromPoints(x, y, z, mc.player.getX(),
                mc.player.getY() + (double) mc.player.getEyeHeight(mc.player.getPose()), mc.player.getZ());
    }
    ...
    public static Rotation rotationFromPoints(double targetX, double targetY, double targetZ, double fromX,
            double fromY, double fromZ) {
    ...
    public static Rotation entityRotation(Entity entity) {
        ...
        double dy = entity.getY() + (double) entity.getEyeHeight()
                - (mc.player.getY() + (double) mc.player.getEyeHeight());
