...
public final class RayTraceUtil
    ...
    public static boolean canRayTrace(Rotation rotation, Direction direction, BlockPos blockPos, boolean checkFace) {
        ...
        BlockHitResult blockHitResult = mc.level.clip(new ClipContext(eyePos,
                endPos = eyePos.add((lookDir = Vec3.directionFromRotation(pitch, yaw)).scale(5.0)),
    ...
    public static HitResult clipWithEntity(Vec3 fromPos, Vec3 toPos, boolean clipFluids, boolean useCollider,
            boolean useVisual, Entity entity) {
        ClipContext.Block blockMode = useCollider ? ClipContext.Block.COLLIDER
                : (useVisual ? ClipContext.Block.VISUAL : ClipContext.Block.OUTLINE);
    ...
    public static HitResult rayTraceForEntity(Rotation rotation, double range, float inflate, Entity entity,
            Entity onlyTarget, boolean ignoreBlocks) {
        if (entity == null || mc.level == null) {
            return null;
        }
        ...
        if (!ignoreBlocks) {
            blockHitResult = mc.level
                    .clip(new ClipContext(eyePos, endPos, ClipContext.Block.OUTLINE, ClipContext.Fluid.NONE, entity));
            blockDist = blockHitResult.getType() == HitResult.Type.BLOCK
                    ? eyePos.distanceTo(blockHitResult.getLocation())
                    : range;
        }
        double searchRange = Math.min(range, blockDist) + (double) inflate;
        AABB searchBox = new AABB(eyePos.x - searchRange, eyePos.y - searchRange, eyePos.z - searchRange,
                eyePos.x + searchRange, eyePos.y + searchRange, eyePos.z + searchRange);
        List<Entity> entities = mc.level.getEntitiesOfClass(Entity.class, searchBox,
                candidate -> candidate != entity && (onlyTarget == null || candidate == onlyTarget)
                        && EntitySelector.NO_SPECTATORS.test(candidate) && candidate.isPickable());
        ...
        for (Entity candidate : entities) {
            ...
            if (!optional.isPresent()
                    || !((candidateDistSqr = eyePos.distanceToSqr(entityHitVec = optional.get())) < bestDistSqr))
                continue;
            boolean visible = ignoreBlocks
                    || (occlusionResult = mc.level.clip(new ClipContext(eyePos, centerVec = inflatedBox.getCenter(),
                            ClipContext.Block.OUTLINE, ClipContext.Fluid.NONE, entity)))
                            .getType() != HitResult.Type.BLOCK
                    || !(eyePos.distanceToSqr(occlusionResult.getLocation()) <= candidateDistSqr);
            if (!visible)
                continue;
