...
public class MotionSimulator {
    ...
    public MotionSimulator(double x, double y, double z, double motionX, double motionY, double motionZ, float yaw,
    ...
    public MotionSimulator(Player player) {
        this(player.getX(), player.getY(), player.getZ(), player.getDeltaMovement().x, player.getDeltaMovement().y,
                player.getDeltaMovement().z, player.getYRot(), player.xxa, player.zza);
        float jumpPower;
        ...
        this.jumpPower = jumpPower = 0.42f * ((double) currentJumpFactor == 1.0 ? belowJumpFactor : currentJumpFactor)
    ...
    private BlockPos rayTraceBlock(Vec3 fromPos, Vec3 toPos) {
        ...
        if (hitResult != null && hitResult.getType() == HitResult.Type.BLOCK && hitResult instanceof BlockHitResult
                && (blockHitResult = (BlockHitResult) hitResult).getDirection() == Direction.UP) {
