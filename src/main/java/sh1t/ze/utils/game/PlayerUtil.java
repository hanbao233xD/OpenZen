...
public final class PlayerUtil
    ...
    public static void sendCarriedItem() {
        try {
            Method method = mc.gameMode.getClass().getDeclaredMethod(
                    ReflectionUtil.getMappedMethodName(mc.gameMode.getClass(), "ensureHasSentCarriedItem", "()V"));
    ...
    public static boolean isSafeToScaffold() {
        ...
        if (belowState.isAir()) {
            ...
            if (mc.level.getBlockState(belowPos.below()).isAir()) {
                return belowPos.below().getY() >= mc.level.getMinBuildHeight()
                        && !mc.level.getBlockState(belowPos.below().below()).isAir();
            }
        }
        return true;
    }
