...
public final class ItemUtil
    ...
    public static boolean hasServerItem() {
        return ItemUtil.getAllItems().stream().anyMatch(itemStack -> {
            if (!itemStack.isEmpty()) {
                ...
                return displayName.contains("长按点击") || displayName.contains("点击使用") || displayName.contains("离开游戏")
                        || displayName.contains("选择一个队伍") || displayName.contains("再来一局");
            }
            return false;
        });
    }
    ...
    public static int getSlot(ItemStack itemStack) {
        ...
        for (int i = 0; i < mc.player.getInventory().items.size(); ++i) {
            if (mc.player.getInventory().items.get(i) != itemStack)
                continue;
            return i;
        }
        return -1;
    ...
    public static boolean isUsableItem(ItemStack itemStack) {
        ...
        if (item == Items.WHEAT_SEEDS || item == Items.BEETROOT_SEEDS || item == Items.MELON_SEEDS
    ...
    public static float getBestArmorScore(EquipmentSlot equipmentSlot) {
        return ItemUtil.getAllItems().stream()
                .filter(stack -> !stack.isEmpty() && stack.getItem() instanceof ArmorItem armor
    ...
    public static ItemStack getBestSword() {
        return ItemUtil.getAllItems().stream()
                .filter(itemStack -> !itemStack.isEmpty() && itemStack.getItem() instanceof SwordItem)
                .max(Comparator.comparingInt(itemStack -> (int) (ItemUtil.getSwordDamage(itemStack) * 100.0f)))
                .orElse(null);
    }
    ...
    public static float getBowScore(ItemStack itemStack) {
        ...
        if (itemStack.getItem() instanceof BowItem) {
            ...
            return (score += (float) EnchantmentHelper.getItemEnchantmentLevel(Enchantments.POWER_ARROWS, itemStack)
                    / 10.0f) + (float) itemStack.getDamageValue() / (float) itemStack.getMaxDamage();
        }
        return 0.0f;
    }
    ...
    public static float getBowScoreAlt(ItemStack itemStack) {
        ...
        if (itemStack.getItem() instanceof BowItem) {
            ...
            score += (float) EnchantmentHelper.getItemEnchantmentLevel(Enchantments.FLAMING_ARROWS, itemStack);
            return (score += (float) EnchantmentHelper.getItemEnchantmentLevel(Enchantments.POWER_ARROWS, itemStack))
                    + (float) itemStack.getDamageValue() / (float) itemStack.getMaxDamage();
        }
        return 0.0f;
    }
    ...
    public static boolean isWeaponItem(ItemStack itemStack) {
        ...
        if (itemStack.getItem() instanceof AxeItem && itemStack.getItem() == Items.GOLDEN_AXE
                && EnchantmentHelper.getItemEnchantmentLevel(Enchantments.SHARPNESS, itemStack) > 100) {
            return true;
        }
        if (itemStack.getItem() == Items.SLIME_BALL
                && EnchantmentHelper.getItemEnchantmentLevel(Enchantments.KNOCKBACK, itemStack) > 1) {
            return true;
        }
    ...
    public static double getAttackDamage(ItemStack itemStack) {
        ...
        for (Attribute attribute : modifiers.keySet()) {
            if (!attribute.getDescriptionId().equals("attribute.name.generic.attack_damage"))
                continue;
    ...
    public static int countFishingRods() {
        return ItemUtil
                .getAllItems().stream().filter(itemStack -> !itemStack.isEmpty()
                        && itemStack.getItem() == Items.FISHING_ROD && ItemUtil.isUsable(itemStack))
                .mapToInt(ItemStack::getCount).sum();
    }

    public static int countFood() {
        return ItemUtil.getAllItems().stream()
                .filter(itemStack -> !itemStack.isEmpty() && itemStack.getItem().isEdible()
                        && itemStack.getItem() != Items.GOLDEN_APPLE
                        && itemStack.getItem() != Items.ENCHANTED_GOLDEN_APPLE && ItemUtil.isUsable(itemStack))
                .mapToInt(ItemStack::getCount).sum();
    }
    ...
    public static ItemStack getBestProjectile() {
        return ItemUtil.getAllItems().stream()
                .filter(itemStack -> !itemStack.isEmpty()
                        && (itemStack.getItem() == Items.EGG || itemStack.getItem() == Items.SNOWBALL)
    ...
    public static ItemStack getFishingRodStack() {
        return ItemUtil
                .getAllItems().stream().filter(itemStack -> !itemStack.isEmpty()
                        && itemStack.getItem() instanceof FishingRodItem && ItemUtil.isUsable(itemStack))
                .findAny().orElse(null);
    }

    public static int countBlocks() {
        return ItemUtil.getAllItems().stream()
                .filter(itemStack -> !itemStack.isEmpty() && itemStack.getItem() instanceof BlockItem
                        && BlockUtil.isPlaceable(itemStack) && ItemUtil.isUsable(itemStack))
                .mapToInt(ItemStack::getCount).sum();
    }
    ...
    public static ItemStack getArrowStack() {
        return ItemUtil
                .getAllItems().stream().filter(itemStack -> !itemStack.isEmpty()
                        && itemStack.getItem() instanceof ArrowItem && ItemUtil.isUsable(itemStack))
                .min(Comparator.comparingInt(ItemStack::getCount)).orElse(null);
    }
    ...
    public static float getBestPickaxeScore() {
        return ItemUtil
                .getAllItems().stream().filter(itemStack -> !itemStack.isEmpty()
                        && itemStack.getItem() instanceof PickaxeItem && ItemUtil.isUsable(itemStack))
                .map(ItemUtil::getDigSpeed).max(Float::compareTo).orElse(0.0f);
    }

    public static ItemStack getBestPickaxe() {
        return ItemUtil.getAllItems().stream()
                .filter(itemStack -> !itemStack.isEmpty() && itemStack.getItem() instanceof PickaxeItem
                        && ItemUtil.isUsable(itemStack))
                .max(Comparator.comparingInt(itemStack -> (int) (ItemUtil.getDigSpeed(itemStack) * 100.0f)))
                .orElse(null);
    }

    public static float getBestAxeScore() {
        return ItemUtil.getAllItems().stream()
                .filter(itemStack -> !itemStack.isEmpty() && itemStack.getItem() instanceof AxeItem
                        && !ItemUtil.isLegitAxe(itemStack) && ItemUtil.isUsable(itemStack))
                .map(ItemUtil::getDigSpeed).max(Float::compareTo).orElse(0.0f);
    }
    ...
    public static ItemStack getBestSharpAxe() {
        return ItemUtil.getAllItems().stream()
                .filter(itemStack -> !itemStack.isEmpty() && itemStack.getItem() instanceof AxeItem
                        && ItemUtil.isLegitAxe(itemStack) && ItemUtil.isUsable(itemStack)
                        && !ItemUtil.isOtherCheat(itemStack))
                .max(Comparator.comparingInt(itemStack -> (int) (ItemUtil.getAxeDamage(itemStack) * 100.0f)))
                .orElse(null);
    }
    ...
    public static float getBestShovelScore() {
        return ItemUtil
                .getAllItems().stream().filter(itemStack -> !itemStack.isEmpty()
                        && itemStack.getItem() instanceof ShovelItem && ItemUtil.isUsable(itemStack))
                .map(ItemUtil::getDigSpeed).max(Float::compareTo).orElse(0.0f);
    }

    public static ItemStack getBestShovel() {
        return ItemUtil.getAllItems().stream()
                .filter(itemStack -> !itemStack.isEmpty() && itemStack.getItem() instanceof ShovelItem
                        && ItemUtil.isUsable(itemStack))
                .max(Comparator.comparingInt(itemStack -> (int) (ItemUtil.getDigSpeed(itemStack) * 100.0f)))
                .orElse(null);
    }
    ...
    public static float getBestCrossbowScore() {
        return ItemUtil.getAllItems().stream()
                .filter(itemStack -> !itemStack.isEmpty() && itemStack.getItem() instanceof CrossbowItem
                        && ItemUtil.isUsable(itemStack))
                .map(ItemUtil::getCrossbowScore).max(Float::compareTo).orElse(0.0f);
    }
    ...
    public static ItemStack getBestCrossbow() {
        return ItemUtil.getAllItems().stream()
                .filter(itemStack -> !itemStack.isEmpty() && itemStack.getItem() instanceof CrossbowItem
                        && ItemUtil.isUsable(itemStack))
                .max(Comparator.comparingInt(itemStack -> (int) (ItemUtil.getCrossbowScore(itemStack) * 100.0f)))
                .orElse(null);
    }
    ...
    public static float getBestBowScore() {
        return ItemUtil
                .getAllItems().stream().filter(itemStack -> !itemStack.isEmpty()
                        && itemStack.getItem() instanceof BowItem && ItemUtil.isUsable(itemStack))
                .map(ItemUtil::getBowScore).max(Float::compareTo).orElse(0.0f);
    }

    public static ItemStack getBestBow() {
        return ItemUtil.getAllItems().stream()
                .filter(itemStack -> !itemStack.isEmpty() && itemStack.getItem() instanceof BowItem
                        && ItemUtil.isUsable(itemStack))
                .max(Comparator.comparingInt(itemStack -> (int) (ItemUtil.getBowScore(itemStack) * 100.0f)))
                .orElse(null);
    }
    ...
    public static float getBestBowScoreAlt() {
        return ItemUtil.getAllItems().stream()
                .filter(itemStack -> !itemStack.isEmpty() && itemStack.getItem() instanceof BowItem
                        && ItemUtil.isUsable(itemStack))
                .map(ItemUtil::getBowScoreAlt).max(Float::compareTo).orElse(0.0f);
    }
    ...
    public static ItemStack getBestBowAlt() {
        return ItemUtil.getAllItems().stream()
                .filter(itemStack -> !itemStack.isEmpty() && itemStack.getItem() instanceof BowItem
                        && ItemUtil.isUsable(itemStack))
                .max(Comparator.comparingInt(itemStack -> (int) (ItemUtil.getBowScoreAlt(itemStack) * 100.0f)))
                .orElse(null);
    }
    ...
    public static boolean hasItem(Item item) {
        return ItemUtil.getAllItems().stream()
                .anyMatch(itemStack -> !itemStack.isEmpty() && itemStack.getItem() == item);
    }

    public static int countItem(Item item) {
        return ItemUtil.getAllItems().stream().filter(itemStack -> !itemStack.isEmpty() && itemStack.getItem() == item)
                .mapToInt(ItemStack::getCount).sum();
    }
