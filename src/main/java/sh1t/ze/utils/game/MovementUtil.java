...
public final class MovementUtil
    ...
    public static void strafeWithYaw(double yawRad, double speed) {
        ...
        mc.player.setDeltaMovement(deltaMovement.x + (double) (-Mth.sin((float) yawRad)) * speed, deltaMovement.y,
                deltaMovement.z + (double) Mth.cos((float) yawRad) * speed);
    }
    ...
    public static void strafeForward(double speed) {
        ...
        Vec3 deltaMovement = mc.player.getDeltaMovement();
        mc.player.setDeltaMovement(deltaMovement.x + (double) (-Mth.sin((float) movementYaw)) * speed, deltaMovement.y,
                deltaMovement.z + (double) Mth.cos((float) movementYaw) * speed);
    }
    ...
    public static void setSpeed(double speed) {
        ...
        double motionX = (double) forward * speed * -Math.sin(Math.toRadians(yaw))
                + (double) strafe * speed * Math.cos(Math.toRadians(yaw));
        double motionZ = (double) forward * speed * Math.cos(Math.toRadians(yaw))
                - (double) strafe * speed * -Math.sin(Math.toRadians(yaw));
    ...
    public static boolean isAboveVoid(double x, double y, double z) {
        while (y > 0.0) {
            ...
            BlockHitResult blockHitResult = mc.level.clip(
                    new ClipContext(startPos, endPos, ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, mc.player));
    ...
    public static void handleStrafe(StrafeEvent strafeEvent, float partialTicks) {
        ...
        float yaw = mc.player.getYRot();
        if (TargetStrafe.strafeTarget != null && TargetStrafe.INSTANCE.isEnabled()
                && (!TargetStrafe.isSmartStrafe() || mc.options.keyJump.isDown())) {
            float yawOffset = (float) (MovementUtil.getBaseSpeed() / ((double) TargetStrafe.getRange() * Math.PI * 2.0)
                    * 360.0) * (float) TargetStrafe.strafeDirectionSign;
            Rotation rotation = RotationUtil.rotationToForBow(
                    new Vec3(TargetStrafe.strafeTarget.getX(), TargetStrafe.strafeTarget.getY(),
                            TargetStrafe.strafeTarget.getZ()),
            ...
            yaw = (float) Math.toDegrees(RotationUtil
                    .rotationToForBow(new Vec3(mc.player.getX(), mc.player.getY(), mc.player.getZ()),
                            new Vec3(circleX, TargetStrafe.strafeTarget.getY(), circleZ))
        ...
        for (int i = -1; i <= 1; ++i) {
            for (int j = -1; j <= 1; ++j) {
                double candidateDir;
                double diff;
                if (j == 0 && i == 0 || !((diff = Math.abs(targetDirection - (candidateDir = Mth.wrapDegrees(
                        Math.toDegrees(MovementUtil.getDirectionYaw(partialTicks, i, j)))))) < (double) bestDiff))
                    continue;
                bestDiff = (float) diff;
                bestForward = i;
                bestStrafe = j;
