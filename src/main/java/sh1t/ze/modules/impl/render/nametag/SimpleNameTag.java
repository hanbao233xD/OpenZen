    ...
    private void updatePositions(float partialTick) {
        ...
        for (Entity entity : mc.level.entitiesForRendering()) {
            ...
            if (entity.getName().getString().startsWith("CIT-"))
                continue;
            if (entity.isInvisible() && !NameTags.INSTANCE.showHealthSetting.getValue())
                continue;
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        Renderer.renderConsumer(ctx -> {
            for (Map.Entry<Entity, Vector2f> entry : this.entityPositions.entrySet()) {
                ...
                int alertCount = NameTags.INSTANCE.showArmorSetting.getValue() && !alertItems.isEmpty()
                        ? alertItems.size()
                        : 0;
                ...
                ctx.drawRoundedRect(RoundedRectangle.ofXYWHR(originX, originY, boxWidth, boxHeight, corner),
                        this.paint);
                if (ratio > 0.0f) {
                    this.paint.setColor(new Color(0, 0, 0, 80).getRGB());
                    ctx.drawRoundedRect(RoundedRectangle.ofXYWHR(originX, originY, boxWidth * ratio, boxHeight, corner),
                            this.paint);
                }
                ...
                if (hasAlerts) {
                    ...
                    for (ItemStack item : alertItems) {
                        ...
                        ctx.drawRoundedRect(
                                RoundedRectangle.ofXYWHR(itemCursorX, originY, itemBoxSize, boxHeight, corner),
                                this.paint);
        ...
        for (BlurRect rect : this.blurRects) {
            RenderUtil.drawBlurredRect(event.guiGraphics().pose(), rect.x, rect.y, rect.width, rect.height, 8.0f, 2.0f,
                    0.6f, 0);
        }
