...
public class OldHitting
        extends Module {
    private final ModeSetting animationModeSetting = new ModeSetting("Animation", "Vanilla", "Leaked", "Slide")
            .withDefault("Leaked");
    ...
    public void applyHitAnimation(PoseStack poseStack, float progress, HumanoidArm humanoidArm, float equipProgress) {
        ...
        if (this.animationModeSetting.getValue().equalsIgnoreCase("leaked")) {
            ...
            poseStack.scale((float) (0.8 + (double) pulse) * size, (float) (0.8 + (double) pulse) * size,
                    (float) (0.8 + (double) pulse) * size);
            OldHitting.applyRotate(-Mth.sin((float) ((double) Mth.sqrt(scaledProgress) * Math.PI)) * 20.0f, 0.0f, 1.2f,
                    -0.8f, poseStack);
            OldHitting.applyRotate(-Mth.sin((float) ((double) Mth.sqrt(scaledProgress) * Math.PI)) * 30.0f, 1.0f, 0.0f,
                    0.0f, poseStack);
