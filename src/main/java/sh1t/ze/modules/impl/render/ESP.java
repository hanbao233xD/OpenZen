...
public class ESP extends Module {
    ...
    private final ModeSetting healthBarPositionSetting = new ModeSetting("Health Bar Position", "Bottom", "Top", "Left",
            "Right").withDefault("Bottom");
    ...
    public boolean isGlowing(Entity entity) {
        if (this.isEnabled() && "Glow".equalsIgnoreCase(this.modeSetting.getValue())) {
            if (entity instanceof Player && this.playersSetting.getValue())
                return true;
            if (entity instanceof Animal && this.animalsSetting.getValue())
                return true;
            if (entity instanceof Mob && this.mobsSetting.getValue())
                return true;
            if (entity instanceof ItemEntity && this.itemsSetting.getValue())
                return true;
    ...
    public void onRender(RenderEvent renderEvent) {
        ...
        for (Entity entity : mc.level.entitiesForRendering()) {
            if (!this.shouldShowEntity(entity) || !this.isInRange(entity))
                continue;
        ...
        for (Entity entity : this.visibleEntities) {
            ...
            if (!ok || this.projectedPoints.isEmpty())
                continue;
            ...
            int pad = 3;
            Vector4d box = new Vector4d((int) (minX - pad), (int) (minY - pad), (int) (maxX - minX + pad * 2),
                    (int) (maxY - minY + pad * 2));
    ...
    public void onRender2D(Render2DEvent event) {
        if (!"Outlined 2D".equals(this.modeSetting.getValue()) || this.entityBoxPositions.isEmpty())
            return;
    ...
    private void drawFilledRect2D(BufferBuilder builder, Matrix4f matrix4f, float x1, float y1, float x2, float y2,
            Color color) {
    ...
    private void drawHealthBar(BufferBuilder builder, Matrix4f matrix4f, LivingEntity entity, Vector4d v, Color color) {
        ...
        RenderUtil.drawQuad(builder, matrix4f, barX - 0.6f, barY - 0.6f, barX + barW + 0.6f, barY + barH + 0.6f,
                Color.BLACK);
        ...
        if (position.equals("Left") || position.equals("Right")) {
            RenderUtil.drawQuad(builder, matrix4f, barX, barY + barH * (1.0f - healthFrac), barX + barW, barY + barH,
                    healthColor);
        } else {
