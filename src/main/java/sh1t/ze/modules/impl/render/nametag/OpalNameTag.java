...
public class OpalNameTag extends NameTagStyle {
    ...
    private void updatePositions(RenderEvent event) {
        ...
        for (Entity entity : mc.level.entitiesForRendering()) {
            ...
            if (!entity.isAlive())
                continue;
            if (entity.isInvisible() && !this.showHealthSetting.getValue())
                continue;
            ...
            if (entity.getName().getString().startsWith("CIT-"))
                continue;
            if (NameTags.INSTANCE.showPingSetting.getValue() && Teams.isSameTeam(entity))
                continue;
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        Renderer.renderConsumer(ctx -> {
            ...
            for (Map.Entry<Entity, Vector2f> entry : this.entityPositions.entrySet()) {
                ...
                ctx.drawRoundedRect(RoundedRectangle.ofXYWHR(cursorX, originY, distBoxW, rowHeight, corner),
                        this.paint);
                ...
                ctx.drawString(distanceText, cursorX + padding + distIconW + 2.0f, textBaseline - 1.0f, this.nameFont,
                        this.paint);
                cursorX += distBoxW + gap;
                this.paint.setColor(PADDING);
                ctx.drawRoundedRect(RoundedRectangle.ofXYWHR(cursorX, originY, nameBoxW + 2.0f, rowHeight, corner),
                        this.paint);
                this.paint.setColor(COLOR_WHITE);
                ctx.drawString(nameIcon, cursorX + padding, textBaseline, this.iconFont, this.paint);
                ctx.drawString(displayName, cursorX + padding + nameIconW + 6.0f, textBaseline - 3.0f, this.mainFont,
                        this.paint);
                ...
                this.paint.setColor(PADDING);
                ctx.drawRoundedRect(RoundedRectangle.ofXYWHR(cursorX, originY, healthBoxW, rowHeight, corner),
                        this.paint);
                this.paint.setColor(COLOR_RED);
                ctx.drawString(healthIcon, cursorX + padding, textBaseline + 1.0f, this.iconFont, this.paint);
                this.paint.setColor(COLOR_WHITE);
                ctx.drawString(healthText, cursorX + padding + healthIconW + 2.5f, textBaseline - 1.0f, this.nameFont,
                        this.paint);
                cursorX += healthBoxW;
                if (!absorbText.isEmpty()) {
                    cursorX += gap;
                    this.paint.setColor(PADDING);
                    ctx.drawRoundedRect(RoundedRectangle.ofXYWHR(cursorX, originY, absorbBoxW, rowHeight, corner),
                            this.paint);
                    this.paint.setColor(COLOR_GOLD);
                    ctx.drawString(healthIcon, cursorX + padding, textBaseline + 1.0f, this.iconFont, this.paint);
                    this.paint.setColor(COLOR_WHITE);
                    ctx.drawString(absorbText, cursorX + padding + absorbIconW + 2.5f, textBaseline, this.nameFont,
                            this.paint);
                    cursorX += absorbBoxW;
                }
                if (hasAlerts) {
                    for (ItemStack item : alertItems) {
                        if (ItemAlertTracker.hasItem(player.getUUID(), item.getItem()))
                            continue;
                        cursorX += gap;
                        this.paint.setColor(PADDING);
                        ctx.drawRoundedRect(RoundedRectangle.ofXYWHR(cursorX, originY, itemBoxH, rowHeight, corner),
                                this.paint);
    ...
    public void onPacket(PacketEvent packetEvent) {
        if (!(packetEvent.getPacket() instanceof ClientboundSetScorePacket packet))
            return;
        if (mc.level == null || mc.player == null)
            return;
