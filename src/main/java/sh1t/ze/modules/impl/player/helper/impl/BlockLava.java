...
public class BlockLava
            ...
            switch (this.currentPlacement) {
                ...
                case LAVA_SUPPORT: {
                    ...
                    this.findSuitableFace(blockPos).ifPresentOrElse(placementData -> {
                        this.targetRotation = RotationUtil.exactRotation(mc.player.getEyePosition(),
                                placementData.hitVec());
    ...
    private void placeBlock() {
        ...
        BlockHitResult blockHitResult = new BlockHitResult(this.currentState.hitVec(), this.currentState.direction(),
                this.currentState.blockPos(), false);
    ...
    public void onRender(RenderEvent renderEvent) {
        ...
        RenderSystem.setShaderColor((float) color.getRed() / 255.0f, (float) color.getGreen() / 255.0f,
                (float) color.getBlue() / 255.0f, 0.25f);
        RenderUtil.drawSolidBox(aABB, poseStack);
        RenderSystem.setShaderColor((float) color.getRed() / 255.0f, (float) color.getGreen() / 255.0f,
                (float) color.getBlue() / 255.0f, 0.75f);
    ...
    private void findTargetPos() {
        ...
        for (int dx = -3; dx <= 3; ++dx) {
            for (int dy = -2; dy <= 2; ++dy) {
                for (int dz = -3; dz <= 3; ++dz) {
                    ...
                    if (Helper.hasLavaPlacement(candidatePos) || !mc.level.getBlockState(candidatePos).is(Blocks.LAVA)
                            || !mc.level.getFluidState(candidatePos).isSource()
                            || !Helper.isPositionInFov(Vec3.atCenterOf(candidatePos)))
                        continue;
        ...
        this.targetPos = (BlockPos) candidates.stream()
                .min(Comparator.comparingDouble(blockPos -> Vec3.atCenterOf((Vec3i) blockPos).distanceToSqr(eyePos)))
                .orElse(null);
    }
    ...
    private int findBlockSlot() {
        ...
        for (int slot = 0; slot < 9; ++slot) {
            ...
            if (!(itemStack.getItem() instanceof BlockItem)
                    || !(block = ((BlockItem) itemStack.getItem()).getBlock()).defaultBlockState().isSolid()
                    || block.getSoundType(block.defaultBlockState()).equals(SoundEvents.WOOD_PLACE))
                continue;
            return slot;
        }
        return -1;
    ...
    private Optional<BlockLava.PlacementData> findSuitableFace(BlockPos blockPos) {
        for (Direction direction : Direction.values()) {
            ...
            if (mc.level.getBlockState(neighborPos).isSolid() && !mc.level.getBlockState(neighborPos).is(Blocks.LAVA)) {
                Vec3 hitVec = Vec3.atCenterOf(neighborPos).add(direction.getOpposite().getStepX() * 0.5,
                        direction.getOpposite().getStepY() * 0.5, direction.getOpposite().getStepZ() * 0.5);
                if (mc.player.getEyePosition().distanceToSqr(hitVec) <= 25.0
                        && this.canSeeBlockFace(neighborPos, direction.getOpposite())) {
            ...
            if (!mc.level.getBlockState(belowPos).isSolid() || mc.level.getBlockState(belowPos).is(Blocks.LAVA))
                continue;
            Vec3 topHitVec = Vec3.atCenterOf(belowPos).add(new Vec3((double) Direction.UP.getStepX() * 0.5,
                    (double) Direction.UP.getStepY() * 0.5, (double) Direction.UP.getStepZ() * 0.5));
            if (!(mc.player.getEyePosition().distanceToSqr(topHitVec) <= 25.0)
                    || !this.canSeeBlockFace(belowPos, Direction.UP))
                continue;
    ...
    private boolean hasLineOfSight(Vec3 targetVec) {
        ...
        Vec3 eyePos = mc.player.getEyePosition();
        return mc.level
                .clip(new ClipContext(eyePos, targetVec, ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, mc.player))
    ...
    private boolean canSeeBlockFace(BlockPos blockPos, Direction direction) {
        ...
        Vec3 eyePos = mc.player.getEyePosition();
        BlockHitResult hit = mc.level.clip(new ClipContext(eyePos,
                targetVec = Vec3.atCenterOf(blockPos)
                        .add(new Vec3((double) direction.getStepX() * 0.49, (double) direction.getStepY() * 0.49,
                                (double) direction.getStepZ() * 0.49)),
