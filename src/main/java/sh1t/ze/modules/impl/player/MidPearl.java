...
public class MidPearl extends Module {
    ...
    private final ModeSetting mainButtonSetting = new ModeSetting("Button", "Middle", "Mouse 4", "Mouse 5")
            .withDefault("Middle");
    private final BooleanSetting interceptSetting = new BooleanSetting("Intercept", false);
    private final ModeSetting interceptButtonSetting = new ModeSetting("Intercept Button", "Mouse 4", "Mouse 5")
            .withDefault("Mouse 4");
    ...
    public void onTick(TickEvent event) {
        ...
        if (this.interceptSetting.getValue()) {
            ...
            boolean sameButton = this.getMouseButton(this.interceptButtonSetting) == this
                    .getMouseButton(this.mainButtonSetting);
            if (sameButton && (pressed || this.isHoldingIntercept))
                return;
        }
        if (this.isAutoRodActive())
            return;
        this.tickNormalMode();
    }
    ...
    private Optional<Projectiles.ProjectileEntry> findNearestEnemyPearl() {
        if (Projectiles.projectileMap.isEmpty())
            return Optional.empty();
