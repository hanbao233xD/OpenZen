...
public class AntiVoid
    ...
    public void onMotion(MotionEvent motionEvent) {
        ...
        if (mc.player.onGround()) {
            if (this.awaitingScaffoldDisable && this.groundTicks > 6
                    && new MotionSimulator(mc.player).findLandingBlock(60) != null) {
        ...
        if (!this.bufferingPackets && !this.sentFlyPacket && !mc.player.onGround()
                && mc.player.getDeltaMovement().y < 0.0) {
            boolean shouldBuffer = mc.player.getDeltaMovement().y < 0.1 && this.isVoidBelow()
                    && !PlayerUtil.isSafe(mc.player.getY() + (double) mc.player.getEyeHeight());
            if (shouldBuffer) {
        ...
        if (!this.scaffoldWasActive && this.bufferingPackets
                && mc.player.fallDistance > this.fallDistanceSetting.getValue().floatValue()) {
    ...
    public void onReceivePacket(ReceivePacketEvent receivePacketEvent) {
        ...
        if (receivePacketEvent.getPacket() instanceof ClientboundPlayerPositionPacket
                && (this.sentFlyPacket || this.bufferingPackets)) {
            this.reset();
    ...
    private boolean isVoidBelow() {
        ...
        for (int dy = 0; dy < scanDepth; ++dy) {
            ...
            if (mc.level.getBlockState(cursor).getBlock() == Blocks.AIR)
                continue;
            return false;
        }
        return true;
