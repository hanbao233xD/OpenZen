...
public class ChestStealer
            ...
            if (screen != this.lastScreen) {
                ...
                if (this.chestSetting.getValue()
                    ...
                    if (this.shouldCloseChest(chestMenu)) {
                        ...
                        && this.shouldCloseChest(chestMenu)) {
                    this.stealFromChest(chestMenu);
                }
            }
        } else {
    ...
    private void buildStealQueue(ChestMenu chestMenu) {
        ...
        for (int slot = 0; slot < chestMenu.getRowCount() * 9; ++slot) {
            ...
            if (itemStack.isEmpty() || !this.shouldStealItem(itemStack))
                continue;
        ...
        this.stealTargetQueue.clear();
        List<String> categories = Arrays.asList("god", "helmet", "chestplate", "leggings", "boots", "sword", "bow",
                "crossbow", "golden_apple", "pickaxe", "axe", "shovel", "special", "utility", "other");
        for (String category : categories) {
            if (!categoryMap.containsKey(category))
                continue;
            List<ChestStealer.StealTarget> categoryItems = categoryMap.get(category);
            boolean isBestOnlyCategory = category.equals("god") || category.equals("helmet")
                    || category.equals("chestplate") || category.equals("leggings") || category.equals("boots")
                    || category.equals("sword") || category.equals("bow") || category.equals("crossbow")
                    || category.equals("pickaxe") || category.equals("axe") || category.equals("shovel");
            if (this.onlyBestSetting.getValue() && isBestOnlyCategory) {
                ChestStealer.StealTarget best = categoryItems.stream().max(Comparator.comparingDouble(t -> t.score))
                        .orElse(null);
                if (best == null)
                    continue;
    ...
    private String getItemCategory(ItemStack itemStack) {
        ...
        if (item == Items.ENDER_PEARL || item == Items.SNOWBALL || item == Items.EGG || item == Items.ARROW
                || item instanceof FishingRodItem || item instanceof BlockItem) {
            return "utility";
        }
        return "other";
    }
    ...
    private void stealRandomMode(ChestMenu chestMenu) {
        ...
        if (this.randomClickSetting.getValue() && !stealableSlots.isEmpty() && this.accessCount > 1) {
            ...
            for (int slot = 0; slot < chestMenu.getRowCount() * 9; ++slot) {
                ItemStack itemStack = chestMenu.getSlot(slot).getItem();
                if (itemStack.isEmpty() || this.accessCount <= 1 || !this.tryStealSlot(chestMenu, slot))
                    continue;
                return;
            }
        }
        ...
        try {
            ...
            if (this.randomClickSetting.getValue() && !stealableSlots.isEmpty() && this.accessCount > 1) {
                ...
                for (int slot = 0; slot < container.getContainerSize(); ++slot) {
                    ...
                    if (itemStack.isEmpty() || this.accessCount <= 1 || !this.shouldStealItem(itemStack))
                        continue;
    ...
    private List<Integer> getStealableChestSlots(ChestMenu chestMenu) {
        ...
        for (int slot = 0; slot < chestMenu.getRowCount() * 9; ++slot) {
            ItemStack itemStack = chestMenu.getSlot(slot).getItem();
            if (itemStack.isEmpty() || !ChestStealer.isWorthStealing(itemStack) && !this.pickTrashSetting.getValue()
                    || !this.shouldStealItem(itemStack))
                continue;
    ...
    private void executePendingClick() {
        if (this.pendingMenu != null && this.pendingSlot >= 0) {
            ...
            mc.gameMode.handleInventoryMouseClick(this.pendingMenu.containerId, this.pendingSlot, 0,
    ...
    private boolean tryStealSlot(ChestMenu chestMenu, int slot) {
        ItemStack itemStack = chestMenu.getSlot(slot).getItem();
        if ((ChestStealer.isWorthStealing(itemStack) || this.pickTrashSetting.getValue())
                && this.shouldStealItem(itemStack)) {
    ...
    private boolean shouldStealItem(ItemStack itemStack) {
        ...
        if (this.onlyBestSetting.getValue()) {
            ...
            if (!(item instanceof SwordItem || item instanceof DiggerItem || item instanceof ArmorItem
    ...
    public static boolean isWorthStealing(ItemStack itemStack) {
        ...
        if (itemStack.getItem() == Items.WATER_BUCKET
                && ItemUtil.countItem(Items.WATER_BUCKET) >= InventoryManager.getMaxWaterBuckets()) {
            return false;
        }
        if (itemStack.getItem() == Items.LAVA_BUCKET
                && ItemUtil.countItem(Items.LAVA_BUCKET) >= InventoryManager.getMaxLavaBuckets()) {
            return false;
        }
        if (itemStack.getItem() instanceof BlockItem && BlockUtil.isPlaceable(itemStack)
                && ItemUtil.countBlocks() + itemStack.getCount() >= InventoryManager.getMaxBlockSize()) {
            return false;
        }
        if (itemStack.getItem() == Items.ARROW
                && ItemUtil.countItem(Items.ARROW) + itemStack.getCount() >= InventoryManager.getMaxArrows()) {
            return false;
        }
        if (itemStack.getItem() instanceof FishingRodItem && ItemUtil.countItem(Items.FISHING_ROD) >= 1) {
            return false;
        }
        if ((itemStack.getItem() == Items.SNOWBALL || itemStack.getItem() == Items.EGG)
                && ItemUtil.countItem(Items.SNOWBALL) + ItemUtil.countItem(Items.EGG)
                        + itemStack.getCount() >= InventoryManager.getMaxEggsSnowballsSize()) {
            return false;
        }
