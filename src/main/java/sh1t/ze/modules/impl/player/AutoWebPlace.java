...
public class AutoWebPlace extends Module {
    ...
    public void onRender(RenderEvent event) {
        if (!this.renderSetting.getValue() || (this.targetBlockPos == null && this.lastDebugSupportPos == null)
        ...
        if (this.targetBlockPos != null) {
            ...
            RenderSystem.setShaderColor(tint.getRed() / 255.0f, tint.getGreen() / 255.0f, tint.getBlue() / 255.0f,
                    0.25f);
        ...
        if (this.currentPlacement != null) {
            this.renderFaceHighlight(poseStack, this.currentPlacement.support, this.currentPlacement.face,
                    new Color(255, 230, 0));
    ...
    private Optional<PlacementTarget> findPlacementTarget() {
        ...
        for (Entity entity : candidates) {
            ...
            Optional<PlacementInfo> cobwebPlace = !inCobweb && cobwebSlot != -1 ? this.findLavaPlacement(entity)
                    : Optional.empty();
    ...
    private Optional<PlacementTarget> findWebExitPlacement(Entity entity, int cobwebSlot) {
        ...
        for (int tickAhead = 2; tickAhead <= 3; tickAhead++) {
            ...
            Optional<PlacementTarget> placement = this.tryPlacements(entity,
                    this.getPositionsForAabb(predicted, tickAhead), movement);
    ...
    private Optional<PlacementTarget> tryPlacements(Entity entity, List<PredictionTick> ticks, Vec3 movement) {
        for (PredictionTick tick : ticks) {
            ...
            Optional<GroundWebData> groundWeb = this.findGroundWebAt(entity, tick.footPos, tick.headPos,
                    "predicted web exit");
            if (groundWeb.isPresent()) {
                GroundWebData data = groundWeb.get();
    ...
    private List<PredictionTick> getPositionsForAabb(AABB aabb, int tickAhead) {
        ...
        list.sort(Comparator
                .comparingDouble(t -> Vec3.atCenterOf(t.footPos).distanceToSqr(centerX, footY + 0.5, centerZ)));
        return list;
    }
    ...
    private Optional<GroundWebData> findGroundWebAt(Entity entity, BlockPos footPos, BlockPos headPos, String label) {
        ...
        Optional<PlacementInfo> footPlacement = this.raycastPlacement(footPos.below(), footPos, entity.getX(),
                entity.getZ(), label + " foot");
    ...
    private Optional<PlacementInfo> findLavaSourcePlacement(Entity entity) {
        ...
        Optional<PlacementInfo> placement = this.findLavaGroundPlacement(entity.getX(), entity.getY(), entity.getZ(),
                "current");
        if (placement.isPresent()) {
            return placement;
        }
        this.debugLog("Lava fail: no lava placement foot=" + this.formatVec3(foot) + " motion="
                + this.formatVec3(entity.getDeltaMovement()));
        return Optional.empty();
    }
    ...
    private Optional<PlacementInfo> raycastPlacement(BlockPos supportPos, BlockPos placePos, double targetX,
            double targetZ, String label) {
        ...
        for (Vec3 candidate : this.getAimCandidates(supportPos, targetX, targetZ)) {
            if (mc.player.getEyePosition().distanceToSqr(candidate) > 25.0)
                continue;
            Vec3 offset = this.offsetTowardsFace(candidate, Direction.UP);
            BlockHitResult hit = mc.level.clip(new ClipContext(mc.player.getEyePosition(), offset,
        ...
        if (lastHit != null) {
            ...
            this.lastDebugFace = Direction.UP;
            this.lastDebugAimPoint = lastAim;
            this.debugLog("Placement fail: " + label + " ray miss support=" + this.formatBlockPos(supportPos) + " aim="
                    + this.formatVec3(lastAim));
        }
        return Optional.empty();
    }
    ...
    private Optional<PlacementInfo> aimAndCheckPlacement(Vec3 aim, BlockPos expectedSupport, String label) {
        BlockHitResult hit = mc.level.clip(new ClipContext(mc.player.getEyePosition(), aim, ClipContext.Block.OUTLINE,
    ...
    private Optional<Vec3> findFaceHitPoint(BlockPos pos, Direction face) {
        for (Vec3 point : this.getFacePoints(pos, face)) {
            if (mc.player.getEyePosition().distanceToSqr(point) > 25.0)
                continue;
    ...
    private Optional<Vec3> raycastFace(BlockPos pos, Direction face, Vec3 aim) {
        ...
        return hit.getBlockPos().equals(pos) && hit.getDirection() == face ? Optional.of(hit.getLocation())
                : Optional.empty();
    }
        ...
        for (double u : fractions) {
            for (double v : fractions) {
                Vec3 point = switch (face) {
                    case UP -> new Vec3(pos.getX() + lerp(faceBox.minX, faceBox.maxX, u), pos.getY() + faceBox.maxY,
                            pos.getZ() + lerp(faceBox.minZ, faceBox.maxZ, v));
                    case DOWN -> new Vec3(pos.getX() + lerp(faceBox.minX, faceBox.maxX, u), pos.getY() + faceBox.minY,
                            pos.getZ() + lerp(faceBox.minZ, faceBox.maxZ, v));
                    case EAST -> new Vec3(pos.getX() + faceBox.maxX, pos.getY() + lerp(faceBox.minY, faceBox.maxY, u),
                            pos.getZ() + lerp(faceBox.minZ, faceBox.maxZ, v));
                    case WEST -> new Vec3(pos.getX() + faceBox.minX, pos.getY() + lerp(faceBox.minY, faceBox.maxY, u),
                            pos.getZ() + lerp(faceBox.minZ, faceBox.maxZ, v));
                    case SOUTH -> new Vec3(pos.getX() + lerp(faceBox.minX, faceBox.maxX, u),
                            pos.getY() + lerp(faceBox.minY, faceBox.maxY, v), pos.getZ() + faceBox.maxZ);
                    case NORTH -> new Vec3(pos.getX() + lerp(faceBox.minX, faceBox.maxX, u),
                            pos.getY() + lerp(faceBox.minY, faceBox.maxY, v), pos.getZ() + faceBox.minZ);
                };
    ...
    private boolean isPlacementValid() {
        ...
        if (this.currentPlacement == null)
            return false;
        if (this.activePlacementType == PlacementType.LAVA) {
            if (!this.shouldPlaceLavaEx(this.target, this.isEntityInCobweb(this.target)))
                return false;
            if (this.intersectsPlayer(this.currentPlacement.place))
                return false;
            if (this.isWater(this.currentPlacement.place))
                return false;
            if (!mc.level.getBlockState(this.currentPlacement.place).canBeReplaced())
                return false;
            if (!mc.level.getFluidState(this.currentPlacement.place).isEmpty())
                return false;
    ...
    private boolean executePlacement() {
        ...
        if (this.activePlacementType == PlacementType.LAVA) {
            ...
            BlockHitResult hit = new BlockHitResult(this.currentPlacement.aim, this.currentPlacement.face,
                    this.currentPlacement.support, false);
    ...
    private boolean useBucket(Rotation rotation) {
        ...
        PacketUtil.sendQueued(
                new ServerboundMovePlayerPacket.Rot(rotation.getYaw(), rotation.getPitch(), mc.player.onGround()));
    ...
    private boolean tryLavaAfterBreak() {
        if (this.target == null || !this.lavaSetting.getValue() || this.findLavaBucketSlot() == -1)
            return false;
        if (!this.shouldPlaceLavaEx(this.target, true))
            return false;
        Optional<PlacementInfo> placement = this.findLavaSourcePlacement(this.target);
        if (placement.isEmpty()) {
    ...
    private void doLavaRetrieve() {
        ...
        if (actualSource != null) {
            ...
            if (!shouldAct) {
                this.debugLog("Lava retrieve wait target water bucket ticks=" + this.retrieveTicks + "/"
                        + this.getLavaTickDelay()
        ...
        if (!this.retriedLavaPlace && this.sourceWaitCount >= 2 && this.findLavaBucketSlot() != -1
    ...
    private boolean tryReapplyLava() {
        ...
        if (this.target.isOnFire())
            return false;
        if (this.lavaReapplyCount >= 1)
            return false;
        if (this.findLavaBucketSlot() == -1)
            return false;
        if (!this.shouldPlaceLavaEx(this.target, true))
            return false;
        Optional<PlacementInfo> placement = this.findLavaSourcePlacement(this.target);
    ...
    private String getRetrieveReason(boolean targetHasWaterBucket, boolean flowing, boolean timedOut) {
        if (flowing)
            return "flowing";
        if (targetHasWaterBucket)
            return "target_water_bucket";
    ...
    private boolean isRotationBlocked() {
        if (Helper.INSTANCE != null && Helper.INSTANCE.isEnabled() && Helper.targetRotation != null)
            return true;
