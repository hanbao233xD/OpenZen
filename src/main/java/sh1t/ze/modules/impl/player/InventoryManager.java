...
public class InventoryManager extends Module {
    ...
    private final ModeSetting offhandItemSetting = new ModeSetting("Offhand Items", "Golden Apple", "Fishing Rod",
            "None").withDefault("Projectile");
    private final ModeSetting bowPrioritySetting = new ModeSetting("Bow Priority", "Crossbow", "Punch Bow")
            .withDefault("Crossbow");
    ...
    private final NumberSetting maxEggsSnowballsSetting = new NumberSetting("Max Eggs & Snowballs Size", 64, 16, 256,
            16);
            ...
            if (this.didInventoryAction && !this.inventoryOnlySetting.getValue()) {
                if (packet instanceof ServerboundMovePlayerPacket) {
                    if (MovementUtil.isInputActive()) {
                        mc.getConnection()
                                .send(new ServerboundContainerClosePacket(mc.player.inventoryMenu.containerId));
                    }
    ...
    private boolean performInventoryAction() {
        ...
        if (this.autoArmorSetting.getValue()) {
            ...
            for (int i = 0; i < mc.player.getInventory().items.size(); i++) {
                ...
                if (candidate.isEmpty() || !(candidate.getItem() instanceof ArmorItem armor))
                    continue;
        ...
        if (ItemUtil.countBlocks() > this.maxBlockSizeSetting.getValue().intValue()) {
            if (this.throwItem(ItemUtil.getWorstBlock()))
                return true;
        }
        if (ItemUtil.countFood() > this.maxFoodSizeSetting.getValue().intValue()) {
            if (this.throwItem(ItemUtil.getBestFoodStack()))
                return true;
        }
        if (ItemUtil.countFishingRods() > this.maxRodSizeSetting.getValue().intValue()) {
            if (this.throwItem(ItemUtil.getFishingRodStack()))
                return true;
        }
        if (ItemUtil.countItem(Items.EGG) + ItemUtil.countItem(Items.SNOWBALL) > this.maxEggsSnowballsSetting.getValue()
                .intValue()) {
            if (this.throwItem(ItemUtil.getWorstProjectile()))
                return true;
        }
        ...
        if (this.bowSlotSetting.getValue().intValue() != 0) {
            ...
            if (ItemUtil.countItem(Items.ARROW) > 256) {
                if (this.throwItem(ItemUtil.getArrowStack()))
                    return true;
            }
        }
    ...
    private boolean throwItem(ItemStack stack) {
        if (mc.gameMode == null || mc.player == null)
            return false;
        if (!this.throwItemsSetting.getValue() || !ItemUtil.isUsable(stack))
            return false;
    ...
    public boolean isUsefulItem(ItemStack stack) {
        ...
        if (stack.getItem() instanceof BowItem && ItemUtil.countItem(Items.BOW) > 1)
            return false;
        if (stack.getItem() == Items.WATER_BUCKET && ItemUtil.countItem(Items.WATER_BUCKET) > getMaxWaterBuckets())
            return false;
        if (stack.getItem() == Items.LAVA_BUCKET && ItemUtil.countItem(Items.LAVA_BUCKET) > getMaxLavaBuckets())
            return false;
