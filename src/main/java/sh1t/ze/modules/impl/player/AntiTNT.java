...
public class AntiTNT
    ...
    public void onRender(RenderEvent renderEvent) {
        ...
        if (this.targetTnt != null && this.targetTnt.isAlive()) {
            ...
            if (fuseSeconds > 0.0f) {
                ...
                poseStack.pushPose();
                poseStack.translate((double) tntPos.getX() + 0.5 - vec3.x, (double) tntPos.getY() + 1.1 - vec3.y,
                        (double) tntPos.getZ() + 0.5 - vec3.z);
                poseStack.mulPose(mc.gameRenderer.getMainCamera().rotation());
                poseStack.scale(-0.025f, -0.025f, 0.025f);
                float textWidth = mc.font.width(fuseLabel);
                mc.font.drawInBatch(fuseLabel, -textWidth / 2.0f, 0.0f, -1, false, poseStack.last().pose(),
                        mc.renderBuffers().bufferSource(), Font.DisplayMode.NORMAL, 0, 0xF000F0);
                poseStack.popPose();
            }
        }
    ...
    private boolean isMoving() {
        ...
        return mc.options.keyUp.isDown() || mc.options.keyDown.isDown() || mc.options.keyLeft.isDown()
                || mc.options.keyRight.isDown() || mc.player.isSprinting();
    }

    private PrimedTnt findNearestTNT() {
        return mc.level.getEntitiesOfClass(PrimedTnt.class, mc.player.getBoundingBox().inflate(20.0)).stream()
                .filter(primedTnt -> primedTnt.getFuse() > 0)
                .filter(primedTnt -> this.isMovingTowardsPlayer(primedTnt) || this.hasLineOfSight(primedTnt))
                .min(Comparator.comparingDouble(primedTnt -> primedTnt.distanceToSqr(mc.player))).orElse(null);
    }
    ...
    private boolean hasLineOfSight(PrimedTnt primedTnt) {
        ...
        BlockHitResult hit = mc.level.clip(new ClipContext(tntPos, eyePos = mc.player.getEyePosition(),
    ...
    private boolean canPlaceAt(BlockPos blockPos) {
        return mc.level.getBlockState(blockPos).canBeReplaced()
                && !mc.player.getBoundingBox().intersects(new AABB(blockPos));
    }

    private int findBlockSlot() {
        for (int slot = 0; slot < 9; ++slot) {
            ItemStack itemStack = mc.player.getInventory().getItem(slot);
            if (itemStack.isEmpty() || !(itemStack.getItem() instanceof BlockItem) || !BlockUtil.isPlaceable(itemStack))
                continue;
            return slot;
        }
        return -1;
