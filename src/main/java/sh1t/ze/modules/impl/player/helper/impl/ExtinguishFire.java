    ...
    public void onMotion(MotionEvent motionEvent) {
        ...
        if (motionEvent.isPost()) {
            if (this.currentState == ExtinguishFire.State.FIRE) {
                if (this.firePos == null || !mc.level.getBlockState(this.firePos).is(Blocks.FIRE)
                        || mc.player.getEyePosition().distanceToSqr(Vec3.atCenterOf(this.firePos)) > 25.0) {
                    this.reset();
                } else {
                    this.targetRotation = RotationUtil.exactRotation(mc.player.getEyePosition(),
                            Vec3.atCenterOf(this.firePos));
                    if (!this.isAiming) {
                        this.isAiming = true;
                        this.aimDelay = 2;
                    }
        ...
        for (int i = -8; i <= 8; ++i) {
            for (int j = -2; j <= 2; ++j) {
                for (int k = -8; k <= 8; ++k) {
                    ...
                    if (!mc.level.getBlockState(blockPos3).is(Blocks.FIRE)
                            || !(Vec3.atCenterOf(blockPos3).distanceToSqr(vec3) <= 25.0)
                            || !Helper.isPositionInFov(Vec3.atCenterOf(blockPos3)))
                        continue;
                    arrayList.add(blockPos3);
                }
            }
        }
        this.firePos = (BlockPos) arrayList.stream()
                .min(Comparator.comparingDouble(blockPos -> Vec3.atCenterOf((Vec3i) blockPos).distanceToSqr(vec3)))
                .orElse(null);
    }
