...
public class AutoMLG
    ...
    public void onTick(TickEvent tickEvent) {
        ...
        if (mc.player.onGround() || mc.player.getAbilities().flying || mc.player.isInWaterRainOrBubble()
                || mc.player.isInLava()) {
            this.accumulatedFall = 0.0f;
        } else {
        ...
        if (this.recoveryActive) {
            ...
            if (recoveryHit.getType() == HitResult.Type.MISS
                    || !recoveryHit.getBlockPos().equals(this.placedWaterPos)) {
                this.recoveryActive = false;
                this.waterBucketSlot = null;
                this.placedWaterPos = null;
                return;
        ...
        if (!this.waterPlaced && !this.recoveryActive && this.placedWaterPos == null && this.postPlaceCooldown == 0
                && this.postActionCooldown == 0 && this.accumulatedFall <= 0.5f
                && ItemUtil.findItemInRange(0, 9, Items.WATER_BUCKET) < 0
                && (slot = ItemUtil.findItemInRange(0, 9, Items.BUCKET)) >= 0
                && (bucketPos = this.findBucketPos()) != null
                && (hit = this.raycastFluid(rotation = RotationUtil.rotationToBlock(bucketPos, 0.0f), 4.5))
        ...
        if (this.waterPlaced && !this.readyToPlace && mc.player.getDeltaMovement().y < 0.0
                && (deltaY = this.distanceToGround(2.5)) > 0.0 && deltaY <= 1.05) {
        ...
        if (ticksLeft <= this.predictTicksSetting.getValue().intValue()) {
            if (this.solidCheckSetting.getValue()
                    && !this.hasSolidBelow(BlockPos.containing(mc.player.getX(), mc.player.getY(), mc.player.getZ()))) {
                return;
            }
            ...
            for (int dx = -radius; dx <= radius; ++dx) {
                for (int dz = -radius; dz <= radius; ++dz) {
                    ...
                    if (!this.isWaterSource(candidatePos)
                            || (distSq = mc.player.position().distanceToSqr((double) candidatePos.getX() + 0.5,
                                    (double) candidatePos.getY() + 0.5,
                                    (double) candidatePos.getZ() + 0.5)) >= closestDistSq
                            || (hit = this.raycastFluid(rotation = RotationUtil.rotationToBlock(candidatePos, 0.0f),
                                    4.5)).getType() == HitResult.Type.MISS
                            || !hit.getBlockPos().equals(candidatePos))
                        continue;
    ...
    private BlockHitResult raycastSolid(Rotation rotation, double range) {
        ...
        return mc.level
                .clip(new ClipContext(eyePos, endPos, ClipContext.Block.OUTLINE, ClipContext.Fluid.NONE, mc.player));
    }
    ...
    private BlockHitResult raycastFluid(Rotation rotation, double range) {
        ...
        return mc.level.clip(
                new ClipContext(eyePos, endPos, ClipContext.Block.OUTLINE, ClipContext.Fluid.SOURCE_ONLY, mc.player));
    }
    ...
    private double distanceToGround(double maxDist) {
        ...
        BlockHitResult hit = mc.level.clip(new ClipContext(startPos, endPos = startPos.add(0.0, -maxDist, 0.0),
