...
public class AntiWeb extends Module {
    ...
    public void onMotion(MotionEvent event) {
        ...
        switch (currentPhase) {
            case IDLE -> {
                if (mc.player.isInWater() || !this.webCheckTimer.hasPassed(5) || !this.isInCobweb())
                    return;
                this.waterBucketSlot = ItemUtil.getSlot(Items.WATER_BUCKET);
                if (this.waterBucketSlot == -1 || this.waterBucketSlot > 8)
                    return;
            ...
            case RECYCLING -> {
                ...
                if (this.sentUsePacket && !this.placementTimer.hasPassed(5))
                    break;
    ...
    public void onRender(RenderEvent renderEvent) {
        if (currentPhase == Phase.IDLE || mc.gameRenderer == null)
            return;
