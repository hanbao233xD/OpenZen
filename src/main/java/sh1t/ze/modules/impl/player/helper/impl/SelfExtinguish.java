...
public class SelfExtinguish
    ...
    public enum State {
        NONE, PLACING, BREAKING, COMPLETE
    }
    ...
    public InteractionResult useItem(Player player, Level level, InteractionHand interactionHand) {
        if (ReflectionUtil.getStaticField(mc.gameMode, "localPlayerMode",
                "net/minecraft/client/multiplayer/MultiPlayerGameMode") == GameType.SPECTATOR) {
    ...
    public static boolean willCollideBelow(double deltaY) {
        ...
        Iterable<VoxelShape> collisions = mc.level.getBlockCollisions(mc.player,
                mc.player.getBoundingBox().move(0.0, deltaY, 0.0));
    ...
    public void onPreMotion(PreMotionEvent preMotionEvent) {
        ...
        if (this.shouldPickupWater) {
            ...
            if (mc.hitResult != null && mc.hitResult.getType() == HitResult.Type.BLOCK
                    && ((BlockHitResult) mc.hitResult).getDirection() == Direction.UP) {
            ...
            this.targetRotation = null;
            if (mc.hitResult != null && mc.hitResult.getType() == HitResult.Type.BLOCK
                    && ((BlockHitResult) mc.hitResult).getBlockPos().above().equals(this.waterBlockPos)) {
