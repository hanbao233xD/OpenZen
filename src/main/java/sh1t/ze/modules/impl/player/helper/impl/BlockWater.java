...
public class BlockWater
    ...
    private void placeBlock() {
        ...
        BlockHitResult blockHitResult = new BlockHitResult(this.currentPlacement.hitVec(),
                this.currentPlacement.direction(), this.currentPlacement.blockPos(), false);
        ...
        for (int dx = -3; dx <= 3; ++dx) {
            for (int dy = -2; dy <= 2; ++dy) {
                for (int dz = -3; dz <= 3; ++dz) {
                    ...
                    if (Helper.hasWaterPlacement(candidatePos) || !mc.level.getBlockState(candidatePos).is(Blocks.WATER)
    ...
    private Optional<BlockWater.PlacementData> findSuitableFace(BlockPos blockPos) {
        for (Direction direction : Direction.values()) {
            ...
            if (mc.level.getBlockState(neighborPos).isSolid()
                    && !mc.level.getBlockState(neighborPos).is(Blocks.WATER)) {
            ...
            if (!mc.level.getBlockState(belowPos).isSolid() || mc.level.getBlockState(belowPos).is(Blocks.WATER))
                continue;
