...
public class AutoClicker extends Module {
    ...
    private void doLeftClick() {
        if (mc.player.isUsingItem()
                ...
                || mc.screen != null
                || !this.leftClickTimer
                        .hasPassed(CpsUtil.toDelayMs(this.cpsMode.getValue(), this.cps.getValue().doubleValue()))) {
            return;
        }
    ...
    private void invokeStartUseItem() {
        try {
            Method method = mc.getClass()
                    .getDeclaredMethod(ReflectionUtil.getMappedMethodName(mc.getClass(), "startUseItem", "()V"));
    ...
    private void invokeStartAttack() {
        try {
            Method method = mc.getClass()
                    .getDeclaredMethod(ReflectionUtil.getMappedMethodName(mc.getClass(), "startAttack", "()V"));
            UnsafeUtil.makeAccessible(method);
            method.invoke(mc);
        } catch (IllegalAccessException | NoSuchMethodException | InvocationTargetException throwable) {
            throw new RuntimeException(throwable);
