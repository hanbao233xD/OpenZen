...
public class AimAssist
    ...
    public void onRender2D(Render2DEvent render2DEvent) {
        ...
        for (Player candidate : list) {
            if (candidate == null || candidate.equals(localPlayer) || !this.isValidTarget(candidate))
                continue;
        ...
        if (currentRot.getPitch() > rotToFeet.getPitch() || currentRot.getPitch() < rotToEye.getPitch()) {
            ...
            this.applyRotation(
                    new Rotation(targetYaw, targetPitch + (float) MathUtil.randomDouble(-randPitch, randPitch)));
        } else {
            this.targetOffset = playerEye.subtract(closestPos);
            this.isPitchAdjusting = false;
            this.applyRotation(new Rotation(targetYaw,
    ...
    public boolean isValidTarget(LivingEntity livingEntity) {
        if (mc.player.distanceTo(livingEntity) > this.range.getValue().floatValue()
                || !KillAura.INSTANCE.isValidTarget(livingEntity)) {
            return false;
        }
