    ...
    public void onMotion(MotionEvent motionEvent) {
        ...
        if (motionEvent.isPost()) {
            if (mc.gameMode.isDestroying()) {
                int bestSlot;
                ItemStack heldStack;
                if (this.checkSword.getValue()
                        && (heldStack = mc.player.getMainHandItem()).getItem() instanceof SwordItem) {
                    return;
                }
    ...
    private int getBestTool(BlockPos blockPos) {
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (destroySpeed > 1.0f && !(block instanceof DropExperienceBlock) && !(block instanceof RedStoneOreBlock)
                    && (efficiencyLevel = EnchantmentHelper.getItemEnchantmentLevel(Enchantments.BLOCK_EFFICIENCY,
                            itemStack)) > 0) {
