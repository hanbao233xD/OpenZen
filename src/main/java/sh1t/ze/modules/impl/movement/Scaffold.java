...
import sh1t.ze.event.impl.JumpEvent;
...
public class Scaffold extends Module {
    ...
    public final ModeSetting mode = new ModeSetting("Mode", "Normal", "Telly Bridge", "Old Telly", "Keep Y")
            .withDefault("Normal");
    ...
    public void onDisable() {
        if (mc != null && mc.player != null) {
            ...
            this.packetBatches.clear();
            boolean jumpDown = InputConstants.isKeyDown(mc.getWindow().getWindow(),
                    mc.options.keyJump.getKey().getValue());
            boolean shiftDown = InputConstants.isKeyDown(mc.getWindow().getWindow(),
    ...
    public void onTick(TickEvent event) {
        ...
        if (this.velocityDelay > 0)
            this.velocityDelay--;
        if (mc.player.onGround() && this.velocityDelay <= 30)
            this.velocityDelay = 0;
        ...
        if (this.currentPlacement == null) {
            ...
            ClientBase.delayPackets.add(() -> {
                ...
                if (previousTarget != null) {
                    if (previousTarget.getYaw() != rotationToBlock.getYaw()) {
                        PacketUtil.sendQueued(new ServerboundMovePlayerPacket.Rot(yaw, rotationToBlock.getPitch(),
                                mc.player.onGround()));
                    }
                } else {
                    PacketUtil.sendQueued(
                            new ServerboundMovePlayerPacket.Rot(yaw, rotationToBlock.getPitch(), mc.player.onGround()));
                }
                this.doSnap();
                this.onTick(event);
            });
            ...
            if (this.mode.is("Normal") && this.snap.getValue()) {
                ...
                this.rots.setYaw(RotationUtil.moveTowards((float) this.getBlockDistance(), this.rots.getYaw(),
                        this.correctRotation.getYaw()));
            }
    ...
    public void onPreMotion(PreMotionEvent event) {
        event.setCancelled(true);
        if (mc.screen != null || mc.player == null || this.currentPlacement == null)
            return;
        if ((this.mode.is("Telly Bridge") || this.mode.is("Old Telly")) && this.airTicks < 1)
            return;
        boolean canRayTrace = RayTraceUtil.canRayTrace(RotationHandler.targetRotation, this.currentPlacement.facing,
                this.currentPlacement.position, false);
        if (!this.canBuildNow && !this.isPlacementReachable(this.currentPlacement))
            return;
        if (this.rotationDelay <= 0 && !this.mode.is("Old Telly") && !canRayTrace)
            return;
        this.doSnap();
    }
    ...
    private boolean isPlacementReachable(PlacementTarget target) {
        if (target == null || mc.player == null)
            return false;
        Vec3 blockCenter = new Vec3(target.position.getX() + 0.5, target.position.getY() + 0.5f,
        ...
        return toBlock.lengthSqr() <= 20.25
                && toBlock.normalize()
                        .dot(Vec3.atLowerCornerOf(target.facing.getNormal().multiply(-1)).normalize()) >= 0.0;
    }
    ...
    private void applyRotations() {
        ...
        if (mc.level.getBlockState(belowFeet).entityCanStandOn(mc.level, belowFeet, mc.player))
            return;
        if (this.isAbovePlaceable(eye, belowFeet))
            return;
        for (int radius = 1; radius <= 6; radius++) {
            if (this.isAbovePlaceable(eye, new BlockPos(feetX, this.targetYLevel - radius, feetZ)))
                return;
                    ...
                    for (int signX = 0; signX <= 1; signX++) {
                        for (int signZ = 0; signZ <= 1; signZ++) {
                            BlockPos test = new BlockPos(
                                    ...
                                    feetZ + (signZ == 0 ? z : -z));
                            if (this.isAbovePlaceable(eye, test))
                                return;
                        }
                    }
                }
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (mc.level.getBlockState(offset).entityCanStandOnFace(mc.level, offset, mc.player, direction)) {
                ...
                if (delta.lengthSqr() <= 20.25
                    ...
                    this.currentPlacement = new PlacementTarget(
                            new BlockPos(offset.getX(), offset.getY(), offset.getZ()), direction.getOpposite());
                    return true;
                }
            }
        }
    ...
    private void doSnap() {
        if (this.currentPlacement == null || mc.player == null || mc.gameMode == null)
            return;
        ...
        if (!this.shouldBuild())
            return;
        BlockHitResult hit = new BlockHitResult(getHitVec(this.currentPlacement.position, facing), facing,
                this.currentPlacement.position, false);
    ...
    private PlacementTarget findPlacementTarget(BlockPos origin) {
        Direction[] directions = { Direction.DOWN, Direction.EAST, Direction.WEST, Direction.NORTH, Direction.SOUTH,
                Direction.UP };
    ...
    private boolean isValidBlock(BlockPos pos) {
        ...
        if (state.isAir())
            return false;
        if (pos.getY() > this.targetYLevel + 1.0)
            return false;
