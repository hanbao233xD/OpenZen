...
public class FireballBlink extends Module {
    ...
    private void releaseImpulse(int index) {
        if (index >= this.impulsePacketBoundaries.size())
            return;
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (this.isBlinking) {
            ...
            status = String.format("Blinking: %d packets, %.1fs, %d/%d", qSize, elapsed / 1000.0f, this.pendingReleases,
                    this.impulseCount);
        ...
        if (this.isBlinking) {
            ...
            RenderUtil.drawRoundedRect(event.poseStack(), barX, barY, barWidth, barHeight, 2.0f,
                    new Color(0, 0, 0, 128).getRGB());
            if (progress > 0.0f) {
                Color color = new Color(0, 200, 0, 200);
                RenderUtil.drawRoundedRect(event.poseStack(), barX, barY, barWidth * progress, barHeight, 2.0f,
                        color.getRGB());
            }
        }
    }
    ...
    public void onMotion(MotionEvent event) {
        ...
        if (event.isPost()) {
            if (this.fireballPrepareTick > 0) {
                ...
                if (this.fireballPrepareTick >= 2) {
                    ...
                    if (slot != -1) {
                        ...
                        ChatUtil.print(
                                "Throwing fireball " + this.queuedFireballs + " (charges=" + this.fireballTick + ")");
                    } else {
                        this.setEnabled(false);
                    }
                }
            ...
            if (current < this.fireballTick) {
                ...
                rotation = null;
                this.isThrowingFireball = false;
                ChatUtil.print("Used fireball " + this.queuedFireballs + " (had " + this.fireballTick + ", now "
                        + current + ")");
