...
public class AutoThrow
    ...
    public void onSprint(SprintEvent sprintEvent) {
        ...
        if (Scaffold.INSTANCE != null && Scaffold.INSTANCE.isEnabled()
                || Stuck.INSTANCE != null && Stuck.INSTANCE.isEnabled() || mc.player.isUsingItem()
                || mc.screen != null) {
            this.ticksUntilThrow = 0;
            this.targetRotation = null;
            return;
        ...
        for (slot = 0; slot < 9; ++slot) {
            ...
            if (itemStack.isEmpty()
                    || !(itemStack.getItem() instanceof EggItem) && !(itemStack.getItem() instanceof SnowballItem))
                continue;
            projectileSlot = slot;
            break;
        }
        if (mc.player.isUsingItem() || mc.player.getMainHandItem().getItem() instanceof BowItem
        ...
        if (--this.ticksUntilThrow == 0) {
            ...
            try {
                if (!(mc.player.getMainHandItem().getItem() instanceof EggItem)
                        && !(mc.player.getMainHandItem().getItem() instanceof SnowballItem))
                    return;
            ...
            if (!this.findTarget().isPresent()
                    || !this.throwTimer.hasPassed((float) ((long) this.throwDelay.getValue().doubleValue()))
    ...
    private Optional<? extends Player> findTarget() {
        ...
        return mc.level.players().stream().filter(player -> player != mc.player)
                .filter(player -> KillAura.INSTANCE.isValidTarget(player)).filter(player -> {
                    double dist = this.getDistanceTo(player);
                    return dist >= this.minDistance.getValue().doubleValue()
                            && dist <= this.maxDistance.getValue().doubleValue();
                }).filter(this::hasLineOfSight).filter(player -> !this.isInvisibleAlly(player))
    ...
    private boolean hasLineOfSight(Entity entity) {
        ...
        Vec3 eyePos = new Vec3(mc.player.getX(), mc.player.getY() + (double) mc.player.getEyeHeight(),
                mc.player.getZ());
        BlockHitResult hit = mc.level.clip(new ClipContext(eyePos,
                targetPos = new Vec3(entity.getX(), entity.getY() + (double) entity.getEyeHeight(), entity.getZ()),
