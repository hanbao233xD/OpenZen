...
public class MixMode
    ...
    public void onReceivePacket(ReceivePacketEvent receivePacketEvent) {
        ...
        if (this.webHitCount <= 0 || mc.player.isInWater() || mc.player.isUnderWater()) {
            if (packet instanceof ClientboundSetEntityMotionPacket motionPacket) {
                ...
                if (motionPacket.getYa() > 0) {
                    ...
                    if (KillAura.INSTANCE.isEnabled() && KillAura.target != null
                            && AntiKB.INSTANCE.tryAttack.getValue()) {
            ...
            if (this.isSuspending) {
                if (packet instanceof ClientboundMoveEntityPacket || packet instanceof ClientboundPingPacket
    ...
    public void onSprint(SprintEvent sprintEvent) {
        ...
        if (this.knockbackPacket != null && this.knockbackPacket.getYa() > 0 && mc.player.hurtTime > 0
                && KillAura.INSTANCE.isEnabled() && KillAura.target != null && this.shouldAttack) {
            this.shouldAttack = false;
            for (int i = 0; i < 2; ++i) {
    ...
    private void restoreMovementKeys() {
        mc.options.keyUp
                .setDown(InputConstants.isKeyDown(mc.getWindow().getWindow(), mc.options.keyUp.getKey().getValue()));
        mc.options.keyDown
                .setDown(InputConstants.isKeyDown(mc.getWindow().getWindow(), mc.options.keyDown.getKey().getValue()));
        mc.options.keyLeft
                .setDown(InputConstants.isKeyDown(mc.getWindow().getWindow(), mc.options.keyLeft.getKey().getValue()));
        mc.options.keyRight
                .setDown(InputConstants.isKeyDown(mc.getWindow().getWindow(), mc.options.keyRight.getKey().getValue()));
    }
    ...
    public void onGameTick(GameTickEvent gameTickEvent) {
        if (this.knockbackPacket != null && !AntiKB.INSTANCE.movementOverride.getValue() && mc.player != null) {
            if (mc.player.hurtTime > 6 && !mc.options.keyJump.isDown()) {
                ...
                boolean jumpKeyDown = InputConstants.isKeyDown(mc.getWindow().getWindow(),
