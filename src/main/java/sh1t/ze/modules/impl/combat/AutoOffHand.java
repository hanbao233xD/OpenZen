...
public class AutoOffHand extends Module {
    ...
    public void onTick(TickEvent event) {
        ...
        if (desired == ItemType.NONE)
            return;
        ItemStack mainHand = mc.player.getMainHandItem();
        if (this.isItemOfType(mainHand, desired))
            return;
        ItemStack offHand = mc.player.getOffhandItem();
        if (this.isItemOfType(offHand, desired))
            return;
    ...
    private int findItemSlot(ItemType type) {
        for (int i = 0; i < mc.player.getInventory().getContainerSize(); ++i) {
            ...
            if (this.isItemOfType(stack, type))
                return i;
        }
        return -1;
    }
    ...
    private void equipOffhand(int slot) {
        if (slot == -1 || this.equipCooldown > 0)
            return;
        if (mc.screen instanceof AbstractContainerScreen)
            return;
        if (slot >= 9 && this.getEmptyHotbarSlot() == -1)
            return;
