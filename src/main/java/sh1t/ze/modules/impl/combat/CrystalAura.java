...
public class CrystalAura
    ...
    public void onPacket(PacketEvent packetEvent) {
        ...
        if (rawPacket instanceof ClientboundAddEntityPacket addEntityPacket) {
            if (this.attackOnPacket.getValue() && addEntityPacket.getType() == EntityType.END_CRYSTAL) {
                EndCrystal endCrystal = new EndCrystal(mc.level, addEntityPacket.getX(), addEntityPacket.getY(),
                        addEntityPacket.getZ());
                ...
                if (mc.player.distanceTo(endCrystal) <= 4.0f) {
                    ...
                    mc.getConnection().send(new ServerboundMovePlayerPacket.PosRot(mc.player.getX(), mc.player.getY(),
                            mc.player.getZ(), rotation.getYaw(), rotation.getPitch(), mc.player.onGround()));
    ...
    public void onTick(TickEvent tickEvent) {
        if (mc.player != null && mc.level != null) {
            ...
            double hitDistance;
            Optional<Entity> crystalOpt = StreamSupport.stream(mc.level.entitiesForRendering().spliterator(), true)
                    .filter(entity -> entity instanceof EndCrystal).findAny();
            aimRotation = null;
            if (crystalOpt.isPresent()
                    && (hitDistance = RotationUtil.getMinHitDistance(crystalEntity = crystalOpt.get(),
                            rotation = RotationUtil.entityRotation(crystalEntity))) <= 3.0) {
