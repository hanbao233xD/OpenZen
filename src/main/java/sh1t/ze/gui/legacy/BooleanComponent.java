...
public class BooleanComponent
    ...
    public void renderWithAlpha(PoseStack poseStack, int mouseX, int mouseY, float partialTicks, float alpha) {
        ...
        this.toggleAnim = Math.abs(this.toggleAnim - target) > 0.01f
        ...
        float textY = (float) (rowY + verticalPadding)
                + ((float) contentHeight - FontStore.OPENSANS_16.getFontHeight()) / 2.0f;
        ...
        Color trackColor = this.booleanSetting.getValue() != false ? new Color(138, 180, 248, (int) (200.0f * alpha))
                : new Color(100, 100, 100, (int) (180.0f * alpha));
        ...
        RenderUtil.drawFilledRect(poseStack, knobX, knobY, knobWidth, knobHeight,
