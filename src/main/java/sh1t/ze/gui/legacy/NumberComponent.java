...
public class NumberComponent
    ...
    public void renderWithAlpha(PoseStack poseStack, int mouseX, int mouseY, float partialTicks, float alpha) {
        ...
        String valueText = this.numberSetting.getStep().doubleValue() % 1.0 == 0.0
                ? String.format("%d", new Object[] { this.numberSetting.getValue().intValue() })
                : String.format("%.2f", new Object[] { this.numberSetting.getValue().floatValue() });
        ...
        FontStore.OPENSANS_16.drawStringWithShadow(poseStack, valueText,
                (float) (panelX + panelWidth - textPadding) - FontStore.OPENSANS_16.getStringWidth(valueText), textY,
                textColor);
        ...
        int sliderHeight = 2;
        RenderUtil.drawFilledRect(poseStack, sliderX, sliderY, sliderWidth, sliderHeight,
                new Color(10, 10, 10, 200).getRGB());
        RenderUtil.drawFilledRect(poseStack, sliderX, sliderY, (int) ((float) sliderWidth * progress), sliderHeight,
                sliderColor);
    }
    ...
    public void mouseClicked(double mouseX, double mouseY, int button) {
        ...
        int valueTextWidth = (int) FontStore.OPENSANS_16.getStringWidth("00.00");
        int sliderWidth = panelWidth - textPadding * 2 - valueTextWidth + 20;
        boolean dup = overSlider = mouseX >= (double) sliderX && mouseX <= (double) (sliderX + sliderWidth)
