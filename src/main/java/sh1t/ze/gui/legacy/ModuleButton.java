...
public class ModuleButton {
    ...
    public int getTotalHeight() {
        ...
        List<SettingComponent> visibleComponents = this.settingComponents.stream()
                .filter(settingComponent -> settingComponent.setting.getVisibility().displayable())
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.drawFilledRect(poseStack, this.panel.x, rowY, this.panel.width, rowHeight,
        ...
        float nameY = (float) (this.panel.y + this.yOffset) + (float) this.panel.rowHeight / 2.0f
                - FontStore.OPENSANS_16.getFontHeight() / 2.0f;
        FontStore.OPENSANS_16.drawStringWithShadow(poseStack, name, nameX, nameY,
                this.module.isEnabled() ? ColorUtil.fromARGB(138, 180, 248, 255) : -1);
        if (!this.module.getSettings().isEmpty()) {
            ...
            poseStack.translate(-expandedHeight, -arrowY, 0.0f);
            FontStore.MATERIAL_20.drawStringWithShadow(poseStack, "",
                    expandedHeight - FontStore.MATERIAL_20.getStringWidth("") / 2.0f,
                    arrowY - FontStore.MATERIAL_20.getFontHeight() / 2.0f, -1);
            poseStack.popPose();
        }
        if ((expandedHeight = (float) this.getExpandedHeight() * this.expandAnim.getValueF()) > 0.0f) {
            ...
            RenderUtil.drawBlurredRect(poseStack, this.panel.x, contentY, this.panel.width, (int) expandedHeight + 1,
                    4.0f, 6.0f, 0.9f, 0);
            RenderUtil.pushScissor(this.panel.x, contentY, this.panel.width, (int) expandedHeight + 1);
            RenderUtil.drawFilledRect(poseStack, this.panel.x, contentY, this.panel.width, (int) expandedHeight + 1,
                    new Color(11, 11, 11, 150).getRGB());
            List<SettingComponent> visibleComponents = this.settingComponents.stream()
                    .filter(settingComponent -> settingComponent.setting.getVisibility().displayable())
                    .collect(Collectors.toList());
            ...
            for (SettingComponent settingComponent : visibleComponents) {
                ...
                if (settingComponent instanceof ModeComponent
                        && ((ModeComponent) (ref = settingComponent)).isDropdownOpen()) {
                    componentOffsetY += ((ModeComponent) ref).getDropdownHeight();
                }
                if (!(settingComponent instanceof MultiSelectComponent)
                        || !((MultiSelectComponent) (ref = settingComponent)).isDropdownOpen())
                    continue;
