...
public class CategoryPanel {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        this.tick();
        RenderUtil.drawFilledRect(guiGraphics.pose(), this.x, this.y, this.width, this.rowHeight,
                new Color(16, 16, 20, 245).getRGB());
        ...
        float iconY = (float) this.y + ((float) this.rowHeight / 2.0f - FontStore.ICON_30.getFontHeight() / 2.0f)
                + 3.0f;
        FontStore.ICON_30.drawStringWithShadow(guiGraphics.pose(), iconChar, this.x + 4, iconY, -1);
        float labelY = (float) this.y + ((float) this.rowHeight / 2.0f - FontStore.OPENSANS_18.getFontHeight() / 2.0f)
                - 0.5f;
        FontStore.OPENSANS_18.drawStringWithShadow(guiGraphics.pose(), this.category.displayName,
                this.x + this.rowHeight + 4, labelY, -1);
        for (ModuleButton moduleButton : this.moduleButtons) {
    ...
    public boolean isHovered(double mouseX, double mouseY) {
        return mouseX > (double) this.x && mouseX < (double) (this.x + this.width) && mouseY > (double) this.y
                && mouseY < (double) (this.y + this.rowHeight);
    }
