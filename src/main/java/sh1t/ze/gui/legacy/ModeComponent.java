...
public class ModeComponent
    ...
    public void renderWithAlpha(PoseStack poseStack, int mouseX, int mouseY, float partialTicks, float alpha) {
        int rowY = this.parentButton.panel.y + this.parentButton.yOffset + this.parentButton.panel.rowHeight
                + this.yOffset + 3;
        ...
        boolean hovered = mouseX >= boxX && mouseX <= boxX + boxWidth && mouseY >= rowY + verticalPadding
                && mouseY <= rowY + verticalPadding + contentHeight;
        Color boxColor = hovered && !this.dropdownOpen ? new Color(45, 45, 45, (int) (180.0f * alpha))
                : new Color(25, 25, 25, (int) (150.0f * alpha));
        ...
        RenderUtil.drawFilledRect(poseStack, panelX + outerPadding + 2, rowY + verticalPadding, innerPadding - 2,
                contentHeight, edgeColor.getRGB());
        RenderUtil.drawFilledRect(poseStack, panelX + panelWidth - outerPadding - innerPadding, rowY + verticalPadding,
                innerPadding - 2, contentHeight, edgeColor.getRGB());
        String name = this.modeSetting.getName();
        float textY = (float) (rowY + verticalPadding)
                + ((float) contentHeight - FontStore.OPENSANS_16.getFontHeight()) / 2.0f - 1.5f;
        ...
        float valueX = (float) (boxX + boxWidth) - FontStore.OPENSANS_16.getStringWidth(selectedValue)
                - (float) innerPadding;
        FontStore.OPENSANS_16.drawStringWithShadow(poseStack, selectedValue, valueX, textY,
                new Color(138, 180, 248).getRGB());
        if (this.dropdownOpen) {
            int dropdownY = rowY + verticalPadding + contentHeight;
            int dropdownHeight = this.getDropdownHeight();
            RenderUtil.drawFilledRect(poseStack, boxX, dropdownY, boxWidth, dropdownHeight,
                    new Color(33, 33, 33, 150).getRGB());
            String[] modes = this.getModes();
            for (int i = 0; i < modes.length; ++i) {
                boolean itemHovered;
                String mode = modes[i];
                float itemY = dropdownY + i * contentHeight;
                boolean dup = itemHovered = mouseX >= boxX && mouseX <= boxX + boxWidth && (float) mouseY >= itemY
                        && (float) mouseY < itemY + (float) contentHeight;
                if (itemHovered) {
                    RenderUtil.drawFilledRect(poseStack, boxX, itemY, boxWidth, contentHeight,
                ...
                FontStore.OPENSANS_16.drawStringWithShadow(poseStack, mode, modeTextX, modeTextY,
                        isSelected ? new Color(138, 180, 248).getRGB() : textColor);
            }
        }
    }
    ...
    public void mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (this.dropdownOpen) {
            ...
            int dropdownHeight = this.getDropdownHeight();
            if (mouseX >= (double) boxX && mouseX <= (double) (boxX + boxWidth) && mouseY >= (double) overHeader
                    && mouseY < (double) (overHeader + dropdownHeight)) {
        ...
        int headerHit = overHeader = mouseX >= (double) boxX && mouseX <= (double) (boxX + boxWidth)
                && mouseY >= (double) (rowY + verticalPadding)
                && mouseY <= (double) (rowY + verticalPadding + contentHeight) ? 1 : 0;
    ...
    public boolean isHovered(double mouseX, double mouseY) {
        ...
        int panelWidth = this.parentButton.panel.width;
        int rowHeight = this.parentButton.panel.rowHeight;
        boolean dup = overHeader = mouseX >= (double) panelX && mouseX <= (double) (panelX + panelWidth)
        ...
        if (this.dropdownOpen) {
            ...
            int dropdownHeight = this.getDropdownHeight();
            return mouseX >= (double) panelX && mouseX <= (double) (panelX + panelWidth) && mouseY >= (double) dropdownY
                    && mouseY <= (double) (dropdownY + dropdownHeight);
        }
        return false;
    }
