...
public class SettingsPopup
    ...
    private void renderPopupContent(GuiGraphics guiGraphics, int mouseX, int mouseY, float scale, float alpha) {
        ...
        TextGlow.drawBackground(guiGraphics.pose(), popupX, popupY, popupWidth, popupHeight, 12.0f * scale,
                effectiveAlpha);
        Renderer.renderConsumer((drawContext -> this.drawPopupBody(drawContext, guiGraphics, popupX, popupY, mouseX,
                mouseY, popupHeight, alphaByte, scale, popupWidth)));
    }

    private void drawPopupBody(DrawContext drawContext, GuiGraphics guiGraphics, int popupX, int popupY, int mouseX,
            int mouseY, int popupHeight, int alphaByte, float scale, int popupWidth) {
        int whiteColor = alphaByte << 24 | 0xFFFFFF;
        FontRenderer iconFont = FontPresets.materialIcons(18.0f * scale);
        GlHelper.drawText("\uE8B8", (float) popupX + 15.0f * scale, (float) popupY + 16.0f * scale, iconFont,
                whiteColor);
        ...
        GlHelper.drawText(title, (float) popupX + ((float) popupWidth - titleWidth) / 2.0f,
                (float) popupY + 35.0f * scale, titleFont, whiteColor);
        ...
        rowY += this.drawDropdown(drawContext, guiGraphics, "Language", this.selectedLanguage, LANGUAGES, "language",
                popupX, rowY, mouseX, mouseY, this.openAlpha, scale, popupWidth);
        rowY = (int) ((float) rowY + 8.0f * scale);
        this.drawDropdown(drawContext, guiGraphics, "Menu Scale", this.selectedScale, SCALES, "scale", popupX, rowY,
                mouseX, mouseY, this.openAlpha, scale, popupWidth);
        ...
        GlHelper.drawText(footer, (float) popupX + ((float) popupWidth - footerWidth) / 2.0f,
                (float) (popupY + popupHeight) - 15.0f * scale, footerFont, labelColor);
    }

    private void drawCloseButton(int popupX, int popupY, FontRenderer iconFont, int alphaByte, float scale,
            int popupWidth) {
        ...
        int r = (int) ((float) colorFrom.getRed()
                + (float) (colorTo.getRed() - colorFrom.getRed()) * this.closeButtonHoverAlpha);
        int g = (int) ((float) colorFrom.getGreen()
                + (float) (colorTo.getGreen() - colorFrom.getGreen()) * this.closeButtonHoverAlpha);
        int b = (int) ((float) colorFrom.getBlue()
                + (float) (colorTo.getBlue() - colorFrom.getBlue()) * this.closeButtonHoverAlpha);
    ...
    private int drawDropdown(DrawContext drawContext, GuiGraphics guiGraphics, String label, String selectedValue,
            String[] items, String key, int popupX, int rowY, int mouseX, int mouseY, float openAlpha, float scale,
            int popupWidth) {
        FontRenderer labelFont = FontPresets.axiformaRegular(13.0f * scale);
        FontRenderer valueFont = FontPresets.axiformaRegular(13.0f * scale);
        ...
        RenderUtil.drawRoundedRect(guiGraphics.pose(), dropdownX, rowY, dropdownWidth,
                dropdownHeaderHeight + expandedHeight, 4.0f * scale,
                this.applyAlpha(POPUP_BG_COLOR.getRGB(), openAlpha));
        float valueX = (float) dropdownX + 8.0f * scale;
        float valueY = (float) rowY + (float) dropdownHeaderHeight / 2.0f - valueFont.getMetrics().capHeight() / 2.0f
                + 3.0f * scale;
        ...
        float arrowY = (float) rowY + (float) dropdownHeaderHeight / 2.0f + arrowFont.getMetrics().capHeight() / 2.0f
                - 10.5f * scale + 7.0f;
        ...
        if (openFactor > 0.01f) {
            ...
            for (String item : filteredItems) {
                ...
                float itemTextY = (float) itemY + (float) itemHeight / 2.0f - valueFont.getMetrics().capHeight() / 2.0f
                        + 3.0f * scale;
    ...
    public boolean onMouseClick(int mouseX, int mouseY, float scale) {
        ...
        int screenWidth = mc.getWindow().getGuiScaledWidth();
        int popupX = (screenWidth - popupWidth) / 2 + this.offsetX;
        if (this.isMouseOverCloseButton(mouseX, mouseY, popupX,
                popupY = ((screenHeight = mc.getWindow().getGuiScaledHeight()) - (int) (200.0f * scale)) / 2
                        + this.offsetY,
                scale, popupWidth)) {
            this.toggleOpen();
            return true;
        ...
        boolean langHandled = this.handleDropdownClick(mouseX, mouseY, dropdownX, langRowY, dropdownWidth, LANGUAGES,
                this.selectedLanguage, "language", value -> {
                    this.selectedLanguage = value;
                }, scale);
        float langExpanded = (float) this.filterDropdownItems(LANGUAGES, this.selectedLanguage).length * (18.0f * scale)
                * this.dropdownAlpha.getOrDefault("language", 0.0f);
        int scaleRowY = (int) ((float) langRowY + 20.0f * scale + langExpanded + 8.0f * scale);
        boolean scaleHandled = this.handleDropdownClick(mouseX, mouseY, dropdownX, scaleRowY, dropdownWidth, SCALES,
    ...
    private boolean handleDropdownClick(int mouseX, int mouseY, int dropdownX, int dropdownY, int dropdownWidth,
            String[] items, String selectedValue, String key, Consumer<String> onSelect, float scale) {
        ...
        if (this.isPointInBounds(mouseX, mouseY, dropdownX, dropdownY, dropdownWidth, headerHeight)) {
            ...
            this.dropdownOpen.keySet().stream().filter(otherKey -> !otherKey.equals(key))
        ...
        if (open) {
            ...
            for (int i = 0; i < filtered.length; ++i) {
                if (!this.isPointInBounds(mouseX, mouseY, dropdownX, dropdownY + headerHeight + i * itemHeight,
                        dropdownWidth, itemHeight))
                    continue;
    ...
    private boolean isMouseInRect(int mouseX, int mouseY, int popupX, int popupY, float scale, int popupWidth) {
        ...
        boolean overCloseBtn = (float) mouseX >= closeBtnX - 10.0f * scale
                && (float) mouseX <= closeBtnX + 15.0f * scale;
        return mouseX >= popupX && mouseX <= popupX + popupWidth && mouseY >= popupY
                && (float) mouseY <= (float) popupY + 30.0f * scale && !overCloseBtn;
    }

    private boolean isMouseOverCloseButton(int mouseX, int mouseY, int popupX, int popupY, float scale,
            int popupWidth) {
        float closeBtnX = (float) (popupX + popupWidth) - 25.0f * scale;
        float closeBtnY = (float) popupY + 16.0f * scale;
        return (float) mouseX >= closeBtnX - 10.0f * scale && (float) mouseX <= closeBtnX + 15.0f * scale
                && (float) mouseY >= closeBtnY - 10.0f * scale && (float) mouseY <= closeBtnY + 10.0f * scale;
    }
    ...
    private void updateCloseButtonHover() {
        this.closeButtonHoverAlpha = this.isCloseButtonHovered ? LerpUtil.lerp(this.closeButtonHoverAlpha, 1.0f, 0.16f)
                : LerpUtil.lerp(this.closeButtonHoverAlpha, 0.0f, 0.16f);
    }
    ...
    private int calculatePopupHeight(float scale) {
        ...
        float langExpanded = (float) this.filterDropdownItems(LANGUAGES, this.selectedLanguage).length * itemHeight
                * this.dropdownAlpha.getOrDefault("language", 0.0f).floatValue();
        float scaleExpanded = (float) this.filterDropdownItems(SCALES, this.selectedScale).length * itemHeight
                * this.dropdownAlpha.getOrDefault("scale", 0.0f).floatValue();
