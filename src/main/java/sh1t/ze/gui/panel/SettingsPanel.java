...
public class SettingsPanel
    ...
    public enum AnimationState {
        NONE, FADE_IN, FADE_OUT, SWITCHING
    }
    ...
    public void render(GuiGraphics guiGraphics, int originX, int originY, int mouseX, int mouseY, Module module,
        ...
        this.scrollOffset = Math.abs(this.scrollOffset - this.scrollTarget) > 0.01f
                ? LerpUtil.smoothLerp(this.scrollOffset, this.scrollTarget, 0.35f)
                : this.scrollTarget;
        float enabledTarget = this.currentModule != null && this.currentModule.isEnabled() ? 1.0f : 0.0f;
        this.enabledAlpha = Math.abs(enabledTarget - this.enabledAlpha) > 0.01f
                ? LerpUtil.smoothLerp(this.enabledAlpha, enabledTarget, 0.28f)
                : enabledTarget;
        try {
            ...
            this.toggleHoverAlpha = Math.abs(toggleHoverTarget - this.toggleHoverAlpha) > 0.01f
                    ? LerpUtil.smoothLerp(this.toggleHoverAlpha, toggleHoverTarget, 0.35f)
                    : toggleHoverTarget;
            RenderUtil.drawRoundedRect(guiGraphics.pose(), panelX, panelY, panelWidth, panelHeight, 4.0f * scale,
                    this.applyAlpha(PANEL_BG_COLOR, alpha));
            this.renderToggleButton(guiGraphics, toggleX, toggleY, this.enabledAlpha, this.toggleHoverAlpha,
                    this.currentModule != null && this.currentModule.isEnabled(), scale, alpha);
            int stencilX = panelX;
            ...
            RenderUtil.drawRoundedRect(guiGraphics.pose(), stencilX, stencilY, stencilWidth, stencilHeight,
                    4.0f * scale, Color.WHITE.getRGB());
            ...
            Renderer.renderConsumer(drawContext -> {
                ...
                Module renderModule = this.animationState == SettingsPanel.AnimationState.FADE_OUT ? this.prevModule
                        : this.currentModule;
                float titleAlpha = this.animationState == SettingsPanel.AnimationState.FADE_OUT
                        ? (1.0f - this.transitionProgress) * alpha
                        : alpha;
                if (renderModule != null) {
                    ...
                    if (renderModule.isEnabled()) {
                        ...
                        TextGlow.drawGlowText(title, (float) panelX + 10.0f * scale, (float) panelY + 12.0f * scale,
                                titleFont, this.applyAlpha(-1, titleAlpha), glowColor, 12.0f * scale);
                    } else {
                        GlHelper.drawText(title, (float) panelX + 10.0f * scale, (float) panelY + 12.0f * scale,
                                titleFont, this.applyAlpha(-1, titleAlpha));
                    }
                }
                drawContext.restore();
                drawContext.save();
                drawContext
                        .clip(Rectangle.ofXYWH(panelX, panelY + headerHeight, panelWidth, panelHeight - headerHeight));
                float slideY = 0.0f;
                ...
                if (bodyModule != null && bodyAlpha > 0.01f) {
                    ...
                    if (settings != null && !settings.isEmpty()) {
                        ...
                        for (Setting<?> setting : settings) {
                            if (setting.getVisibility() != null && !setting.getVisibility().displayable())
                                continue;
                            int dy = SettingRendererRegistry.getInstance().render(guiGraphics, setting,
                                    panelX + (int) (10.0f * scale), settingY, panelWidth - (int) (20.0f * scale),
                                    mouseX, mouseY, bodyAlpha, scale);
                            settingY += dy;
                        }
                    } else {
                        ...
                        if (description != null && !description.isEmpty()) {
                            ...
                            this.renderWrappedText(description, panelX + (int) (10.0f * scale),
                                    panelY + headerHeight + (int) (10.0f * scale), panelWidth - (int) (20.0f * scale),
                                    descFont, -5592406, bodyAlpha, scale);
                        }
                    }
    ...
    private void renderScrollbar(GuiGraphics guiGraphics, int panelX, int panelY, int panelHeight, float scale,
            float alpha) {
        float targetAlpha;
        int headerHeight = (int) (30.0f * scale);
        int bottomPadding = (int) (10.0f * scale);
        ...
        this.scrollbarAlpha = Math.abs(this.scrollbarAlpha - targetAlpha) > 0.01f
                ? LerpUtil.smoothLerp(this.scrollbarAlpha, targetAlpha, 0.35f)
                : targetAlpha;
        ...
        RenderUtil.drawRoundedRect(guiGraphics.pose(), thumbX, thumbY, thumbWidth, thumbHeight, thumbWidth / 2.0f,
                thumbColor);
    }

    private void checkToggleHover(int toggleX, int toggleY, int mouseX, int mouseY, int toggleHeight) {
        this.isToggleHovered = mouseX >= toggleX && mouseX <= toggleX + toggleHeight * 2 && mouseY >= toggleY
                && mouseY <= toggleY + toggleHeight;
    }

    private void renderToggleButton(GuiGraphics guiGraphics, int toggleX, int toggleY, float enabledFactor,
            float hoverFactor, boolean enabled, float scale, float alpha) {
        int overlayColor;
        ...
        if (enabled) {
            ...
            RenderUtil.drawRoundedRect(guiGraphics.pose(), (float) toggleX - scale, (float) toggleY - scale,
                    (float) toggleWidth + 2.0f * scale, (float) toggleHeight + 2.0f * scale,
                    (float) toggleHeight / 2.0f, overlayColor);
        }
        RenderUtil.drawRoundedRect(guiGraphics.pose(), toggleX, toggleY, toggleWidth, toggleHeight,
                (float) toggleHeight / 2.0f, trackColorAlpha);
        ...
        if (hoverFactor > 0.0f) {
            ...
            RenderUtil.drawRoundedRect(guiGraphics.pose(), knobX - 1, knobY - 1, highlightSize, highlightSize,
                    highlightSize / 2.0f, this.applyAlpha(highlightColor, alpha));
        }
        RenderUtil.drawRoundedRect(guiGraphics.pose(), knobX, knobY, overlayColor, overlayColor,
                (float) overlayColor / 2.0f, this.applyAlpha(-1, alpha));
    }

    private int renderWrappedText(String text, int x, int y, int maxWidth, FontRenderer fontRenderer, int textColor,
    ...
    private void calculateTotalHeight(Module module, float scale) {
        ...
        if (settings != null && !settings.isEmpty()) {
            ...
            if (description != null && !description.isEmpty()) {
                ...
                this.totalContentHeight = this.calcWrappedTextHeight(description, (int) (385.0f * scale), fontRenderer,
                        scale);
            } else {
                this.totalContentHeight = 0.0f;
            }
    ...
    public boolean onMouseClick(int originX, int originY, int mouseX, int mouseY, int button, float scale) {
        ...
        if (this.totalContentHeight > visibleHeight) {
            float maxScroll = this.totalContentHeight - visibleHeight;
            float thumbHeight = Math.max(20.0f * scale, visibleHeight / this.totalContentHeight * visibleHeight);
            float thumbY = (float) (panelY + headerHeight)
                    + this.scrollOffset / maxScroll * (visibleHeight - thumbHeight);
            float thumbWidth = 4.0f * scale;
            int thumbX = panelX + panelWidth - (int) thumbWidth - 2;
            if (mouseX >= thumbX && (float) mouseX <= (float) thumbX + thumbWidth && (float) mouseY >= thumbY
                    && (float) mouseY <= thumbY + thumbHeight) {
        ...
        int toggleX = panelX + panelWidth - toggleHeight * 2 - toggleRightPadding;
        int toggleY = panelY + (headerHeight - toggleHeight) / 2;
        if (button == 0 && mouseX >= toggleX && mouseX <= toggleX + toggleHeight * 2 && mouseY >= toggleY
                && mouseY <= toggleY + toggleHeight) {
        ...
        if (settings != null && !settings.isEmpty()) {
            ...
            for (Setting setting : settings) {
                ...
                if (adjustedMouseY >= settingY && adjustedMouseY <= settingY + settingHeight
                        && SettingRendererRegistry.getInstance().onClick(setting, panelX + (int) (10.0f * scale),
                                settingY - (int) this.scrollOffset, panelWidth - (int) (20.0f * scale), mouseX, mouseY,
                                button, scale)) {
                    return true;
                }
    ...
    public void onMouseDrag(double mouseX, double mouseY, float scale) {
        ...
        if (this.currentModule != null) {
            for (Setting setting : this.currentModule.getSettings()) {
                ...
                if (setting.getVisibility() != null && !setting.getVisibility().displayable()
                        || (settingRenderer = SettingRendererRegistry.getInstance().findRenderer(setting)) == null)
                    continue;
    ...
    private void rescaleScroll(float scale) {
        ...
        if (this.totalContentHeight > (visibleHeight = (float) (contentHeight = (int) (400.0f * scale)
                - (int) (40.0f * scale) - (int) (20.0f * scale)) - (headerHeight = 30.0f * scale))) {
            float maxScroll = this.totalContentHeight - visibleHeight;
