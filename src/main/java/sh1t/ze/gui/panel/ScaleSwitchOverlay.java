...
public class ScaleSwitchOverlay
    ...
    private void drawBackground(GuiGraphics guiGraphics, int screenWidth, int screenHeight) {
        Color color = new Color(OVERLAY_BG_COLOR.getRed(), OVERLAY_BG_COLOR.getGreen(), OVERLAY_BG_COLOR.getBlue(),
                (int) ((float) OVERLAY_BG_COLOR.getAlpha() * this.alpha));
    ...
    private void drawContent(GuiGraphics guiGraphics, int boxX, int boxY, float boxWidth, float scale) {
        Renderer.renderConsumer((drawContext -> {
            ...
            String description = String.format(Locale.US, "Switching scale from %.0f%% to %.0f%%",
                    new Object[] { this.fromScale * 100.0f, this.toScale * 100.0f });
