...
public class ModuleListPanel
    ...
    public void setSearchQuery(String query) {
        ...
        this.searchResults = !query.isEmpty()
                ? ZenClient.instance.getModuleManager().getModules().stream()
                        .filter(module -> module.getName().toLowerCase().contains(query.toLowerCase()))
    ...
    public void render(GuiGraphics guiGraphics, int originX, int originY, int mouseX, int mouseY, Category category,
        ...
        if (this.currentCategory != category) {
            ...
            List<Module> filtered = this.currentModules = category == null ? null
                    : ZenClient.instance.getModuleManager().getModules().stream()
        ...
        try {
            ...
            RenderUtil.drawRoundedRect(guiGraphics.pose(), panelX, panelY, panelWidth, panelHeight, 4.0f * scale,
                    this.applyAlpha(HOVER_BG_COLOR, alpha));
            Renderer.render(guiGraphics, drawContext -> {
                ...
                if (renderCategory != null) {
                    ...
                    this.renderModuleList(renderCategory, renderList, panelX, panelY, panelHeight, mouseX, mouseY,
                            renderAlpha, this.animationState != ModuleListPanel.AnimationState.NONE, scale);
                    drawContext.restore();
                }
                drawContext.restore();
            });
    ...
    private void renderSearchResults(GuiGraphics guiGraphics, int originX, int originY, int mouseX, int mouseY,
            float scale, float alpha) {
        try {
            int panelWidth = (int) (160.0f * scale);
            int marginX = (int) (20.0f * scale);
            ...
            RenderUtil.drawRoundedRect(guiGraphics.pose(), panelX, panelY, panelWidth, panelHeight, 4.0f * scale,
                    this.applyAlpha(HOVER_BG_COLOR, alpha));
            Renderer.render(guiGraphics, drawContext -> this.renderModuleList(null, this.searchResults, panelX, panelY,
                    panelHeight, mouseX, mouseY, alpha, false, scale));
            this.renderScrollbar(guiGraphics, panelX, panelY, panelHeight, scale, alpha);
        } catch (Exception exception) {
            // empty catch block
        }
    ...
    private void renderModuleList(Category category, List<Module> modules, int panelX, int panelY, int panelHeight,
            int mouseX, int mouseY, float alpha, boolean animating, float scale) {
        float headerHeight = 30.0f * scale;
        int panelWidth = (int) (160.0f * scale);
        FontRenderer titleFont = FontPresets.axiformaBold(20.0f * scale);
        String titleText = category == null ? "Search"
                : category.name().substring(0, 1).toUpperCase() + category.name().substring(1).toLowerCase();
        GlHelper.drawText(titleText, (float) panelX + 10.0f * scale, (float) panelY + 12.0f * scale, titleFont,
                this.applyAlpha(-1, alpha));
        ...
        GlHelper.drawText(subtitle, subtitleX, (float) panelY + 14.0f * scale, subtitleFont,
                this.applyAlpha(-5592406, alpha));
        DrawContext drawContext = GlHelper.getCanvas();
        drawContext.save();
        drawContext.clip(Rectangle.ofXYWH(panelX, (float) panelY + headerHeight, panelWidth,
                (float) panelHeight - headerHeight));
        // Recaf used +2*scale here, but the deobfuscated GlyphMetrics flips the ascent
        // sign so
        // GlHelper.drawText draws ~7*scale ABOVE drawY — at +2 the first row's text top
        // slips out
        ...
        if (modules != null) {
            ...
            for (Module module : modules) {
                ...
                FontRenderer moduleFont = module.isEnabled() ? FontPresets.axiformaBold(16.0f * scale)
                        : FontPresets.axiformaRegular(16.0f * scale);
                ...
                if (module.isEnabled()) {
                    ...
                    TextGlow.drawGlowText(moduleName, (float) panelX + 10.0f * scale, drawY, moduleFont, textColor,
                            glowColor, 8.0f * scale);
                    ...
                    if (!bindName.equalsIgnoreCase("None")) {
                        ...
                        TextGlow.drawGlowText(iconText, bindX, drawY - (float) Math.round(scale), iconFont, textColor,
                                glowColor, 8.0f * scale);
                        TextGlow.drawGlowText(bindName, bindX + iconWidth + spacing, drawY - 2.0f, moduleFont,
                                textColor, glowColor, 8.0f * scale);
                    }
                } else {
                    ...
                    if (!bindName.equalsIgnoreCase("None")) {
                        ...
                        float bindX = (float) (panelX + panelWidth) - totalWidth - 10.0f * scale;
                        TextGlow.drawGlowText(iconText, bindX, drawY - (float) Math.round(scale), iconFont, textColor,
                                glowColor, 5.0f * scale);
                        TextGlow.drawGlowText(bindName, bindX + iconWidth + spacing, drawY - 2.0f, moduleFont,
                                textColor, glowColor, 5.0f * scale);
                    }
                }
    ...
    public boolean onMouseClick(int originX, int originY, int mouseX, int mouseY, Category category, int button,
            float scale) {
        List<Module> modules;
        List<Module> dup = modules = !this.searchQuery.isEmpty() ? this.searchResults : this.currentModules;
        if (modules == null
                || this.searchQuery.isEmpty() && this.animationState != ModuleListPanel.AnimationState.NONE) {
            return false;
        }
        int panelWidth = (int) (160.0f * scale);
        int marginX = (int) (20.0f * scale);
        ...
        // Keep in sync with renderModuleList's rowY offset (+8*scale) so click
        // hit-boxes match
        // the visible row positions.
        int rowY = panelY + headerHeight + Math.round(8.0f * scale);
        for (Module module : modules) {
            int adjustedRowY;
            if (this.isMouseOverModule(module, panelX, adjustedRowY = (int) ((float) rowY - this.scrollOffset), mouseX,
                    mouseY, scale)) {
                if (button == 0) {
                    module.toggle();
    ...
    public boolean isMouseOverPanel(int originX, int originY, int mouseX, int mouseY, float scale) {
        ...
        int panelY = originY + marginY + (int) (23.0f * scale);
        int panelHeight = baseSize - 2 * marginX - (int) (20.0f * scale);
        return mouseX >= panelX && mouseX <= panelX + panelWidth && mouseY >= panelY + headerHeight
