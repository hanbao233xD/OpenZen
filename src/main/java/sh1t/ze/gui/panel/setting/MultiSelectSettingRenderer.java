...
public class MultiSelectSettingRenderer
    ...
    public int render(GuiGraphics guiGraphics, Setting<?> setting, int x, int y, int width, int mouseX, int mouseY,
        ...
        TextGlow.drawGlowText(multiSelectSetting.getName(), x, nameY, nameFont, this.applyAlpha(-1, alpha),
        ...
        for (String option : multiSelectSetting.getOptions()) {
            ...
            if (selectedAmount > 0.01f) {
                ...
                RenderUtil.drawRoundedRect(guiGraphics.pose(), (float) boxX - scale, (float) boxY - scale,
                        (float) boxSize + 2.0f * scale, (float) boxSize + 2.0f * scale, 2.0f * scale, color);
            }
            RenderUtil.drawRoundedRect(guiGraphics.pose(), boxX, boxY, boxSize, boxSize, 2.0f * scale,
                    this.applyAlpha(-5592406, alpha));
            if ((double) selectedAmount > 0.01) {
                float inset = (1.0f - selectedAmount) * ((float) boxSize / 2.0f);
                RenderUtil.drawRoundedRect(guiGraphics.pose(), (float) boxX + inset, (float) boxY + inset,
                        (float) boxSize - inset * 2.0f, (float) boxSize - inset * 2.0f, scale,
                        this.applyAlpha(COLOR_SELECTED, alpha));
            }
            ...
            if (selectedAmount > 0.01f) {
                ...
                TextGlow.drawGlowText(option, x + rightPadding, labelY, labelFont, this.applyAlpha(labelColor, alpha),
                        glowColor, 8.0f * scale * selectedAmount);
            } else {
    ...
    private void updateAnimation(Map<String, Float> map, String key, boolean target, float speed) {
        ...
        current = Math.abs(targetValue - current) > 0.01f ? LerpUtil.smoothLerp(current, targetValue, speed)
                : targetValue;
        map.put(key, current);
    }
