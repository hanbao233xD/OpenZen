...
public class ModeSettingRenderer
    ...
    public int render(GuiGraphics guiGraphics, Setting<?> setting, int x, int y, int width, int mouseX, int mouseY,
        ...
        String[] otherModes = Arrays.stream((Object[]) modeSetting.getModes())
                .filter(mode -> !Objects.equals(mode, modeSetting.getValue())).toArray(String[]::new);
        ...
        try {
            ...
            int dropdownY = y + 1;
            RenderUtil.drawRoundedRect(guiGraphics.pose(), (float) dropdownX, (float) dropdownY, (float) dropdownWidth,
                    (float) (itemHeight + expandedHeight), 4.0f * scale, this.applyAlpha(DROPDOWN_BG_COLOR, alpha));
            ...
            TextGlow.drawGlowText(modeSetting.getName(), x, nameY, nameFont, this.applyAlpha(-1, alpha),
                    this.applyAlpha(nameGlow, alpha), 8.0f * scale);
            ...
            float selectedY = (float) dropdownY + (float) itemHeight / 2.0f - valueFont.getMetrics().capHeight() / 2.0f
                    + 2.0f * scale;
            TextGlow.drawGlowText(selectedValue, selectedX, selectedY, valueFont, this.applyAlpha(-3355444, alpha),
                    this.applyAlpha(new Color(255, 255, 255, 150).getRGB(), alpha), 10.0f * scale);
            ...
            if ((double) openFactor2 > 0.01) {
                ...
                for (int i = 0; i < otherModes.length; ++i) {
                    ...
                    float modeTextY = (float) itemY + (float) itemHeight / 2.0f
                            - valueFont.getMetrics().capHeight() / 2.0f + 2.0f * scale;
    ...
    public boolean onClick(Setting<?> setting, int x, int y, int width, int mouseX, int mouseY, int button,
        ...
        if (open) {
            ...
            for (int i = 0; i < otherModes.length; ++i) {
                if (!this.isMouseInBounds(mouseX, mouseY, dropdownX, firstItemY + i * itemHeight, dropdownWidth,
                        itemHeight) || button != 0)
                    continue;
