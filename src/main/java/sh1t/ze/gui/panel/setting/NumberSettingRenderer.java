...
import sh1t.ze.render.GlyphMetrics;
...
public class NumberSettingRenderer
    ...
    public int render(GuiGraphics guiGraphics, Setting<?> setting, int x, int y, int width, int mouseX, int mouseY,
        ...
        this.updateHoverStates(numberSetting, mouseX, mouseY, widgetX, widgetY, widgetWidth, sidePadding, widgetHeight,
                iconX, iconY, iconSize);
        this.drawNumberWidget(guiGraphics, numberSetting, x, y, width, widgetX, widgetY, widgetWidth, widgetHeight,
                iconX, iconY, editing, alpha, scale);
        return rowHeight;
    }
    ...
    private static void commitEdit() {
        ...
        try {
            ...
            PanelClickGui.panelClickGui.addToast(editingNumberSetting.getName() + " set to "
                    + String.format(Locale.US, "%.1f", new Object[] { parsed }));
    ...
    private void drawNumberWidget(GuiGraphics guiGraphics, NumberSetting numberSetting, int x, int y, int width,
            int widgetX, int widgetY, int widgetWidth, int widgetHeight, int iconX, int iconY, boolean editing,
            float alpha, float scale) {
        String displayText;
        ...
        float nameY = centerY - (nameMetrics.ascent() + nameMetrics.descent()) / 2.0f + nameMetrics.ascent()
                + 6.0f * scale;
        TextGlow.drawGlowText(numberSetting.getName(), x, nameY, nameFont, this.applyAlpha(-1, alpha),
                this.applyAlpha(new Color(255, 255, 255, 120).getRGB(), alpha), 8.0f * scale);
        this.drawEditIcon(guiGraphics, iconX, iconY, numberSetting, alpha, scale);
        int sidePadding = Math.round(12.0f * scale);
        RenderUtil.drawRoundedRect(guiGraphics.pose(), widgetX, widgetY, widgetWidth, widgetHeight, 5.0f * scale,
                this.applyAlpha(0x50F5F5F5, alpha));
        int middleX = widgetX + sidePadding;
        int middleWidth = widgetWidth - sidePadding * 2;
        RenderUtil.drawRoundedRect(guiGraphics.pose(), middleX, widgetY, middleWidth, widgetHeight, 0.0f,
                this.applyAlpha(1086900424, alpha));
        GlyphMetrics signMetrics = signFont.getMetrics();
        float widgetCenterY = (float) widgetY + (float) widgetHeight / 2.0f;
        float signY = widgetCenterY - (signMetrics.ascent() + signMetrics.descent()) / 2.0f + signMetrics.ascent()
                + 5.0f * scale - 2.5f;
        int minusColor = this.minusButtonHover.getOrDefault(numberSetting, false) != false
                ? new Color(255, 255, 255).getRGB()
                : -1;
        GlHelper.drawText("-",
                (float) widgetX + (float) sidePadding / 2.0f - GlHelper.getStringWidth("-", signFont) / 2.0f, signY,
                signFont, this.applyAlpha(minusColor, alpha));
        int plusColor = this.plusButtonHover.getOrDefault(numberSetting, false) != false
                ? new Color(255, 255, 255).getRGB()
                : -1;
        GlHelper.drawText("+",
                (float) (widgetX + widgetWidth - sidePadding)
                        + ((float) sidePadding / 2.0f - GlHelper.getStringWidth("+", signFont) / 2.0f),
                signY, signFont, this.applyAlpha(plusColor, alpha));
        ...
        if (editing) {
            ...
            RenderUtil.drawFilledRect(guiGraphics.pose(), caretX, caretY, Math.round(scale), Math.round(caretHeight),
                    caretColor);
        } else {
            int glowColor = new Color(255, 255, 255, 120).getRGB();
            TextGlow.drawGlowText(displayText, displayX, displayY, valueFont, this.applyAlpha(-1, alpha),
                    this.applyAlpha(glowColor, alpha), 6.0f * scale);
        }
    }

    private void drawEditIcon(GuiGraphics guiGraphics, int iconX, int iconY, NumberSetting numberSetting, float alpha,
            float scale) {
        ...
        float drawY = (float) iconY + (float) iconSize / 2.0f - (iconMetrics.ascent() + iconMetrics.descent()) / 2.0f
                + iconMetrics.ascent() + 1.0f;
    ...
    private void updateHoverStates(NumberSetting numberSetting, int mouseX, int mouseY, int widgetX, int widgetY,
            int widgetWidth, int sidePadding, int widgetHeight, int iconX, int iconY, int iconSize) {
        boolean iconHovered;
        boolean overWidget = mouseX >= widgetX && mouseX <= widgetX + widgetWidth && mouseY >= widgetY
                && mouseY <= widgetY + widgetHeight;
        ...
        boolean dup = iconHovered = mouseX >= iconX && mouseX <= iconX + iconSize && mouseY >= iconY
