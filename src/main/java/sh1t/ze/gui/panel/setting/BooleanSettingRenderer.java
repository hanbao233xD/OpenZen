...
public class BooleanSettingRenderer
    ...
    public int render(GuiGraphics guiGraphics, Setting<?> setting, int x, int y, int width, int mouseX, int mouseY,
        ...
        float toggleAnim = this.toggleAnimations
                .getOrDefault(booleanSetting, booleanSetting.getValue() != false ? 1.0f : 0.0f).floatValue();
        float toggleTarget = booleanSetting.getValue() != false ? 1.0f : 0.0f;
        toggleAnim = Math.abs(toggleTarget - toggleAnim) > 0.01f ? LerpUtil.smoothLerp(toggleAnim, toggleTarget, 0.25f)
                : toggleTarget;
        ...
        hoverAnim = Math.abs(hoverTarget - hoverAnim) > 0.01f ? LerpUtil.smoothLerp(hoverAnim, hoverTarget, 0.3f)
                : hoverTarget;
        ...
        TextGlow.drawGlowText(booleanSetting.getName(), x, nameY, nameFont, this.applyAlpha(-1, alpha),
                this.applyAlpha(new Color(255, 255, 255, 120).getRGB(), alpha), 6.0f * scale);
    ...
    private void drawToggle(GuiGraphics guiGraphics, int toggleX, int toggleY, float onFactor, float hoverFactor,
            float alpha, float scale) {
        ...
        if (onFactor > 0.01f) {
            ...
            RenderUtil.drawRoundedRect(guiGraphics.pose(), (float) toggleX - scale, (float) toggleY - scale,
                    ...
                    (float) toggleHeight / 2.0f, this.applyAlpha(overlayColor, alpha));
        }
        RenderUtil.drawRoundedRect(guiGraphics.pose(), toggleX, toggleY, toggleWidth, toggleHeight,
                (float) toggleHeight / 2.0f, this.applyAlpha(trackColor, alpha));
        ...
        if (hoverFactor > 0.0f) {
            ...
            RenderUtil.drawRoundedRect(guiGraphics.pose(), knobX - 1, knobY - 1, knobSize + 2, knobSize + 2,
                    (float) (knobSize + 2) / 2.0f, this.applyAlpha(highlightColor, alpha));
        }
        RenderUtil.drawRoundedRect(guiGraphics.pose(), knobX, knobY, knobSize, knobSize, (float) knobSize / 2.0f,
                this.applyAlpha(knobColor, alpha));
    }

    private void updateHoverState(BooleanSetting booleanSetting, int toggleX, int toggleY, int mouseX, int mouseY,
            int toggleHeight) {
        boolean hovered = mouseX >= toggleX && mouseX <= toggleX + toggleHeight * 2 && mouseY >= toggleY
    ...
    public boolean onClick(Setting<?> setting, int x, int y, int width, int mouseX, int mouseY, int button,
        ...
        int toggleX = x + width - toggleWidth - rightPadding;
        int toggleY = y + (rowHeight - toggleH) / 2;
        if (button == 0 && mouseX >= toggleX && mouseX <= toggleX + toggleWidth && mouseY >= toggleY
