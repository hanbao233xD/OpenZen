...
public class ProfileWidget
    ...
    public void render(GuiGraphics guiGraphics, int originX, int originY, int mouseX, int mouseY, float scale,
            float alpha) {
        if (mc.player == null) {
            return;
        }
        try {
            ...
            Renderer.renderConsumer(drawContext -> {
                if (this.hoverAlpha > 0.01f) {
                    ...
                    RenderUtil.drawRoundedRect(guiGraphics.pose(), avatarX - 2, avatarY - 2, avatarSize + 4,
                            avatarSize + 4, cornerRadius + 1.0f, hoverColor);
                }
                if (mc.player instanceof AbstractClientPlayer) {
                    GlHelper.drawPlayerHeadRounded(mc.player, avatarX, avatarY, avatarSize, avatarSize, alpha,
                            cornerRadius);
                }
                FontRenderer nameFont = FontPresets.axiformaRegular(14.0f * scale);
                int glowColor = new Color(255, 255, 255, (int) (100.0f * alpha)).getRGB();
                TextGlow.drawGlowText(userId, textX, textY, nameFont, this.applyAlpha(-1, alpha), glowColor,
                        8.0f * scale);
                ...
                int roleShadowColor = new Color(roleColor.getRed(), roleColor.getGreen(), roleColor.getBlue(),
                        (int) (180.0f * alpha)).getRGB();
                RenderUtil.drawRoundedRect(guiGraphics.pose(), roleBoxX - 3, (float) roleBoxY + 2.5f * scale - 2.0f,
                        roleBoxW + 2, roleBoxH + 2, 5.0f * scale, this.applyAlpha(roleShadowColor, alpha * 0.35f));
                RenderUtil.drawRoundedRect(guiGraphics.pose(), roleBoxX - 2, (float) roleBoxY + 3.5f * scale - 2.0f,
                        roleBoxW, roleBoxH, 4.0f * scale, this.applyAlpha(roleColor.getRGB(), alpha));
                int roleTextGlow = new Color(255, 255, 255, (int) (120.0f * alpha)).getRGB();
                TextGlow.drawGlowText(userRole, (float) roleBoxX + 1.5f * scale, (float) roleBoxY + 6.5f * scale,
                        roleFont, this.applyAlpha(-1, alpha), roleTextGlow, 5.0f * scale);
            });
    ...
    public boolean isMouseOverAvatar(int originX, int originY, int mouseX, int mouseY, float scale) {
        ...
        int avatarX = textX - avatarSize - (int) (5.0f * scale);
        int avatarY = textY - (int) (8.0f * scale);
        return mouseX >= avatarX && mouseX <= avatarX + avatarSize && mouseY >= avatarY
                && mouseY <= avatarY + avatarSize;
    }
    ...
    private void checkHover(int avatarX, int avatarY, int mouseX, int mouseY, int avatarSize) {
        this.isHovered = mouseX >= avatarX && mouseX <= avatarX + avatarSize && mouseY >= avatarY
                && mouseY <= avatarY + avatarSize;
    }

    private void updateHoverAlpha() {
        this.hoverAlpha = this.isHovered ? LerpUtil.lerp(this.hoverAlpha, 1.0f, 0.12f)
                : LerpUtil.lerp(this.hoverAlpha, 0.0f, 0.12f);
    }
