...
public class NumberSettingElement
    ...
    public void render(NewClickGui clickGui, GuiGraphics guiGraphics, PoseStack poseStack, int mouseX, int mouseY,
        ...
        FontStore.AXIFORMA_REGULAR_14.drawString(poseStack, name, this.x + 6.0f, nameY,
        ...
        FontStore.AXIFORMA_BOLD_13.drawString(poseStack, valueText,
                this.x + 120.0f - FontStore.AXIFORMA_BOLD_13.getStringWidth(valueText) - 6.0f, nameY,
                ColorUtil.withAlpha(-1, alpha * 0.92f));
        ...
        if (this.isDragging) {
            ...
            double rawValue = numberSetting.getMin().floatValue()
                    + (numberSetting.getMax().floatValue() - numberSetting.getMin().floatValue()) * dragRatio;
            double step = numberSetting.getStep().floatValue();
            double stepped = (double) Math.round(
                    MathUtil.clamp(rawValue, numberSetting.getMin().floatValue(), numberSetting.getMax().floatValue())
                            / step)
                    * step;
            numberSetting.setValue((double) Math.round(stepped * 1000.0) / 1000.0);
        }
        RenderUtil.drawRoundedRect(poseStack, this.x + 6.0f, sliderY, sliderWidth, 5.0f, 2.0f,
                ColorUtil.withAlpha(ColorUtil.fromRGB(60, 60, 60), alpha));
        float fillAmount = this.sliderTimer.getValueF();
        dragRatio = 10.0f;
        float glowSize = 5.0f + dragRatio * 2.0f;
        RenderUtil.drawRoundedRect(poseStack, this.x + 6.0f - dragRatio, sliderY - dragRatio,
                Math.max(sliderWidth * fillAmount + dragRatio * 2.0f, glowSize), glowSize,
                2.0f + dragRatio / 2.0f + 1.0f, dragRatio, ColorUtil.withAlpha(-13768502, 0.26f * alpha));
        RenderUtil.drawRoundedRect(poseStack, this.x + 6.0f, sliderY, sliderWidth * fillAmount, 5.0f, 2.0f,
                ColorUtil.withAlpha(CategoryPanel.ACCENT_COLOR, alpha));
        float knobX = Math.max(this.x + 6.0f + this.sliderTimer.getValueF() * sliderWidth - 5.0f - 0.5f,
                this.x + 6.0f - 0.5f);
        RenderUtil.drawRoundedRect(poseStack, knobX - dragRatio, sliderY - 0.5f - dragRatio, 6.0f + dragRatio * 2.0f,
                6.0f + dragRatio * 2.0f, 2.0f + dragRatio / 2.0f + 1.0f, dragRatio,
                ColorUtil.withAlpha(-1, 0.36f * alpha));
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        float sliderY = this.y + this.getHeight() / 2.0f + (this.getHeight() / 2.0f - sliderHeight) / 2.0f;
        if (this.isHovered
                && CursorUtil.isInBounds((float) mouseX, (float) mouseY, this.x + 6.0f, sliderY, sliderWidth, 5.0f)) {
