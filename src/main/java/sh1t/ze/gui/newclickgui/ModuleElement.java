...
public class ModuleElement
    ...
    public void render(NewClickGui clickGui, GuiGraphics guiGraphics, PoseStack poseStack, int mouseX, int mouseY,
        ...
        this.isHovered = this.parentPanel.equals(NewClickGui.focusedPanel)
                && CursorUtil.isInBounds(mouseX, mouseY, this.posX, this.posY, 120.0f, this.totalHeight);
        RenderUtil.drawFilledRect(poseStack, this.posX + 1.0f, this.posY + 20.0f, 118.0f, this.totalHeight - 20.0f,
                ColorUtil.withAlpha(BG_COLOR, expandAmount * alpha));
        float hoverAmount = this.hoveredTimer.getValueF();
        if (hoverAmount > 0.0f) {
            RenderUtil.drawFilledRect(poseStack, this.posX + 0.5f, this.posY, 119.0f, 20.0f,
                    ColorUtil.withAlpha(-1, 0.1f * alpha * hoverAmount));
        }
        if (1.0f - (enabledAmount = this.enabledTimer.getValueF()) > 0.0f) {
            FontStore.AXIFORMA_REGULAR_16.drawStringCentered(poseStack, this.module.getName(), this.posX + 60.0f,
                    this.posY + (20.0f - FontStore.AXIFORMA_REGULAR_16.getFontHeight()) / 2.0f,
        ...
        if (enabledAmount > 0.0f) {
            ...
            RenderUtil.drawShadow(poseStack, this.posX + (120.0f - titleWidth) / 2.0f,
                    titleY + FontStore.AXIFORMA_BOLD_16.getFontHeight() / 4.0f, titleWidth,
                    FontStore.AXIFORMA_BOLD_16.getFontHeight() / 2.0f, 12,
                    ColorUtil.withAlpha(-13768502, alpha * enabledAmount * 0.36f));
            FontStore.AXIFORMA_BOLD_16.drawStringCentered(poseStack, this.module.getName(), this.posX + 60.0f, titleY,
        ...
        if (!this.module.getSettings().isEmpty()) {
            ...
            RenderHelper.pushRotateAround(poseStack, arrowX + titleY / 2.0f,
                    arrowY + FontStore.MATERIAL_20.getFontHeight() / 2.0f - 1.0f, 180.0f * expandAmount);
            FontStore.MATERIAL_20.drawString(poseStack, arrowIcon, arrowX, arrowY,
                    ColorUtil.withAlpha(-1, (0.8f - 0.3f * expandAmount) * alpha));
        ...
        if (this.isExpanded) {
            ...
            for (SettingElement settingElement : this.settingElements) {
                ...
                settingElement.render(clickGui, guiGraphics, poseStack, mouseX, mouseY, alpha * expandAmount,
                        partialTicks);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (CursorUtil.isInBounds((float) mouseX, (float) mouseY, this.posX, this.posY + 20.0f, 120.0f,
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        if (this.isHovered && CursorUtil.isInBounds((float) mouseX, (float) mouseY, this.posX, this.posY + 20.0f,
                120.0f, this.totalHeight - 20.0f)) {
            Iterator<SettingElement<?>> iterator = this.settingElements.iterator();
