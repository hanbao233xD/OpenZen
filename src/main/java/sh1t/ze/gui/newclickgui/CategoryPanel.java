...
public class CategoryPanel
    ...
    public void render(NewClickGui clickGui, GuiGraphics guiGraphics, PoseStack poseStack, int mouseX, int mouseY,
        ...
        RenderHelper.pushScaleAround(poseStack, this.posX + 60.0f, this.posY + this.panelHeight / 2.0f,
                0.4f + 0.6f * scaleAmount);
        float shadowSize = 12.0f;
        RenderUtil.drawRoundedRect(poseStack, this.posX - shadowSize, this.posY - shadowSize,
                120.0f + shadowSize * 2.0f, this.panelHeight + shadowSize * 2.0f, 6.0f + shadowSize / 2.0f, shadowSize,
                ColorUtil.fromARGB(0, 0, 0, (int) (80.0f * alpha * 1.0f)));
        RenderUtil.drawRoundedRect(poseStack, this.posX, this.posY, 120.0f, this.panelHeight, 6.0f,
                ColorUtil.withAlpha(BG_COLOR, alpha));
        ...
        StencilHelper.end();
        FontStore.AXIFORMA_EXTRABOLD_18.drawString(poseStack, this.category.displayName, this.posX + 8.0f,
                this.posY + (20.0f - FontStore.AXIFORMA_EXTRABOLD_18.getFontHeight()) / 2.0f + 3.0f,
        ...
        RenderUtil.drawRoundedRect(poseStack, this.posX, this.posY + 20.0f, 120.0f, this.panelHeight - 20.0f - 0.5f,
                6.0f, -1);
        StencilHelper.beginRead(true);
        for (ModuleElement moduleElement : this.moduleElements) {
        ...
        if (tooltipAmount > 0.0f) {
            ...
            RenderUtil.drawShadow(poseStack, mouseX + 5, mouseY + 5, tooltipWidth + 6.0f,
                    FontStore.AXIFORMA_REGULAR_16.getFontHeight() + 4.0f, 12,
                    ColorUtil.withAlpha(BG_COLOR, alpha * tooltipAmount * 0.66f));
            RenderUtil.drawRoundedRect(poseStack, mouseX + 5, mouseY + 5, tooltipWidth + 6.0f,
                    FontStore.AXIFORMA_REGULAR_16.getFontHeight() + 4.0f, 3.0f,
                    ColorUtil.withAlpha(BG_COLOR, alpha * tooltipAmount));
            FontStore.AXIFORMA_REGULAR_16.drawString(poseStack, this.tooltipText, mouseX + 5 + 3, mouseY + 5 + 1,
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.isHovered) {
            ...
            } else if (CursorUtil.isInBounds((float) mouseX, (float) mouseY, this.posX, this.posY + 20.0f, 120.0f,
                    this.panelHeight - 20.0f)) {
                for (ModuleElement moduleElement : this.moduleElements) {
