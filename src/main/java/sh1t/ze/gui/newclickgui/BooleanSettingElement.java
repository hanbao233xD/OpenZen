...
public class BooleanSettingElement
    ...
    public void render(NewClickGui clickGui, GuiGraphics guiGraphics, PoseStack poseStack, int mouseX, int mouseY,
        ...
        poseStack.pushPose();
        FontStore.AXIFORMA_REGULAR_14.drawString(poseStack, name, this.x + 6.0f,
                this.y + (this.getHeight() - FontStore.AXIFORMA_REGULAR_14.getFontHeight()) / 2.0f,
                ColorUtil.withAlpha(-1, alpha * 0.8f));
        float toggleAmount = this.toggleTimer.getValueF();
        if (toggleAmount > 0.0f) {
            RenderUtil.drawShadow(poseStack, this.x + 120.0f - 20.0f - 6.0f, this.y + (this.getHeight() - 10.0f) / 2.0f,
                    20.0f, 10.0f, 12, ColorUtil.withAlpha(-13768502, 0.36f * alpha * toggleAmount));
        }
        RenderUtil.drawRoundedRect(poseStack, this.x + 120.0f - 20.0f - 6.0f,
                this.y + (this.getHeight() - 10.0f) / 2.0f, 20.0f, 10.0f, 4.0f,
                ColorUtil.withAlpha(ColorUtil.fromRGB(60, 60, 60), alpha));
        if (toggleAmount > 0.0f) {
            RenderUtil.drawRoundedRect(poseStack, this.x + 120.0f - 20.0f - 6.0f,
                    this.y + (this.getHeight() - 10.0f) / 2.0f, 10.0f + 10.0f * toggleAmount, 10.0f, 4.0f,
                    ColorUtil.withAlpha(CategoryPanel.ACCENT_COLOR, alpha * toggleAmount));
        }
        RenderUtil.drawRoundedRect(poseStack, this.x + 120.0f - 20.0f - 6.0f + 10.0f * toggleAmount,
                this.y + (this.getHeight() - 10.0f) / 2.0f, 10.0f, 10.0f, 4.8f, ColorUtil.withAlpha(-1, alpha));
        poseStack.popPose();
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.isHovered && CursorUtil.isInBounds((float) mouseX, (float) mouseY, this.x + 120.0f - 20.0f - 6.0f,
                this.y + (this.getHeight() - 10.0f) / 2.0f, 20.0f, 10.0f)) {
