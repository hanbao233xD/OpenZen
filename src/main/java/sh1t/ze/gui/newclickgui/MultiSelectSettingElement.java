...
public class MultiSelectSettingElement
    ...
    @Override
    public void render(NewClickGui clickGui, GuiGraphics guiGraphics, PoseStack poseStack, int mouseX, int mouseY,
        ...
        this.isDropdownHovered = CursorUtil.isInBounds(mouseX, mouseY, this.x + 6.0f, dropdownY, dropdownWidth,
                itemHeight);
        this.hoverTimer.animate(this.isDropdownHovered ? 1.0 : 0.0, 0.22, Easings.EASE_OUT_POW2);
        this.hoverTimer.tick();
        this.visibilityTimer.animate(this.setting.getVisibility().displayable() ? 1.0 : 0.0, 0.2,
                Easings.EASE_OUT_POW2);
        ...
        FontStore.AXIFORMA_REGULAR_14.drawString(poseStack, this.setting.getName(), this.x + 6.0f, nameY,
        ...
        if (openAmount > 0.0f) {
            ...
            RenderUtil.drawRoundedRect(poseStack, this.x + 6.0f, dropdownY, dropdownWidth, dropdownHeight, 3.0f,
            ...
            for (String option : this.setting.getOptions()) {
                if (CursorUtil.isInBounds(mouseX, mouseY, this.x + 6.0f, itemY - hoverAmount, dropdownWidth,
                        itemHeight)) {
                ...
                if (dropdownY + dropdownHeight > itemY + FontStore.AXIFORMA_BOLD_13.getFontHeight()) {
                    FontStore.AXIFORMA_BOLD_13.drawStringCentered(poseStack, option, this.x + 60.0f, itemY,
                            ColorUtil.withAlpha(-1, alpha * 0.8f * openAmount));
                    if (this.setting.getValue().contains(option)) {
                        FontStore.MATERIAL_14.drawString(poseStack, "",
                                this.x + 6.0f + dropdownWidth - FontStore.MATERIAL_14.getStringWidth("") - 4.0f,
                                itemY - hoverAmount + (itemHeight - FontStore.MATERIAL_14.getFontHeight()) / 2.0f
                                        + 0.5f,
        ...
        this.highlightTimer.animate(this.hoveredOption == null || !this.isOpen ? 0.0 : 1.0, 0.18,
                Easings.EASE_OUT_POW2);
        this.highlightTimer.tick();
        this.highlightYTimer.tick();
        dropdownHeight = this.highlightTimer.getValueF();
        if (dropdownHeight > 0.0f) {
            RenderUtil.drawRoundedRect(poseStack, this.x + 6.0f, this.highlightYTimer.getValueF(), dropdownWidth,
                    itemHeight, 3.0f,
                    ColorUtil.withAlpha(ColorUtil.fromRGB(255, 255, 255), alpha * dropdownHeight * 1.0f * 0.1f));
        }
        hoverAmount = this.hoverTimer.getValueF();
        RenderUtil
                .drawRoundedRect(poseStack, this.x + 6.0f, dropdownY, dropdownWidth, itemHeight, 3.0f,
                        ColorUtil.withAlpha(
                                ColorUtil.fromRGB((int) (60.0f + 30.0f * hoverAmount),
                                        (int) (60.0f + 30.0f * hoverAmount), (int) (60.0f + 30.0f * hoverAmount)),
                                alpha));
        ...
        FontStore.AXIFORMA_BOLD_13.drawStringCentered(poseStack, selectedLabel, this.x + 60.0f,
                dropdownY + (itemHeight - FontStore.AXIFORMA_BOLD_13.getFontHeight()) / 2.0f,
                ColorUtil.withAlpha(-1, alpha * 0.8f));
        Object arrowIcon = String.valueOf('');
        FontStore.MATERIAL_20.drawString(poseStack, (String) arrowIcon,
                this.x + 6.0f + dropdownWidth - FontStore.MATERIAL_20.getStringWidth((String) arrowIcon) - 2.0f,
