...
public class PanelClickGui
    ...
    public void render(@Nonnull GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (effectiveAlpha > 0.005f) {
            ...
            this.moduleListPanel.render(guiGraphics, panelX, panelY, mouseX, mouseY, this.getSelectedCategory(),
                    this.currentScale, effectiveAlpha);
            this.settingsPanel.render(guiGraphics, panelX, panelY, mouseX, mouseY,
                    this.moduleListPanel.getHoveredModule(), this.currentScale, effectiveAlpha);
    ...
    private void drawPanelGlow(GuiGraphics guiGraphics, int panelX, int panelY, float partialTicks, float alpha) {
        int panelWidth = (int) (600.0f * this.currentScale);
        int panelHeight = (int) (400.0f * this.currentScale);
        TextGlow.drawBackground(guiGraphics.pose(), panelX, panelY, panelWidth, panelHeight, 12.0f * this.currentScale,
                alpha);
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        int panelX = centerX - panelWidth / 2;
        int panelY = centerY - panelHeight / 2;
        if (this.profileWidget.isPopupOpen()
                && this.profileWidget.onMouseClick(panelX, panelY, (int) mouseX, (int) mouseY, this.currentScale)) {
            return true;
        }
        ...
        if (button == 0 || button == 1 || button == 2) {
            if (button == 0
                    && this.categoryBar.onMouseClick(panelX, panelY, (int) mouseX, (int) mouseY, this.currentScale)) {
                if (this.searchActive) {
            ...
            if (!handled && this.moduleListPanel.onMouseClick(panelX, panelY, (int) mouseX, (int) mouseY,
                    this.getSelectedCategory(), button, this.currentScale)) {
                handled = true;
            }
            if (!handled
                    && this.profileWidget.onMouseClick(panelX, panelY, (int) mouseX, (int) mouseY, this.currentScale)) {
            ...
            if (!handled && (button == 0 || button == 1) && this.settingsPanel.onMouseClick(panelX, panelY,
                    (int) mouseX, (int) mouseY, button, this.currentScale)) {
                handled = true;
            }
        }
        if (!handled) {
    ...
    private void drawSearchBar(GuiGraphics guiGraphics, int panelX, int panelY, int mouseX, int mouseY, float alpha) {
        try {
            ...
            int searchX = panelX + (panelWidth - searchWidth) / 2;
            int searchY = panelY + panelHeight + (int) (15.0f * this.currentScale);
            TextGlow.drawBackground(guiGraphics.pose(), searchX, searchY, searchWidth, searchHeight,
                    8.0f * this.currentScale, alpha);
            Renderer.renderConsumer(drawContext -> {
                ...
                float iconY = searchY + searchHeight / 2.0f + iconFont.getMetrics().capHeight() / 2.0f
                        - 12.0f * this.currentScale + 10F;
                int iconColor = new Color(255, 255, 255, (int) (180.0f * alpha)).getRGB();
                TextGlow.drawGlowText(iconText, iconX, iconY, iconFont, 0xffdddddd, iconColor,
                        12.0f * this.currentScale);
                ...
                if (!this.searchActive && this.searchQuery.isEmpty()) {
                    ...
                    float placeholderY = searchY + searchHeight / 2.0f + placeholderFont.getMetrics().capHeight() / 2.0f
                            - 8.0f * this.currentScale + 5F;
                    ...
                    float queryY = searchY + searchHeight / 2.0f + queryFont.getMetrics().capHeight() / 2.0f
                            - 9.0f * this.currentScale + 6F;
                    int queryColor = new Color(255, 255, 255, (int) (120.0f * alpha)).getRGB();
                    TextGlow.drawGlowText(this.searchQuery, queryX, queryY, queryFont, -1, queryColor,
                            8.0f * this.currentScale);
                    if (this.searchFocused) {
                        ...
                        float cursorX = queryX + GlHelper.getStringWidth(this.searchQuery, queryFont)
                                + 2.0f * this.currentScale;
        ...
        try {
            Renderer.renderConsumer(drawContext -> {
                ...
                for (PanelClickGui.ToastEntry toast : this.toasts) {
                    ...
                    TextGlow.drawGlowText(toast.message, toastX, toastY + 6.0f * this.currentScale, toastFont,
                            textColor, glowColor, 8.0f * this.currentScale);
                }
            });
        } catch (Exception exception) {
            // empty catch block
