    ...
    public static void transformMethod(MethodNode methodNode) {
        ...
        for (AbstractInsnNode insn : instructions) {
            if (!(insn instanceof MethodInsnNode methodInsn))
                continue;
            if (!methodInsn.owner.equals("net/minecraft/network/chat/Component$Serializer"))
                continue;
            if (!methodInsn.name.equals(ReflectionUtil.getMappedMethodName(Component.Serializer.class, "fromJson",
                    "(Ljava/lang/String;)Lnet/minecraft/network/chat/MutableComponent;")))
                continue;
            if (!methodInsn.desc.equals("(Ljava/lang/String;)Lnet/minecraft/network/chat/MutableComponent;"))
                continue;
            targetCall = insn;
            break;
        }
