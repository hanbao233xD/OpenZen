...
public class GameRendererPatch {
    ...
    @Inject(method = "render", desc = "(FJZ)V", at = @At(value = At.Type.AFTER_INVOKE, method = "net/minecraft/client/gui/Gui/render", desc = "(Lnet/minecraft/client/gui/GuiGraphics;F)V"))
    public static void onRender(GameRenderer gameRenderer, float partialTick, long nanoTime, boolean renderLevel,
            CallbackInfo callbackInfo) {
        GuiGraphics graphics = new GuiGraphics(gameRenderer.getMinecraft(),
                gameRenderer.getMinecraft().renderBuffers().bufferSource());
    ...
    @WrapInvoke(method = "getProjectionMatrix", desc = "(D)Lorg/joml/Matrix4f;", target = "org/joml/Matrix4f/setPerspective", targetDesc = "(FFFF)Lorg/joml/Matrix4f;")
    public static Matrix4f onGetProjectionMatrix(GameRenderer gameRenderer, double fov,
            Invocation<GameRenderer, Matrix4f> original) throws Exception {
        if (!ZenClient.isReady() || AspectRatio.INSTANCE == null || !AspectRatio.INSTANCE.isEnabled()) {
            return original.call();
        }
        PoseStack poseStack = new PoseStack();
        poseStack.last().pose().identity();
        float zoom = (Float) ReflectionUtil.getStaticField(gameRenderer, "zoom",
                "net/minecraft/client/renderer/GameRenderer");
        float zoomX = (Float) ReflectionUtil.getStaticField(gameRenderer, "zoomX",
                "net/minecraft/client/renderer/GameRenderer");
        float zoomY = (Float) ReflectionUtil.getStaticField(gameRenderer, "zoomY",
                "net/minecraft/client/renderer/GameRenderer");
        if (zoom != 1.0f) {
    ...
    public static void onBobHurt(GameRenderer gameRenderer, PoseStack poseStack, float partial,
            CallbackInfo callbackInfo) {
