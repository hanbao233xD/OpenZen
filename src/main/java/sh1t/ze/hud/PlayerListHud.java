...
public class PlayerListHud
    ...
    public static final class PlayerEntry {
        ...
        public PlayerEntry(PlayerListHud outer, net.minecraft.world.entity.player.Player player,
                java.util.List<ItemStack> initialItems) {
            this.outer = outer;
            this.player = player;
        ...
        public void updateItems(java.util.List<ItemStack> items) {
            ...
            this.totalWidth = padding + headSize + gap + this.nameWidth + gap + items.size() * (itemSize + gap)
                    + padding;
    ...
    private boolean isCheatItem(ItemStack itemStack) {
        ...
        return ItemUtil.isOtherCheat(itemStack) || ItemUtil.isWeaponItem(itemStack)
                || ItemUtil.isEnchantedGoldenApple(itemStack) || ItemUtil.isEndCrystal(itemStack)
                || ItemUtil.isKBSlimeBall(itemStack) || ItemUtil.isKBStick(itemStack)
                || ItemUtil.getPunchLevel(itemStack) > 2 && itemStack.getItem() instanceof BowItem
                || ItemUtil.getPowerLevel(itemStack) > 3 && itemStack.getItem() instanceof BowItem;
    }
    ...
    public void onTick(TickEvent tickEvent) {
        ...
        for (PlayerListHud.PlayerEntry entry : this.playerEntryList) {
            if (!playerList.stream().noneMatch(p -> p.getUUID().equals(entry.player.getUUID())))
                continue;
            entry.startRemove();
        }
        for (Player currentPlayer : playerList) {
            List<ItemStack> playerItems;
            boolean alreadyTracked = this.playerEntryList.stream()
                    .anyMatch(e -> e.player.getUUID().equals(currentPlayer.getUUID()));
            if (alreadyTracked || (playerItems = this.getPlayerItems(currentPlayer)).isEmpty())
                continue;
        ...
        for (PlayerListHud.PlayerEntry entry : this.playerEntryList) {
            playerList.stream().filter(p -> p.getUUID().equals(entry.player.getUUID())).findFirst().ifPresent(p -> {
                if (p == mc.player) {
                    ...
                    this.getPlayerItems(p)
                            .forEach(itemStack -> entry.itemStacks.putIfAbsent(itemStack.getItem(), itemStack));
                    ...
                    for (ItemStack itemStack : entry.itemStacks.values()) {
                        if (entry.flaggedItems.contains(itemStack.getItem()))
                            continue;
    ...
    private List<ItemStack> getPlayerItems(Player player) {
        Stream<ItemStack> stream = player == mc.player
                ? Stream.concat(player.getInventory().items.stream(),
                        Stream.concat(player.getInventory().armor.stream(), player.getInventory().offhand.stream()))
                : Stream.concat(player.getInventory().armor.stream(),
    ...
    public void onPacket(PacketEvent packetEvent) {
        ...
        if (packet instanceof ClientboundEntityEventPacket
                && (entityEventPacket = (ClientboundEntityEventPacket) packet).getEventId() == 35
                && (entity = entityEventPacket.getEntity(mc.level)) instanceof Player) {
    ...
    private void updatePlayerItem(Player player, Item item) {
        this.playerEntryList.stream().filter(entry -> entry.player.getUUID().equals(player.getUUID())).findFirst()
                .ifPresent(entry -> {
    ...
    public void onGlRender(GlRenderEvent glRenderEvent, float x, float y) {
        ...
        boolean rightSide = isRightAligned = x + this.getWidth() / 2.0f > (float) mc.getWindow().getGuiScaledWidth()
                / 2.0f;
        ...
        if (!this.playerEntryList.isEmpty()) {
            ...
            for (PlayerListHud.PlayerEntry entry : this.playerEntryList) {
                ...
                float rowWidth = headSize + gap + entryY + gap
                        + (itemsWidth = (float) entry.items.size() * (itemSize + gap)) + padding * 2.0f;
        ...
        if (!this.rightAlignAnim.isDone()) {
            ...
            this.renderLegacy(guiGraphics.pose(), finalX, y, panelWidth, this.rightAlign, globalAlpha * prevSideFade,
                    entryHeight, spacing, cornerRadius, blurStrength, padding);
            this.renderLegacy(guiGraphics.pose(), finalX, y, panelWidth, this.wasRightAligned, globalAlpha * leftFade,
                    entryHeight, spacing, cornerRadius, blurStrength, padding);
        } else {
            this.renderLegacy(guiGraphics.pose(), finalX, y, panelWidth, this.wasRightAligned, globalAlpha, entryHeight,
                    spacing, cornerRadius, blurStrength, padding);
        }
        ...
        if (!this.rightAlignAnim.isDone()) {
            ...
            this.renderEntry(drawContext, finalX, y, panelWidth, this.rightAlign, globalAlpha * leftFade, iconText,
                    titleText, iconWidth, titleWidth, headerHeight, padding, gap, spacing, headSize);
            this.renderEntry(drawContext, finalX, y, panelWidth, this.wasRightAligned, globalAlpha * rightFade,
                    iconText, titleText, iconWidth, titleWidth, headerHeight, padding, gap, spacing, headSize);
        } else {
            this.renderEntry(drawContext, finalX, y, panelWidth, this.wasRightAligned, globalAlpha, iconText, titleText,
                    iconWidth, titleWidth, headerHeight, padding, gap, spacing, headSize);
        }
        if (!this.rightAlignAnim.isDone()) {
            leftFade = 1.0f - this.rightAlignAnim.getValueF();
            rightFade = this.rightAlignAnim.getValueF();
            this.renderEntryGui(guiGraphics, finalX, y, panelWidth, this.rightAlign, globalAlpha * leftFade, headSize,
                    itemSize, padding, gap);
            this.renderEntryGui(guiGraphics, finalX, y, panelWidth, this.wasRightAligned, globalAlpha * rightFade,
                    headSize, itemSize, padding, gap);
        } else {
            this.renderEntryGui(guiGraphics, finalX, y, panelWidth, this.wasRightAligned, globalAlpha, headSize,
                    itemSize, padding, gap);
        }
        guiGraphics.flush();
        if (this.wasRightAligned) {
    ...
    private void renderLegacy(PoseStack poseStack, float x, float y, float width, boolean rightAligned, float alpha,
            float entryHeight, float spacing, float cornerRadius, float blurStrength, float padding) {
        if (alpha <= 0.01f) {
            return;
        }
        for (PlayerListHud.PlayerEntry entry : this.playerEntryList) {
            ...
            float drawX = entryX;
            RenderUtil.drawBlurredRect(poseStack, drawX, entryY, entryWidth, entryHeight, spacing, cornerRadius,
                    entryAlpha, 0);
        }
    }

    private void renderEntry(DrawContext drawContext, float x, float y, float width, boolean rightAligned, float alpha,
            String iconText, String titleText, float iconWidth, float titleWidth, float headerHeight, float padding,
            float gap, float spacing, float headSize) {
        if (alpha <= 0.01f) {
            return;
        }
        float headerTextY = y + padding
                + (headerHeight - padding * 2.0f - (float) GlHelper.getFontAscent(this.nameFont)) / 2.0f + 1.0f;
        ...
        if (rightAligned) {
            ...
            GlHelper.drawTextWithShadow(titleText, x + padding + iconWidth + gap, headerTextY, this.nameFont,
                    textPaint);
        }
        for (PlayerListHud.PlayerEntry entry : this.playerEntryList) {
            float headX;
            float entryAlpha;
            float heightFactor = entry.heightAnim.getValueF();
            if (heightFactor <= 0.0f || (entryAlpha = alpha * heightFactor) <= 0.0f)
                continue;
            float baseX = rightAligned ? x + width - entry.widthAnim.getValueF() : x;
            float entryX = baseX + entry.slideAnim.getValueF();
            float entryY = entry.fadeAnim.getValueF();
            ...
            float nameY = entryY + padding
                    + (entryHeight - padding * 2.0f - (float) GlHelper.getFontAscent(this.headerFont)) / 2.0f;
            if (rightAligned) {
                ...
                if (entry.player instanceof AbstractClientPlayer) {
                    GlHelper.drawPlayerHeadRounded((AbstractClientPlayer) entry.player, headDrawX, contentY, headSize,
                            headSize, entryAlpha, spacing);
                }
            ...
            if (entry.player instanceof AbstractClientPlayer) {
                ...
                GlHelper.drawPlayerHeadRounded((AbstractClientPlayer) entry.player, headX, contentY, headSize, headSize,
                        entryAlpha, spacing);
            }
    ...
    private void renderEntryGui(GuiGraphics guiGraphics, float x, float y, float width, boolean rightAligned,
            float alpha, float headSize, float itemSize, float padding, float gap) {
        if (alpha <= 0.01f) {
            return;
        }
        for (PlayerListHud.PlayerEntry entry : this.playerEntryList) {
            float entryAlpha;
            float heightFactor = entry.heightAnim.getValueF();
            if (heightFactor <= 0.0f || (entryAlpha = alpha * heightFactor) <= 0.01f)
                continue;
            float baseX = rightAligned ? x + width - entry.widthAnim.getValueF() : x;
            float entryX = baseX + entry.slideAnim.getValueF();
            float entryY = entry.fadeAnim.getValueF();
