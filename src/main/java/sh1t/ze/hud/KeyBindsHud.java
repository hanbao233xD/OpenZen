...
public class KeyBindsHud
    ...
    public void onTick(TickEvent tickEvent) {
        ...
        for (KeyBindsHud.KeyBindRow row : this.rowList) {
            if (this.rowMap.containsKey(row.module))
                continue;
            row.startRemove();
        }
        boolean added = false;
    ...
    private void renderRows(DrawContext drawContext, float x, float y, float width, boolean rightAligned, float alpha,
            float headerHeight, float rowH) {
        float keyBoxLeft;
        float boxWidth;
        float rowAlpha;
        ...
        if (rightAligned) {
            ...
            GlHelper.drawTextWithShadow("Hotkeys", x + 5.0f + this.maxWidth + 3.0f, headerTextY, this.nameFont,
                    this.disabledPaint);
        }
        if (this.rowList.isEmpty()) {
            return;
        ...
        for (KeyBindsHud.KeyBindRow row : this.rowList) {
            ...
            if (row.enabled) {
                ...
                if (rightAligned) {
                    ...
                    GlHelper.drawTextWithShadow(row.displayName, keyBoxLeft + 5.0f, boxWidth, this.keyFont,
                            this.disabledPaint);
                }
                if (row.keyName == null) {
                ...
                float keyDrawY = rowHeightFactor + 5.0f
                        + (rowH - 10.0f - (float) GlHelper.getFontAscent(fontRenderer)) / 2.0f;
                GlHelper.drawTextWithShadow(row.keyName, keyTextX, keyDrawY, fontRenderer, this.disabledPaint);
                continue;
            }
            GlHelper.drawTextWithShadow(row.displayName, row.alphaValue + 5.0f, boxWidth, this.keyFont,
                    this.disabledPaint);
        }
    }
