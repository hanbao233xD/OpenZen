...
public class WatermarkHud
    ...
    public void render(DrawContext drawContext, float x, float y, float width, float height, float alpha) {
        ...
        try (Paint paint = new Paint()) {
            ...
            this.drawText(drawContext, paint, "|", (drawX += 12.0f) - 13.0f, centerY, subFont, subLineHeight, subColor,
                    shadow, true);
            ...
            this.drawText(drawContext, paint, this.line1Text, line1X, centerY - 2.0f, subFont, 0.0f, textColor, shadow,
                    false);
            this.drawText(drawContext, paint, this.line2Text, line2X, centerY + 7.0f, subFont, 0.0f, subColor, shadow,
                    false);
        }
    }

    private void drawText(DrawContext drawContext, Paint paint, String text, float x, float y,
            FontRenderer fontRenderer, float lineHeight, int color, int shadowColor, boolean centerVertical) {
        float drawY = y;
        if (centerVertical) {
    ...
    private String[] getServerInfo() {
        ...
        if (mc.getConnection() != null && mc.player != null
                && (playerInfo = mc.getConnection().getPlayerInfo(mc.player.getUUID())) != null) {
