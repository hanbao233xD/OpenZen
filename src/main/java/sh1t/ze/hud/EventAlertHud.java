...
import sh1t.ze.render.GradientFactory;
import sh1t.ze.render.Paint;
import sh1t.ze.render.Path;
import sh1t.ze.render.PathMeasure;
...
public class EventAlertHud
    ...
    private Optional<EventAlertHud.AlertEntry> findProjectileAlert() {
        ...
        return nearest.map(e -> new EventAlertHud.AlertEntry(e.getVelocity(), e.getZ(), Optional.of((float) e.getX()),
                "Find an ender pearl!", ""));
    }
    ...
    private Optional<EventAlertHud.AlertEntry> findEntityAlert() {
        ...
        StreamSupport.stream(mc.level.entitiesForRendering().spliterator(), false)
                .filter(entity -> entity instanceof LightningBolt && entity.isAlive())
                .forEach(entity -> this.activeAlerts.put(entity.position(), now));
        this.activeAlerts.entrySet().removeIf(entry -> now - entry.getValue() > 5000L);
        if (this.activeAlerts.isEmpty()) {
            return Optional.empty();
        }
        return this.activeAlerts.keySet().stream().filter(v -> mc.player.position().distanceToSqr(v) < 65536.0)
                .min(Comparator.comparingDouble(v -> mc.player.position().distanceToSqr(v)))
                .map(v -> new EventAlertHud.AlertEntry(v, mc.player.position().distanceTo(v), Optional.empty(),
                        "Found a lightning strike!", ""));
    }

    private Optional<EventAlertHud.AlertEntry> findBestAlert() {
        return java.util.stream.Stream.of(this.findProjectileAlert(), this.findEntityAlert())
                .filter(Optional::isPresent).map(Optional::get).min(Comparator.comparingDouble(a -> a.distance));
    }
    ...
    public void render(DrawContext drawContext, float x, float y, float width, float height, float alpha) {
        ...
        this.findBestAlert().ifPresent(alert -> {
            ...
            try (Paint paint = new Paint()) {
                ...
                float iconY = centerY - (titleFont.getMetrics().ascent() + titleFont.getMetrics().descent()) / 2.0f
                        - titleFont.getMetrics().descent();
                ...
                if (Math.sqrt(dx * dx + dz * dz) < 1.0) {
                    ...
                    float arrowY = centerY
                            - (subtitleFont.getMetrics().ascent() + subtitleFont.getMetrics().descent()) / 2.0f
                            - subtitleFont.getMetrics().descent();
