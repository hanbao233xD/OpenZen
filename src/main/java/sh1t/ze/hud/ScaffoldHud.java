...
public class ScaffoldHud
    ...
    public void render(DrawContext drawContext, float x, float y, float width, float height, float alpha) {
        ...
        try (Paint paint = new Paint()) {
            ...
            drawContext.drawRoundedRect(RoundedRectangle.ofXYWHR(barX, barY, barWidth, barHeight, barHeight / 2.0f),
                    paint);
            if (this.progressAnim.getValue() > 0.0f) {
                paint.setColor(this.colorWithAlpha(new Color(153, 0, 255).getRGB(), alpha));
                drawContext.drawRoundedRect(RoundedRectangle.ofXYWHR(barX, barY,
                        barWidth * this.progressAnim.getValue(), barHeight, barHeight / 2.0f), paint);
            }
        }
        ...
        try (Paint paint = new Paint()) {
            ...
            drawContext.drawString(countText, countX, centerY - blockCountFont.getMetrics().capHeight() / 2.0f + 2.0f,
                    blockCountFont, paint);
            paint.setColor(this.colorWithAlpha(Color.GRAY.getRGB(), alpha));
            drawContext.drawString(speedText, speedX, centerY + speedFont.getMetrics().capHeight() / 2.0f + 8.0f,
                    speedFont, paint);
        }
    }
