...
public class TabListHud
    ...
    @Override
    public void renderGui(GuiGraphics guiGraphics, PoseStack poseStack, float x, float y, float width, float height,
            float alpha) {
        float rowWidth;
        float maxRowWidth;
        ...
        float headerHeight = header != null && !header.getString().isEmpty()
                ? (float) header.getString().split("\n").length * 11.0f
                : 20.0f;
        ...
        try (Paint paint = new Paint()) {
            ...
            if (header != null && !header.getString().isEmpty()) {
                ...
                for (String line : header.getString().split("\n")) {
                    ...
                    this.drawFormattedComponent(drawContext, Component.literal(line), maxRowWidth,
                            lineY + titleFont.getMetrics().ascent() / 2.0f + 9.0f, titleFont, paint, alpha);
                    lineY += 11.0f;
                }
            } else {
                headerHeight = 20.0f;
                ...
                GlHelper.drawTextShadowLegacy(title, titleX, y + 10.0f + headerFont.getMetrics().ascent() + 15.0f,
                        headerFont, -1);
            }
            ...
            for (i = 0; i < players.size(); ++i) {
                ...
                float nameMaxWidth = columnCount > 1 ? 128.0f - pingWidth - 5.0f
                        : maxRowWidth - 8.0f - 4.0f - pingWidth - 5.0f;
                ...
                drawContext.drawString(displayName, entryX + 8.0f + 4.0f + 0.5f,
                        rowWidth + nameFont.getMetrics().ascent() / 2.0f + 11.0f + 0.5f, nameFont,
                        paint.setColor(ColorUtil.fromARGB(0, 0, 0, (int) (alpha * 0.65f * 255.0f))));
                drawContext.drawString(displayName, entryX + 8.0f + 4.0f,
                        rowWidth + nameFont.getMetrics().ascent() / 2.0f + 11.0f, nameFont, paint.setColor(-1));
                ...
                drawContext.drawString(ping, pingX, rowWidth + nameFont.getMetrics().ascent() / 2.0f + 11.0f, nameFont,
                        paint);
            }
            if (footer != null && !footer.getString().isEmpty()) {
                ...
                for (String line : footer.getString().split("\n")) {
                    ...
                    this.drawFormattedComponent(drawContext, Component.literal(line), centerX, footerY, titleFont,
                            paint, alpha);
                    footerY += 11.0f;
                }
            }
    ...
    private void drawFormattedComponent(DrawContext drawContext, Component component, float centerX, float startY,
            FontRenderer fontRenderer, Paint paint, float alpha) {
        float lineY = startY;
    ...
    public IHudElement.Size getHudAlignment() {
        ...
        headerHeight = header != null && !header.getString().isEmpty()
                ? (float) header.getString().split("\n").length * 11.0f
                : 20.0f;
