...
public class NeverloseWatermark {
    ...
    public void onRender2D(Render2DEvent render2DEvent) {
        ...
        currentX = this.renderSectionLegacy(render2DEvent.poseStack(), currentX, y, this.getServerName(),
                this.smallFont, "\uE8A6", cornerRadius, blurRadius, gap);
        currentX = this.renderSectionLegacy(render2DEvent.poseStack(), currentX, y, this.getPingText(),
                this.regularFont, "\uE8B8", cornerRadius, blurRadius, gap);
        currentX = this.renderSectionLegacy(render2DEvent.poseStack(), currentX, y, this.getFpsText(), this.regularFont,
                "\uEBCA", cornerRadius, blurRadius, gap);
        currentX = this.renderSectionLegacy(render2DEvent.poseStack(), currentX, y, this.getTimeText(),
                this.regularFont, "\uEB66", cornerRadius, blurRadius, gap);
        currentX = this.renderSectionLegacy(render2DEvent.poseStack(), currentX, y, this.getCpsText(), this.regularFont,
                "\uE0C8", cornerRadius, blurRadius, gap);
        this.renderSectionLegacy(render2DEvent.poseStack(), currentX, y, this.getCoordText(), this.regularFont,
                "\uE192", cornerRadius, blurRadius, gap);
    }

    private float renderSectionLegacy(PoseStack poseStack, float x, float y, String text, FontRenderer fontRenderer,
            String icon, float cornerRadius, float blurRadius, float gap) {
    ...
    public void onGlRender(GlRenderEvent glRenderEvent) {
        ...
        float gap = 6.0f;
        float cornerRadius = 4.5f;
        currentX = this.renderSection(glRenderEvent.drawContext(), currentX, y, "ZEN", this.boldFont, cornerRadius,
                gap);
        currentX = this.renderSectionWithSub(glRenderEvent.drawContext(), currentX, y, this.getServerName(),
                this.smallFont, "", cornerRadius, gap);
        currentX = this.renderSectionWithSub(glRenderEvent.drawContext(), currentX, y, this.getPingText(),
                this.regularFont, "", cornerRadius, gap);
        currentX = this.renderSectionWithSub(glRenderEvent.drawContext(), currentX, y, this.getFpsText(),
                this.regularFont, "", cornerRadius, gap);
        currentX = this.renderSectionWithSub(glRenderEvent.drawContext(), currentX, y, this.getTimeText(),
                this.regularFont, "", cornerRadius, gap);
        currentX = this.renderSectionWithSub(glRenderEvent.drawContext(), currentX, y, this.getCpsText(),
                this.regularFont, "", cornerRadius, gap);
        this.renderSectionWithSub(glRenderEvent.drawContext(), currentX, y, this.getCoordText(), this.regularFont, "",
                cornerRadius, gap);
    }

    private float renderSection(DrawContext drawContext, float x, float y, String text, FontRenderer fontRenderer,
            float cornerRadius, float gap) {
        float padding = 8.0f;
    ...
    private float renderSectionWithSub(DrawContext drawContext, float x, float y, String text,
            FontRenderer fontRenderer, String icon, float cornerRadius, float gap) {
        float padding = 8.0f;
        ...
        GlHelper.drawTextShadowLegacy(text, x + padding + iconWidth + 5.0f - 3.0f, textY, fontRenderer,
    ...
    private float measureTextWithSub(String text, FontRenderer fontRenderer, String icon) {
        float padding = 8.0f;
        return padding + GlHelper.getStringWidth(icon, this.tinyFont) + 5.0f
                + GlHelper.getStringWidth(text, fontRenderer) + padding - 4.0f;
    }
