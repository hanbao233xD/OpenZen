...
public class LieDetector
    ...
    public void onGlRender(GlRenderEvent glRenderEvent, float x, float y) {
        ...
        this.needleAnim.animate(
                KillAura.aimingTarget == null ? 0.0 : (double) ((LivingEntity) KillAura.aimingTarget).hurtTime, 0.32,
                Easings.BACK_OUT);
        this.needleAnim.tick();
        PoseStack poseStack = glRenderEvent.poseStack();
        RenderHelper.resetShaderColor();
        RenderUtil.drawTexture(this.panelTexture.getId(), poseStack, x, y, this.panelDisplayWidth,
                this.panelDisplayHeight, 1.0f, -1);
        RenderUtil.drawTexture(this.panelTexture.getId(), poseStack, x, y, this.panelDisplayWidth,
                this.panelDisplayHeight, 1.0f, -1);
        ...
        RenderHelper.pushRotateAround(poseStack, pointerX + 9.0f, pointerY + 8.5f,
                this.needleAnim.getValueF() * -18.0f);
        RenderUtil.drawTexture(this.pointerTexture.getId(), poseStack, pointerX, pointerY, this.panelWidth,
                this.pointerHeight, 1.0f, -1);
        RenderUtil.drawTexture(this.pointerTexture.getId(), poseStack, pointerX, pointerY, this.panelWidth,
                this.pointerHeight, 1.0f, -1);
