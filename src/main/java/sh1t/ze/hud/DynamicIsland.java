...
public class DynamicIsland {
    ...
    final List<IHudElement> elements = Arrays.asList(new TabListHud(), new ScaffoldHud(), new EventAlertHud(),
            new AutoPlayHud(), new WatermarkHud());
    private final DynamicIsland.ActiveElementSelector activeElementSelector = new DynamicIsland.ActiveElementSelector(
            this);
    ...
    public void onRender2D(Render2DEvent render2DEvent) {
        ...
        float activeY = this.activeElement.getHudSize() == IHudElement.Alignment.CENTER
                ? anchorCenterY - islandHeight / 2.0f
                : topMargin;
        if (this.outgoingElement != null && progress < 1.0f) {
            outgoingY = this.outgoingElement.getHudSize() == IHudElement.Alignment.CENTER
                    ? anchorCenterY - islandHeight / 2.0f
                    : topMargin;
            ...
            Renderer.renderConsumer((drawContext -> {
                try (Paint paint = new Paint()) {
                    ...
                    drawContext.drawRoundedRect(
                            RoundedRectangle.ofXYWHR(islandX, finalY, islandWidth, islandHeight, finalCornerRadius),
                            paint);
                }
                drawContext.save();
                drawContext.clipRoundedRect(
                        RoundedRectangle.ofXYWHR(islandX, finalY, islandWidth, islandHeight, finalCornerRadius), true);
        ...
        this.activeElement.renderGui(render2DEvent.guiGraphics(), render2DEvent.poseStack(), islandX, islandY,
                islandWidth, islandHeight, progress);
