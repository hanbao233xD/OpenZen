...
public class DrawContext {
    ...
    public void drawRectXYWH(float x, float y, float width, float height, Paint paint) {
        ...
        if (paint.getGradCoords() != null) {
            ...
            boolean isVertical = vertical = Math.abs(gradCoords.y2 - gradCoords.y1) >= Math
                    .abs(gradCoords.x2 - gradCoords.x1);
            if (vertical) {
                bufferBuilder.vertex(pose, x, y, 0.0f).color(color1[0], color1[1], color1[2], color1[3]).endVertex();
                bufferBuilder.vertex(pose, x, y + height, 0.0f).color(color2[0], color2[1], color2[2], color2[3])
                        .endVertex();
                bufferBuilder.vertex(pose, x + width, y + height, 0.0f)
                        .color(color2[0], color2[1], color2[2], color2[3]).endVertex();
                bufferBuilder.vertex(pose, x + width, y, 0.0f).color(color1[0], color1[1], color1[2], color1[3])
                        .endVertex();
            } else {
                bufferBuilder.vertex(pose, x, y, 0.0f).color(color1[0], color1[1], color1[2], color1[3]).endVertex();
                bufferBuilder.vertex(pose, x, y + height, 0.0f).color(color1[0], color1[1], color1[2], color1[3])
                        .endVertex();
                bufferBuilder.vertex(pose, x + width, y + height, 0.0f)
                        .color(color2[0], color2[1], color2[2], color2[3]).endVertex();
                bufferBuilder.vertex(pose, x + width, y, 0.0f).color(color2[0], color2[1], color2[2], color2[3])
                        .endVertex();
            }
        } else {
            ...
            bufferBuilder.vertex(pose, x + width, y + height, 0.0f).color(color[0], color[1], color[2], color[3])
                    .endVertex();
    ...
    public void drawRoundedRect(RoundedRectangle roundedRectangle, Paint paint) {
        ...
        float strokeWidth = paint.getCapStyle() == Paint.StrokeCap.STROKE ? Math.max(0.5f, paint.getStrokeWidth())
                : 0.0f;
        ROUNDED_RECT_SHADER.draw(this.poseStack.last().pose(), roundedRectangle.x1, roundedRectangle.y1,
                roundedRectangle.x2, roundedRectangle.y2, roundedRectangle.topLeftRadius,
                roundedRectangle.topRightRadius, roundedRectangle.bottomRightRadius, roundedRectangle.bottomLeftRadius,
                color1, color2, hasGradient, strokeWidth);
    }

    private void drawCornerArc(BufferBuilder bufferBuilder, Matrix4f pose, float centerX, float centerY, float radius,
            float startAngle, float sweepAngle, int segments, Paint paint, RoundedRectangle roundedRectangle) {
        int color = paint.getColor();
        Paint.GradientCoords gradCoords = paint.getGradCoords();
        if (radius <= 0.0f) {
            float[] colorFloats = DrawContext.interpolateGradientColor(gradCoords, centerX, centerY, roundedRectangle,
                    color);
            bufferBuilder.vertex(pose, centerX, centerY, 0.0f)
                    .color(colorFloats[0], colorFloats[1], colorFloats[2], colorFloats[3]).endVertex();
            return;
        }
        for (int i = 0; i <= segments; ++i) {
            ...
            bufferBuilder.vertex(pose, vx, vy, 0.0f)
                    .color(colorFloats[0], colorFloats[1], colorFloats[2], colorFloats[3]).endVertex();
        }
    }

    private static float[] interpolateGradientColor(Paint.GradientCoords gradCoords, float vx, float vy,
            RoundedRectangle roundedRectangle, int fallbackColor) {
        if (gradCoords == null) {
        ...
        float[] color1 = DrawContext.colorToFloats(gradCoords.color1);
        float[] color2 = DrawContext.colorToFloats(gradCoords.color2);
        return new float[] { color1[0] + (color2[0] - color1[0]) * t, color1[1] + (color2[1] - color1[1]) * t,
                color1[2] + (color2[2] - color1[2]) * t, color1[3] + (color2[3] - color1[3]) * t };
    }
    ...
    private void drawRoundedRectStroke(RoundedRectangle roundedRectangle, Paint paint) {
        ...
        this.drawRectStroke(roundedRectangle.x1, roundedRectangle.y1, roundedRectangle.getWidth(),
                roundedRectangle.getHeight(), paint);
    }
    ...
    public void drawLine(float x1, float y1, float x2, float y2, Paint paint) {
        ...
        BufferBuilder bufferBuilder = tesselator.getBuilder();
        bufferBuilder.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
        bufferBuilder.vertex(pose, x1 + normalX, y1 + normalY, 0.0f).color(color[0], color[1], color[2], color[3])
                .endVertex();
        bufferBuilder.vertex(pose, x1 - normalX, y1 - normalY, 0.0f).color(color[0], color[1], color[2], color[3])
                .endVertex();
        bufferBuilder.vertex(pose, x2 - normalX, y2 - normalY, 0.0f).color(color[0], color[1], color[2], color[3])
                .endVertex();
        bufferBuilder.vertex(pose, x2 + normalX, y2 + normalY, 0.0f).color(color[0], color[1], color[2], color[3])
                .endVertex();
        tesselator.end();
    ...
    public void drawArc(float x1, float y1, float x2, float y2, float startAngle, float sweepAngle, boolean unused,
            Paint paint) {
        this.setupColorShader();
        Matrix4f pose = this.poseStack.last().pose();
        ...
        if (paint.getCapStyle() == Paint.StrokeCap.STROKE) {
            ...
            bufferBuilder.vertex(pose, centerX, centerY, 0.0f).color(color[0], color[1], color[2], color[3])
                    .endVertex();
            for (int i = 0; i <= segments; ++i) {
                float t = (float) i / (float) segments;
                double angle = Math.toRadians(startAngle + sweepAngle * t);
        ...
        if (paint.getCapStyle() == Paint.StrokeCap.STROKE || paint.getCapStyle() == Paint.StrokeCap.STROKE_AND_FILL) {
            for (Path.PathSegment path$PathSegment : path.getSegments()) {
                switch (path$PathSegment.type) {
                    ...
                    case LINE_TO: {
                        if (hasCurrent) {
                            this.drawStrokedLineSegment(pose, currX, currY, path$PathSegment.coords[0],
                                    path$PathSegment.coords[1], strokeWidth, color, strokeState);
                        }
                        currX = path$PathSegment.coords[0];
                        currY = path$PathSegment.coords[1];
                        hasCurrent = true;
                    ...
                    case QUAD_TO: {
                        if (hasCurrent) {
                            this.drawStrokedCubicBezier(pose, currX, currY,
                                    (currX + 2.0f * path$PathSegment.coords[0]) / 3.0f,
                                    (currY + 2.0f * path$PathSegment.coords[1]) / 3.0f,
                                    (path$PathSegment.coords[2] + 2.0f * path$PathSegment.coords[0]) / 3.0f,
                                    (path$PathSegment.coords[3] + 2.0f * path$PathSegment.coords[1]) / 3.0f,
                                    path$PathSegment.coords[2], path$PathSegment.coords[3], strokeWidth, color,
                                    strokeState);
                        }
                    ...
                    case CUBIC_TO: {
                        if (hasCurrent) {
                            this.drawStrokedCubicBezier(pose, currX, currY, path$PathSegment.coords[0],
                                    path$PathSegment.coords[1], path$PathSegment.coords[2], path$PathSegment.coords[3],
                                    path$PathSegment.coords[4], path$PathSegment.coords[5], strokeWidth, color,
                                    strokeState);
                        }
                    ...
                    case CLOSE: {
                        if (hasCurrent) {
                            this.drawStrokedLineSegment(pose, currX, currY, startX, startY, strokeWidth, color,
                                    strokeState);
                        }
                        currX = startX;
                        currY = startY;
                    ...
                    case RECT: {
                        this.drawRectStroke(path$PathSegment.rect.getX(), path$PathSegment.rect.getY(),
                                path$PathSegment.rect.getWidth(), path$PathSegment.rect.getHeight(), paint);
                    }
                }
            }
        }
    ...
    private void fillPolygon(List<float[]> polygon, Paint paint) {
        ...
        for (float[] vertex : polygon) {
            bufferBuilder.vertex(pose, vertex[0], vertex[1], 0.0f).color(color[0], color[1], color[2], color[3])
                    .endVertex();
        }
        tesselator.end();
    }

    private void drawStrokedCubicBezier(Matrix4f pose, float p0x, float p0y, float p1x, float p1y, float p2x, float p2y,
            float p3x, float p3y, float strokeWidth, float[] color, DrawContext.StrokeState strokeState) {
        int steps = 24;
        float prevX = p0x;
        float prevY = p0y;
        for (int i = 1; i <= steps; ++i) {
            ...
            float x = oneMinusT * oneMinusT * oneMinusT * p0x + 3.0f * oneMinusT * oneMinusT * t * p1x
                    + 3.0f * oneMinusT * t * t * p2x + t * t * t * p3x;
            float y = oneMinusT * oneMinusT * oneMinusT * p0y + 3.0f * oneMinusT * oneMinusT * t * p1y
                    + 3.0f * oneMinusT * t * t * p2y + t * t * t * p3y;
    ...
    private void drawStrokedLineSegment(Matrix4f pose, float x1, float y1, float x2, float y2, float strokeWidth,
            float[] color, DrawContext.StrokeState strokeState) {
    ...
    private void drawLineSegment(Matrix4f pose, float x1, float y1, float x2, float y2, float strokeWidth,
            float[] color) {
        float dx = x2 - x1;
        float dy = y2 - y1;
        float length = (float) Math.hypot(dx, dy);
    ...
    public void drawTexture(Texture texture, Rectangle srcRect, Rectangle dstRect, Paint paint) {
        ...
        bufferBuilder.vertex(pose, dstX, dstY, 0.0f).uv(u1, v1).color(color[0], color[1], color[2], color[3])
                .endVertex();
        bufferBuilder.vertex(pose, dstX, dstY + dstHeight, 0.0f).uv(u1, v2)
                .color(color[0], color[1], color[2], color[3]).endVertex();
        bufferBuilder.vertex(pose, dstX + dstWidth, dstY + dstHeight, 0.0f).uv(u2, v2)
                .color(color[0], color[1], color[2], color[3]).endVertex();
        bufferBuilder.vertex(pose, dstX + dstWidth, dstY, 0.0f).uv(u2, v1).color(color[0], color[1], color[2], color[3])
                .endVertex();
        tesselator.end();
    }

    public void drawBlurredRoundedRect(RoundedRectangle roundedRectangle, float offsetX, float offsetY,
            float blurRadius, float spread, int color) {
        ...
        RoundedRectangle expanded = RoundedRectangle.ofXYWHRadii(x, y, width, height,
                new float[] { tlRadius, trRadius, brRadius, blRadius });
    ...
    void clearClipStack() {
        while (!this.clipStack.isEmpty()) {
            ...
            if (!useScissor || this.guiGraphics == null)
                continue;
            this.guiGraphics.disableScissor();
        }
    }
    ...
    static float[] colorToFloats(int color) {
        return new float[] { (float) (color >> 16 & 0xFF) / 255.0f, (float) (color >> 8 & 0xFF) / 255.0f,
