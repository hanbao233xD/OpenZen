...
public final class RoundedRectShader {
    ...
    public void init() {
        ...
        int vertexShader = RoundedRectShader.compileShader(35633,
                "#version 150\nin vec2 Position;\nin vec2 LocalPos;\nin vec2 UV;\nuniform mat4 ModelViewMat;\nuniform mat4 ProjMat;\nout vec2 localPos;\nout vec2 uvCoord;\nvoid main() {\n    gl_Position = ProjMat * ModelViewMat * vec4(Position, 0.0, 1.0);\n    localPos = LocalPos;\n    uvCoord = UV;\n}\n");
        int fragmentShader = RoundedRectShader.compileShader(35632,
    ...
    public void draw(Matrix4f pose, float x1, float y1, float x2, float y2, float tlRadius, float trRadius,
            float brRadius, float blRadius, int color1, int color2, boolean useGradient, float strokeWidth) {
        this.drawInternal(pose, x1, y1, x2, y2, tlRadius, trRadius, brRadius, blRadius, color1, color2, useGradient,
                strokeWidth, -1, 0.0f, 0.0f, 1.0f, 1.0f);
    }

    public void drawTextured(Matrix4f pose, float x1, float y1, float x2, float y2, float tlRadius, float trRadius,
            float brRadius, float blRadius, int color, int textureId, float u1, float v1, float u2, float v2) {
        this.drawInternal(pose, x1, y1, x2, y2, tlRadius, trRadius, brRadius, blRadius, color, color, false, 0.0f,
                textureId, u1, v1, u2, v2);
    }

    private void drawInternal(Matrix4f pose, float x1, float y1, float x2, float y2, float tlRadius, float trRadius,
            float brRadius, float blRadius, int color1, int color2, boolean useGradient, float strokeWidth,
            int textureId, float u1, float v1, float u2, float v2) {
        int i;
        this.init();
        ...
        float[][] cornerOffsets = new float[][] { { -expandedHalfW, -expandedHalfH }, { expandedHalfW, -expandedHalfH },
                { expandedHalfW, expandedHalfH }, { -expandedHalfW, -expandedHalfH }, { expandedHalfW, expandedHalfH },
                { -expandedHalfW, expandedHalfH } };
        for (i = 0; i < 6; ++i) {
        ...
        GL20.glUniform4f(this.uColor1, (float) (color1 >> 16 & 0xFF) / 255.0f, (float) (color1 >> 8 & 0xFF) / 255.0f,
                (float) (color1 & 0xFF) / 255.0f, (float) (color1 >>> 24 & 0xFF) / 255.0f);
        GL20.glUniform4f(this.uColor2, (float) (color2 >> 16 & 0xFF) / 255.0f, (float) (color2 >> 8 & 0xFF) / 255.0f,
                (float) (color2 & 0xFF) / 255.0f, (float) (color2 >>> 24 & 0xFF) / 255.0f);
