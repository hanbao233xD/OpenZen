...
public final class BlurRenderer {
    ...
    public static void renderBlur(DrawContext drawContext, float x, float y, float width, float height, float radius,
            Runnable drawCallback) {
        ...
        Matrix4f ortho = new Matrix4f().setOrtho(paddedX, paddedX + paddedWidth, paddedY + paddedHeight, paddedY,
                1000.0f, 3000.0f);
        fboA.bind();
        fboA.clear();
    ...
    private static void blitTexture(DrawContext drawContext, int textureId, float x, float y, float width,
            float height) {
        ...
        bufferBuilder.vertex(pose, x + width, y + height, 0.0f).uv(1.0f, 0.0f).color(1.0f, 1.0f, 1.0f, 1.0f)
                .endVertex();
