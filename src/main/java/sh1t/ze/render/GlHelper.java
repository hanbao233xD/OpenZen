...
public final class GlHelper {
    ...
    public static void drawPlayerHead(AbstractClientPlayer player, float x, float y, float width, float height,
            float alpha) {
    ...
    public static void drawPlayerHeadRounded(AbstractClientPlayer player, float x, float y, float width, float height,
        ...
        roundedRectShader.drawTextured(pose, x, y, x + width, y + height, radius, radius, radius, radius, packedColor,
                glId, baseU1, baseV1, baseU2, baseV2);
        roundedRectShader.drawTextured(pose, x, y, x + width, y + height, radius, radius, radius, radius, packedColor,
                glId, hatU1, hatV1, hatU2, hatV2);
    ...
    public static float drawTextFormatted(String text, float x, float y, FontRenderer fontRenderer, Paint paint,
            boolean keepColor) {
        ...
        for (int i = 0; i < parts.length; ++i) {
            ...
            String part = parts[i];
            if (i > 0 && !part.isEmpty()
                    && (chatFormatting = ChatFormatting.getByCode(code = part.charAt(0))) != null) {
    ...
    public static float drawTextWithShadow(String text, float x, float y, FontRenderer fontRenderer, Paint paint) {
        int color = paint.getColor();
        GlHelper.drawTextFormatted(text, x + 0.5f, y + 0.5f, fontRenderer, paint.setColor(
                ColorUtil.fromARGB(0, 0, 0, (int) ((float) ColorUtil.getAlpha(paint.getColor()) * 0.65f * 255.0f))),
                true);
    ...
    public static float drawTextShadowLegacy(String text, float x, float y, FontRenderer fontRenderer, int color) {
        ...
        GlHelper.drawTextFormatted(text, x + 0.5f, y + 0.5f, fontRenderer,
                GlHelper.toPaint(ColorUtil.fromARGB(0, 0, 0, (int) (alpha * 0.65f * 255.0f))), true);
        return GlHelper.drawTextFormatted(text, x, y, fontRenderer, paint, false);
    }

    public static float drawTextBlurred(String text, float x, float y, FontRenderer fontRenderer, int color,
            int blurColor, float radius) {
        if (text == null || text.isEmpty()) {
            return x;
        }
        DrawContext drawContext = GlHelper.getCanvas();
        ...
        drawContext.drawBlur(x, y - height, width, height * 2.0f, halfBlur,
                () -> drawContext.drawString(text, x, y, fontRenderer, GlHelper.toPaint(blurColor)));
        return GlHelper.drawText(text, x, y, fontRenderer, color);
    }

    public static float drawTextCentered(float centerX, float centerY, String text, FontRenderer fontRenderer,
            Paint paint) {
        float width = fontRenderer.getWidth(text);
        float height = fontRenderer.getFont() != null ? fontRenderer.getFont().getFontHeight() : fontRenderer.getSize();
        return GlHelper.drawTextFormatted(text, centerX - width / 2.0f, centerY - height / 2.0f, fontRenderer, paint,
                false);
    }
    ...
    public static float getStringWidth(String text, FontRenderer fontRenderer) {
        ...
        return stringWidthCache.computeIfAbsent(fontRenderer, key -> new HashMap<>()).computeIfAbsent(text,
                key -> key != null ? fontRenderer.getWidth(key.replaceAll("§.", "")) : 0.0f);
    }
    ...
    public static void drawRoundedRectCorners(float x, float y, float width, float height, float tlRadius,
            float trRadius, float brRadius, float blRadius, Paint paint) {
    ...
    public static void drawGradientRoundedRect(float x, float y, float width, float height, float radius, Color color,
            Color color2) {
        Paint.GradientCoords gradCoords = new Paint.GradientCoords(x, y, x, y + height, GlHelper.colorToInt(color),
                GlHelper.colorToInt(color2));
        Paint paint = new Paint().setGradCoords(gradCoords);
        GlHelper.getCanvas().drawRoundedRect(RoundedRectangle.ofXYWHR(x, y, width, height, radius), paint);
    }

    public static void drawBlurredRoundedRectColor(float x, float y, float width, float height, float radius,
            Color color, float blurRadius, float offsetX, float offsetY) {
        GlHelper.getCanvas().drawBlurredRoundedRect(RoundedRectangle.ofXYWHR(x, y, width, height, radius), offsetX,
                offsetY, blurRadius, 1.0f, color.getRGB());
    }

    public static void drawShadowRoundedRect(float x, float y, float width, float height, float radius, Color color) {
        GlHelper.getCanvas().drawBlurredRoundedRect(RoundedRectangle.ofXYWHR(x, y, width, height, radius), 0.0f, 0.0f,
                10.0f, 1.0f, color.getRGB());
    }

    public static void drawRoundedRectCornersColor(float x, float y, float width, float height, float tlRadius,
            float trRadius, float brRadius, float blRadius, Color color) {
        float[] radii = new float[] { tlRadius, tlRadius, trRadius, trRadius, brRadius, brRadius, blRadius, blRadius };
        GlHelper.getCanvas().drawBlurredRoundedRect(RoundedRectangle.ofXYWHRadii(x, y, width, height, radii), 0.0f,
                0.0f, 10.0f, 1.0f, color.getRGB());
    }
