...
public class StencilHelper {
    ...
    private static final Supplier<TextureTarget> maskTarget = Suppliers
            .memoize(() -> new TextureTarget(360, 360, false, Minecraft.ON_OSX));
    private static final Supplier<TextureTarget> contentTarget = Suppliers
            .memoize(() -> new TextureTarget(360, 360, false, Minecraft.ON_OSX));
    ...
    public static void applyStencil(PoseStack poseStack, Runnable drawMask, Runnable drawContent, float opacity) {
        ...
        if (mask.width != StencilHelper.mainRenderTarget.width
                || mask.height != StencilHelper.mainRenderTarget.height) {
        ...
        if (content.width != StencilHelper.mainRenderTarget.width
                || content.height != StencilHelper.mainRenderTarget.height) {
            content.resize(StencilHelper.mainRenderTarget.width, StencilHelper.mainRenderTarget.height,
                    Minecraft.ON_OSX);
        }
        ...
        bufferBuilder.vertex(pose, 0.0f, (float) ClientBase.mc.getWindow().getGuiScaledHeight(), 0.0f).uv(0.0f, 0.0f)
                .endVertex();
        bufferBuilder.vertex(pose, (float) ClientBase.mc.getWindow().getGuiScaledWidth(),
                (float) ClientBase.mc.getWindow().getGuiScaledHeight(), 0.0f).uv(1.0f, 0.0f).endVertex();
        bufferBuilder.vertex(pose, (float) ClientBase.mc.getWindow().getGuiScaledWidth(), 0.0f, 0.0f).uv(1.0f, 1.0f)
                .endVertex();
    ...
    public static void beginWriteFull(boolean keepColor, RenderTarget renderTarget, boolean clearStencil,
            boolean invertMask) {
    ...
    public static void attachStencilBuffer(RenderTarget renderTarget) {
        ...
        EXTFramebufferObject.glRenderbufferStorageEXT(36161, 34041, ClientBase.mc.getMainRenderTarget().width,
                ClientBase.mc.getMainRenderTarget().height);
