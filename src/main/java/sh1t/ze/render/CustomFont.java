...
public class CustomFont
    ...
    public void drawStringColor(PoseStack poseStack, String text, double x, double y, Color color) {
        this.drawStringRGB(poseStack, text, (float) x, (float) y, (float) color.getRed() / 255.0f,
                (float) color.getGreen() / 255.0f, (float) color.getBlue() / 255.0f, color.getAlpha());
    }
    ...
    public void drawStringRGBFull(PoseStack poseStack, String text, float x, float y, float baseR, float baseG,
            float baseB, float alpha, boolean rainbow, int rainbowOffset) {
        ...
        synchronized (this.glyphPageMap) {
            ...
            for (ResourceLocation textureLocation : this.glyphPageMap.keySet()) {
                ...
                for (GlyphEntry entry : entries) {
                    ...
                    bufferBuilder.vertex(pose, atX + 0.0F, atY + glyphHeight, 0.0F).uv(u1, v2).color(r, g, b, alpha)
                            .endVertex();
                    bufferBuilder.vertex(pose, atX + glyphWidth, atY + glyphHeight, 0.0F).uv(u2, v2)
                            .color(r, g, b, alpha).endVertex();
                    bufferBuilder.vertex(pose, atX + glyphWidth, atY + 0.0F, 0.0F).uv(u2, v1).color(r, g, b, alpha)
                            .endVertex();
                    bufferBuilder.vertex(pose, atX + 0.0F, atY + 0.0F, 0.0F).uv(u1, v1).color(r, g, b, alpha)
                            .endVertex();
                }
    ...
    public void drawStringCentered(PoseStack poseStack, String text, double x, double y, int color) {
        ...
        float b = (float) (color & 0xFF) / 255.0f;
        float a = (float) (color >> 24 & 0xFF) / 255.0f;
        this.drawStringRGB(poseStack, text, (float) (x - (double) (this.getStringWidth(text) / 2.0f)), (float) y, r, g,
                b, a);
    }

    public void drawStringCenteredColor(PoseStack poseStack, String text, double x, double y, Color color) {
        this.drawStringRGB(poseStack, text, (float) (x - (double) (this.getStringWidth(text) / 2.0f)), (float) y,
                (float) color.getRed() / 255.0f, (float) color.getGreen() / 255.0f, (float) color.getBlue() / 255.0f,
                (float) color.getAlpha() / 255.0f);
    }

    public void drawStringCenteredRGB(PoseStack poseStack, String text, float x, float y, float r, float g, float b,
            float a) {
    ...
    public float getStringHeight(String text) {
        ...
        for (char c : chars) {
            if (c == '\n') {
                if (lineHeight == 0.0f) {
                    lineHeight = this.getOrLoadGlyph(' ') == null ? 0.0f
                            : (float) ((Glyph) (Objects.requireNonNull((Object) (this.getOrLoadGlyph(' '))))).height()
                                    / (float) this.scale;
                }
    ...
    public float getFontHeight() {
        return (float) (this.fontMetrics.getLeading() + this.fontMetrics.getAscent() + this.fontMetrics.getDescent())
                / (float) this.scale;
    }
    ...
    private static String generateRandomName() {
        return IntStream.range(0, 32).mapToObj(i -> String.valueOf((char) new Random().nextInt(97, 123)))
